...
`[!]` **`03` 머리말은 이 결정 이전의 문구다 — 고치지 않기로 했다** (2026-09-14 사용자 결정).
그 문서는 여전히 자신을 `FINAL / READ-ONLY` 라 소개하고 「Navigation·Action 은 재봉인 절차
밖에서 변경할 수 없다」고 적는데, 둘 다 §1.1 이 놓은 것이다. 고치는 실질은 두 줄인데
`06` §4.1 회차표·`06` 버전·해시가 함께 열리고, `verify-docs.js` `V24` 가 「기준선이 생성기보다
앞서면 안 된다」를 git 이력으로 보므로 **고칠 내용 없는 `wireframe/screens/*` 커밋까지
강요된다.** 사내로 나가는 공개본은 이미 제 지위를 스스로 말한다(§3). 다시 계산하지 마라.
...
- **공개본 `03` 은 자기 지위를 스스로 말한다.** 설계 시점의 화면 정의이고 실행 프로그램과 다른
  부분이 있다는 한 줄이 첫 장 「범례 · 전제」에 실린다(§1.1 과 `winforms/AGENTS.md` 가 정한 그
  결정이다). 받아 보는 쪽은 이 저장소를 열지 않으므로 `AGENTS.md` 에 적어 두는 것으로는 닿지
  않는다 — 그 문장이 없으면 그림과 다른 프로그램을 결함으로 읽는다.
  **문구와 판정은 `tools/docgen/wireframe/screens/a0_list.js` 와 `verify_output.js` 가 한 쌍으로
  갖는다** — 여기에 베끼지 않는다. 빠지면 `exit 1` 이다.
