...
`[!]` **Codex 는 지침 예산을 넘으면 경고 없이 잘라낸다.** 전역 `~/.codex/AGENTS.md` 가 먼저
들어가고 프로젝트 파일이 뒤에 붙으므로 **잘리는 쪽은 이 저장소**다. 한도는 32,768 bytes 다.

```text
전역 ~/.codex/AGENTS.md   11,426
AGENTS.md (이 파일)        6,302
database/AGENTS.md         8,120
                          ------
                          25,848   OK
개발 킷 AGENTS.md 13,674 를 설치하면 39,522 -> **초과한다.**
그때 합치면서 줄인다. 줄이지 않고 설치하면 뒷부분이 조용히 사라진다.
```
...
R4  HC-RSV-RCP-20260908-R4   04·05 만 (SP 전면 한글화). 00~03 은 내용이 안 바뀌어 R3 표기 유지
```

**기준선 ID 는 세트가 아니라 문서마다 "마지막 봉인 회차" 를 뜻한다.** 그래서 지금 `00`~`03` 은 R3,
`04`~`06` 은 R4 이며, 이것은 어긋난 것이 아니라 그 문서가 마지막으로 열린 때를 적은 것이다.
...
여섯 종을 만들고 **반드시** `tools/docgen/verify_output.js` 를 지난다. 하나씩 손으로 돌리면 마지막
검사를 빼먹는다.

- `output/` 은 `.gitignore` 대상이라 **어떤 커밋에도 남지 않는다.** 재현 수단은 `tools/docgen` 뿐이다.
- 바이너리를 손으로 만지지 않는다. 산출물을 고치려면 **생성기를 고쳐 다시 돌린다.**
- **`06` 은 공개본을 만들지 않는다.** 내용 있는 줄의 27% 가 내부 어휘이고 값이 표가 아니라 산문에 있다.
  최종 상태는 원본 8종 · 공개본 7종이며 그 비대칭은 의도한 것이다.
- 공개본에 저장소 통제 어휘(`기준선`·`FINAL`·`Phase`·시험 판정·`회귀`·`커밋`·절 참조 …)를 내보내지 않는다.
  판정은 `verify_output.js` 가 하고 한 건이라도 남으면 `exit 1` 이다.

## 4. 디렉터리 경계

```text
docs/baseline/      봉인된 계약 00~06 (+ 07 예정) · output/
docs/phase4/        Phase 4 **기록** — plans/ · 초안 · 기각안. 계약은 여기 없다
...
R4 로 DB 계약이 한글이 되었고 화면·C# 계층은 영문을 유지한다(`05` §16.2). 두 계층이 공존하므로
**이름이 갈리는 지점이 생긴다.**

```text
00 · 01 · 03      화면·C# 계층    PatientId · WorkId · RowVersion …
04 · 05 · 06      DB 계약         수검자ID · 업무ID · 행버전 …
```

대응 26종의 **단일 출처는 `05` §16.5** 다. 다른 곳에 같은 표를 만들지 않는다.
`verify-docs.js` 의 `V20` 이 그 표를 양방향으로 지킨다 — 화면 이름이 `00`·`01`·`03` 에 없거나
DB 이름이 `05` 에 없으면 `FAIL` 이다.
