> # 폐기 (2026-09-07 사용자 결정)
>
> **계정·권한(`ROLE`·`USER`·`GRANT`·`DENY`)은 이 프로젝트의 산출물이 아니다.**
> 사용자 결정 그대로다 — "Security 는 Grant, Role 와 같은 계정·권한이라면 폐기한다".
>
> `NOT RUN` 으로 미루지 않고 **범위에서 뺐다.** 미룬 것이 아니므로 게이트에 잔여가 남지 않는다.
> 그래서 이 파일은 `plans/` 밖으로 옮겼다 — `tools/verify-docs.js` 의 `V03b` 가 계획서의 Test ID 를
> 스펙 카탈로그와 대조하는데, 폐기된 계획이 계속 `SEC-001`~`009`·`011` 을 주장하면 안 된다.
>
> **`SEC-010`(secret 스캔)만 살아 있다.** 그것은 계정·권한이 아니라 배포 원본·로그·보고서에
> 자격증명이 섞이지 않았는지 보는 검사이며 `scripts/verify-no-secret.sh` 로 매 회귀에서 판정된다.
>
> 아래 설계는 **실행하지 않는다.** 실무 이관 시의 참고 기록으로만 남긴다 (`06` §32 · §39 도 같다).
