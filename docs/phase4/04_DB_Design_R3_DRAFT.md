...
| 시트 | 행 | 무엇을 바꾸는가 |
|---|---|---|
| 1 `문서정보` | 3 · 4 · 7 | 문서 버전 `v1.2`→`v2.0` · 기준일 · 기준선 ID 2곳 |
| 2 `기능정의` | 25 · 35 · 37 | `F-RSV-003` 예약취소 · `F-RCP-003` 접수취소 · `F-COM-001` *"예약(RSV), 접수완료(RCP), 취소(CNL)"* |
| 3 `업무Rule` | 5 | `TGT-02` 의 `RSV/RCP/CNL` |
...
| 5 `설계근거` | 12 · 20 | *"CNL만 제외한다"* · *"CNL 상태 보존·복원금지"* |
| 6 `DB추적` | 4~19 | 물리 테이블명 전 행 · r6 에 `변경이력` 추가(`CP-06` 파급) · r9 `→CNL`→`CNR` · r12 `→CNL`→`CNC` · r16 제외정보 참조 제거 |
| 7 `최종검수` | 23 | *"RSV→RCP, RSV→CNL, RCP→CNL, 복원금지"* |
