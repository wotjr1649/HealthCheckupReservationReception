...
Parameter 11 / 11 / 2                      -> 12 / 12 / 3. R3 이 Write SP 8개에 @조작자명 을 더했다.
```

구현의 기준은 이 계획서의 SQL 본문이 아니라 `05` 계약과 `06` 스펙이다. 나머지 서술은 그대로 두었다.

# Stage 7 — 예약 Write Stored Procedure 3개 · 시간대 잠금
...
SELECT m.[검사항목코드]
FROM (VALUES ('OPT01', @추가검사01선택여부), ('OPT02', @추가검사02선택여부), ('OPT03', @추가검사03선택여부),
             ('OPT04', @추가검사04선택여부), ('OPT05', @추가검사05선택여부), ('OPT06', @추가검사06선택여부),
             ('OPT07', @추가검사07선택여부)) v ([추가검사코드], IsSelected)
JOIN [dbo].[검사코드] m ON m.[추가검사코드] = v.[추가검사코드]
...
```sql
DECLARE @자원시간대 NVARCHAR(255) =
    N'HC|SLOT|' + CONVERT(CHAR(8), @예약일, 112) + N'|' + @시간대코드;
```

문자열 오름차순이 곧 (날짜, 시간대) 오름차순이므로, 예약 이동에서 두 시간대을 잡을 때 **`ORDER BY 자원명 ASC`** 로 획득하면 교차이동 교착이 구조적으로 사라진다.
...
**Interfaces:**
- Consumes: 4개 TVF
- Produces: RS0 + RS1 `(업무ID BIGINT, 상태코드 CHAR(3), 행버전 BINARY(8))`
- Parameter 12개: `@수검자ID BIGINT`, `@예약구분 VARCHAR(10)`, `@예약일 DATE`, `@시간대코드 CHAR(2)`, `@추가검사01선택여부`~`@추가검사07선택여부 BIT`, `@조작자명 NVARCHAR(50)`

**applock:** `HC|PAT|{수검자ID}` → `HC|SLOT|{yyyyMMdd}|{AM|PM}`

**허용 결과코드:** `0, 100~102, 200, 300~306, 308~309, 400~401, 410~412, 700~701`
...
`[X]` **`INSERT … EXEC` 는 쓸 수 없다**(스펙 §33.1a). RS0 `결과코드` 판정은 계약 시나리오로, DB 결과 판정은 이 파일로 나눈다.

**계약 시나리오** — Parameter 순서(`05` §9.1): `@수검자ID, @예약구분, @예약일, @TimeSlotCode, @Opt01, @Opt02, @Opt03, @Opt04, @Opt05, @Opt06, @Opt07`
...
| `RWR-012` | `@Pf, 'NORMAL', '2026-11-16', 'AM', 0,0,0,0,0,0,0` | `305` | 20/20 시간대 |
| `RWR-032` | `@Pt, 'NORMAL', '2026-11-19', 'AM', 0,0,0,0,0,0,0` | `306` | 타 유효업무 **1건** (`05` §17, 스펙 §33.4) |
| `RWR-033` | `@P2, 'NORMAL', '2026-11-20', 'AM', 0,0,0,0,0,0,0` | `701` | 타 유효업무 **2건** — 1건과 2건은 다른 결과코드 다 |
...
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2020-01-06', 'AM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-22', 'AM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-12-25', 'AM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-21', 'PM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'WALKIN', '2026-11-16', 'AM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @P19,'NORMAL', '2026-11-17', 'AM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-17', 'AM', 0,0,1,0,0,0,0;
...
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-17', 'AM', 1,0,0,0,0,0,0;
...
DECLARE @저장국가검사건수 INT = (SELECT LEN([국가검사항목]) - LEN(REPLACE([국가검사항목], N',', N'')) + 1
                       FROM [dbo].[예약접수] WHERE [업무ID] = @W);
IF (@저장국가검사건수 BETWEEN 8 AND 11
...
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-18', 'AM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-19', 'AM', 0,0,0,0,0,0,0;
IF ((SELECT COUNT(*) FROM [dbo].[예약접수] WHERE [수검자ID] = @Pt) = @Before)
...
EXEC [dbo].[USP_HC_예약_등록] @P2, 'NORMAL', '2026-11-19', 'AM', 0,0,0,0,0,0,0;
EXEC [dbo].[USP_HC_예약_등록] @P2, 'NORMAL', '2026-11-20', 'AM', 0,0,0,0,0,0,0;
...
DECLARE @시간대 INT = (SELECT COUNT(*) FROM [dbo].[예약접수]
                      WHERE [예약일] = '2026-11-16' AND [시간대코드] = 'AM'
                        AND [상태코드] IN ('RSV', 'RCP'));
IF @시간대 = 20 PRINT 'PASS RWR-012 사전조건 20/20 성립';
ELSE BEGIN PRINT 'FAIL RWR-012 사전조건 시간대=' + CONVERT(VARCHAR(5), @시간대); SET @Fail += 1; END

DECLARE @P21 BIGINT = (SELECT [수검자ID] FROM [dbo].[수검자] WHERE [차트번호] = 'T013');
EXEC [dbo].[USP_HC_예약_등록] @P21, 'NORMAL', '2026-11-16', 'AM', 0,0,0,0,0,0,0;
...
`[I]` `RWR-043`(예약취소)이 취소할 Work 는 `ORDER BY 업무ID` 로 `F001` 을 고정한다 — `F020` 을 건드리지 않는다.
...
```text
[Transaction 밖]
 1. 필수값: @수검자ID / @예약구분 / @예약일 / @시간대코드 / AEX 7 BIT → NULL 이면 100
 2. 허용값: @예약구분 ∈ {NORMAL, WALKIN} 아니면 101 / @시간대코드 ∈ {AM, PM} 아니면 101
 3. 조합:  WALKIN 인데 @예약일 <> @오늘날짜 → 102, 오류항목='예약일'

[Transaction 안]
 4. applock  HC|PAT|{@수검자ID}
 5. applock  HC|SLOT|{yyyyMMdd}|{slot}
 6. Patient 존재                                        → 200
 7. 검사 Master 구성 확인 (Exam 19행 / AEX 7종)          → 700 ExamSetupError
 8. UFN_HC_일정확인(@서버시각, @오늘날짜, 'AM', 'NONE')
      현재업무가능=0                                      → 업무가능코드(308/309)
 9. 다른 유효업무 COUNT
      >= 2                                              → 701
      =  1                                              → 306, 오류항목='수검자ID'
10. UFN_HC_일정확인(@서버시각, @예약일, @시간대코드,
                   CASE WHEN @예약구분='WALKIN' THEN 'RECEPTION' ELSE 'NORMAL' END)
      일정가능=0                                          → 사유코드(300~304)
11. 정원 재조회
      COUNT(RSV+RCP) + 1 > 20                           → 305 SlotFull, 오류항목='시간대코드'
12. UFN_HC_검진대상확인  검진대상여부=0                      → 사유코드(400/401)
...
14. UFN_HC_추가검사확인  요청선택여부=1 인데 선택가능=0 인 행 존재
      → 그 행의 사유코드(410/411/412), 오류항목='AexOpt0nSelected' (추가검사코드 ASC 첫 건)
15. INSERT 예약접수 (StatusCode='RSV', CreationDate/최종수정일시 = @저장시각)
    SCOPE_IDENTITY() → @업무ID
16. [R3] 검사구성은 15 의 INSERT 안에서 두 컬럼으로 함께 저장한다. 별도 Detail INSERT 가 없다.
      국가검사항목 = UFN_HC_국가검사구성 결과를 검사항목코드 오름차순 쉼표 연결
      추가검사항목 = UFN_HC_추가검사확인 의 유효선택여부=1 을 같은 방식으로 연결. 0개면 NULL
17. COMMIT → RS0 + RS1 (업무ID, 'RSV', 새 행버전 재조회)
```

`행버전` 은 `COMMIT` **이후** `SELECT [행버전] FROM 예약접수 WHERE 업무ID=@업무ID` 로 다시 읽어 반환한다.
...
- [x] **Step 5: Commit** — `feat(phase4): USP_HC_예약_등록 구현 및 예약 경계 테스트 12건`
...
```sql
-- RWR-050  USP_HC_예약_등록 가 변경이력 1행을 남긴다 (성공·업무실패 각각)
...
**Interfaces:**
- Produces: RS0 + RS1 `(업무ID, 상태코드, 행버전)`
- Parameter 12개: `@업무ID BIGINT`, `@행버전 BINARY(8)`, `@예약일 DATE`, `@시간대코드 CHAR(2)`, `@추가검사01선택여부`~`@추가검사07선택여부 BIT`, `@조작자명 NVARCHAR(50)`

**applock:** `HC|PAT|{수검자ID}` → `HC|WORK|{업무ID}` → `HC|SLOT|…` (기존·신규, **자원명 오름차순**)

**허용 결과코드:** `0, 1, 100~102, 300~306, 308~309, 400~401, 410~412, 500, 502, 601, 700~701`
...
-- RWR-025 stale [행버전] → 601
-- RWR-026 [상태코드]=CNR 인 Work 변경 → 502
-- RWR-027 미존재 [업무ID] → 500
-- RWR-028 예약변경에서 자기 Work 를 306 으로 오인하지 않는다  (핵심)
-- RWR-029 예약일 변경 후 TGT 비대상 → 400 이고 기존 Work 완전 보존
-- RWR-030 20/20 시간대 을 그대로 유지하는 시간대 미변경 → [적용후인원]=20, 성공
```

`RWR-028` 구현 예:

**계약 시나리오** — Parameter 순서(`05` §9.2): `@업무ID, @행버전, @예약일, @TimeSlotCode, @Opt01..@Opt07`
...
EXEC [dbo].[USP_HC_예약_변경] @Ws, @Rv, '2026-11-17', 'PM', 1,0,0,0,0,0,0;
...
-- [X] 초안은 @Ws(T015, **남** 만 46세)를 썼다. NEX-05(EX012) 술어가 [성별]='F' AND [나이] IN (54,60,66)
...
BEGIN
    ...
    ELSE
    BEGIN
        EXEC [dbo].[USP_HC_예약_변경] @W20, @Rv20, '2026-11-20', 'AM', 0,0,0,1,0,0,0;
...
BEGIN
    ...
    EXEC [dbo].[USP_HC_예약_변경] @Wx, @Rvx, '2026-11-19', 'AM', 0,0,0,0,0,0,0;
...
`[X 실측]` `@CurrentAex`·`@RequestAex` 테이블 대신 요청 7 BIT 를 `@요청목록` 에 담고, `예약접수.추가검사항목` CSV 를 `검사코드` 에 양끝 패딩 `LIKE` 로 편 집합과 EXCEPT 양방향 비교한다.

```sql
DECLARE @예약일변경여부  BIT = CASE WHEN @현재예약일 <> @예약일 THEN 1 ELSE 0 END;
DECLARE @시간대변경여부  BIT = CASE WHEN @현재시간대코드 <> @시간대코드        THEN 1 ELSE 0 END;
DECLARE @추가검사변경여부 BIT = CASE
...
`[X 실측]` 6번의 "사전조회 수검자ID 와 재조회 수검자ID 가 다르면 THROW" 는 구현에서 `THROW` 가 아니라 `결과코드=701` 이다 — 손상은 RS0 계약 안에서 돌려준다.

```text
[Transaction 밖]
 1. 필수값·허용값 (AEX 7 BIT NULL 불허)
 2. 사전조회: SELECT @수검자ID = 수검자ID FROM 예약접수 WHERE 업무ID=@업무ID
       없으면 → 500 WorkNotFound  (트랜잭션을 열지 않는다)

[Transaction 안]
 3. applock  HC|PAT|{@수검자ID}
 4. applock  HC|WORK|{@업무ID}
 5. applock  HC|SLOT|… × (기존·신규, 중복 제거 후 자원명 오름차순)
 6. Work 재조회      없으면 500 WorkNotFound
    ※ `501 WrongPatient` 를 쓰지 않는다. `05` §13 의 `UPDATE_예약변경` 허용 결과코드 는
      `0, 1, 100~102, 300~306, 308~309, 400~401, 410~412, 500, 502, 601, 700~701` 로
      **501 이 없다**. 게다가 이 SP 는 `@수검자ID` Parameter 자체가 없어(§11.2 는 11개)
      "수검자ID 불일치" 를 판정할 외부 입력이 존재하지 않고, `EP-02` 에 의해 수검자ID 는
      불변이므로 사전조회값과 재조회값이 달라질 수도 없다 — 초안의 501 분기는 계약 위반이자 죽은 코드였다.
      사전조회 수검자ID 와 재조회 수검자ID 가 다르면 그것은 설계 위반이므로 `THROW` 한다.
 7. StatusCode <> 'RSV'                          → 502 WrongStatus
 8. 행버전 <> @행버전                     → 601 WorkChanged
 9. 변경범위 계산 (Step 2)
10. 셋 다 0                                       → 결과코드=1 NoChange, UPDATE 없이 COMMIT,
                                                    RS1 에 기존 행버전 반환
11. @예약일변경여부=1 이면 검사 Master 구성 확인      → 700
12. @추가검사변경여부=1 이면 저장 NEX 무결성 + AEX Master → 700 / 701
13. 공통 업무 가능 여부                            → 308 / 309
14. @예약일변경여부=1 또는 @시간대변경여부=1 이면
       다른 유효업무 (업무ID <> @업무ID)  2건↑ 701 / 1건 306
       UFN_HC_일정확인 (마감구분='NORMAL')       → 300~304
       정원  적용후인원 > 20                       → 305
15. @예약일변경여부=1 이면 TGT(400/401) → **저장 Work 무결성 3종**(스펙 §21.2a) → AEX(410~412)
    ...
    [X] 초안은 "NEX < 8" 하한만 봤다. 그러면 NEX 12행(CORRUPT-5)이 통과한다.
        ...
        네 SP 모두에 세 검사를 요구한다.
16. @예약일변경여부=0 이고 @추가검사변경여부=1 이면 저장 NEX 기준 AEX (410~412)
17. [R3] 저장은 언제나 예약접수 한 행의 조건부 UPDATE 다 (05 §11.2). 변경범위 밖 컬럼은 현재값을 그대로 둔다.
       예약일 포함 : 예약일 + 시간대코드 + 국가검사항목 + 추가검사항목
       시간대만    : 시간대코드
       AEX만       : 추가검사항목
       시간대+AEX  : 시간대코드 + 추가검사항목
       공통        : 최종수정일시. 같은 행이라 행버전이 자동으로 바뀐다
18. COMMIT → RS0 + RS1 (새 행버전 재조회)
```

- [x] **Step 4: `적용후인원` 계산 (스펙 §30.1)**

```sql
DECLARE @현재인원 INT =
    (SELECT COUNT(*) FROM [dbo].[예약접수]
      WHERE [예약일] = @예약일 AND [시간대코드] = @시간대코드
        AND [상태코드] IN ('RSV','RCP')
        AND [업무ID] <> @업무ID);                       -- 현재 Work 제외
DECLARE @적용후인원 INT = @현재인원 + 1;            -- 이 시간대 에 배치될 예정
IF @적용후인원 > 20 BEGIN SET @결과코드 = 305; SET @오류항목 = '[시간대코드]'; END
```

현재 Work가 이미 20/20 시간대에 있고 같은 시간대을 유지하면 `@현재인원 = 19`, `@적용후인원 = 20` 이 되어 통과한다. **21로 계산하면 안 된다.**

- [x] **Step 5: 시간대 자원 정렬 획득**
...
IF @자원A = @자원B SET @자원B = NULL;                       -- 같은 시간대 유지면 하나만
IF @자원B IS NOT NULL AND @자원B < @자원A                   -- 문자열 오름차순 정렬
BEGIN
    DECLARE @Tmp NVARCHAR(255) = @자원A; SET @자원A = @자원B; SET @자원B = @Tmp;
END

-- @자원A 획득 (index 문서 "applock 획득 블록" 그대로: [rc] 로깅 + -3 은 50002)
-- IF @자원B IS NOT NULL 이면 @자원B 획득
```
...
git commit -m "feat(phase4): USP_HC_예약_변경 구현 및 변경 Matrix 테스트 11건"
```
...
```sql
-- RWR-051  USP_HC_예약_변경 가 변경이력 1행을 남긴다 (성공·업무실패 각각)
DECLARE @H0 INT = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상테이블] = N'예약접수');
--   … 성공 호출 1회(값이 바뀌는 것) + 업무실패 호출 1회를 수행한다 …
--   EAV 는 성공한 변경만 남기므로 업무실패 호출은 행을 만들지 않는다.
DECLARE @H1 INT = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상테이블] = N'예약접수');
...
**applock:** `HC|WORK|{업무ID}` 만.

**허용 결과코드:** `0, 100, 500, 502, 601, 308~309`
...
```sql
-- RWR-040 미존재 [업무ID] → 500
-- RWR-041 stale [행버전] → 601
-- RWR-042 이미 CNR → 502
...
**계약 시나리오**: `RWR-043` = `EXEC [dbo].[USP_HC_예약_취소] @Wc, @RvC;` → 기대 RS0 `성공여부=1, 결과코드=0`.
...
```text
[Transaction 밖]  필수값 (@업무ID, @행버전) → 100
[Transaction 안]
...
cd /d/AIDEV/HealthCheckupReservationReception
git add database/deploy/06_Procedures_Reservation_Write.sql database/tests/06_Reservation_Write_Tests.sql
git commit -m "feat(phase4): USP_HC_예약_취소 구현 및 취소 테스트 5건"
```
...
```sql
-- RWR-052  USP_HC_예약_취소 가 변경이력 1행을 남긴다 (성공·업무실패 각각)
DECLARE @H0 INT = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상테이블] = N'예약접수');
--   … 성공 호출 1회(값이 바뀌는 것) + 업무실패 호출 1회를 수행한다 …
--   EAV 는 성공한 변경만 남기므로 업무실패 호출은 행을 만들지 않는다.
DECLARE @H1 INT = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상테이블] = N'예약접수');
