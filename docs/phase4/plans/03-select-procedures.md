...
```text
SEL-025   조회 SP 의 RS2 Capacity = 20 (AM·PM 동일)   scripts/verify-contract-all.sh · 시각 무관
CON-002   19/20 동시예약 → 최종 20                     저장 쪽 경계 · 창 안
CON-008   20/20 에서 WalkIn 신규 → 305                 저장 쪽 경계 · 창 안
```

`Capacity` 는 `@PatientId` 를 요구하므로 계약 루프 **앞**(픽스처가 살아 있을 때)에서 잰다 —
뒤에 두었더니 `099_cleanup` 이 지운 뒤라 `100 PatientId` 가 나왔다(실측).
