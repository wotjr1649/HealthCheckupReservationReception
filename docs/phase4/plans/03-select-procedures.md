...
- 시작 시 `DECLARE @서버시각 DATETIME2(7) = SYSDATETIME();` 을 한 번만 캡처하고 TVF에 그대로 전달한다.
- RS0의 5개 컬럼에 **반드시 명시적 `CAST`** 를 건다. 실패 시 RS0만 출력하고 `RETURN` 한다.
- 조회 0건은 실패가 아니다 — `성공여부=1, 결과코드=0` + RS1 0행 (`05` §3.4).
...
`[X]` **`T22` 는 시나리오를 *"SELECT SP 11개"* 로 셌다.** §45.2 는 `SEL` **전건**의 RS0 결과코드 판정을
`contract/` 에 맡긴다. SP 단위 11개로는 `SEL` 20건 중 9건이 기계 판정 없이 남아 §45.2 를 어긴다.
**시나리오는 `SEL` 1건당 파일 하나(20개) + `05` §9.11 변경범위 Cardinality 를 덮는 5개 = 25개다.**

```text
파일명   tests/contract/<NN>_<시나리오>.sql      NN 은 01~20 이 SEL-001~SEL-020 과 1:1, 21~25 가 변경범위. SEL-021~024 는 Test ID 명이다
...
소유     시나리오 .sql 은 그 SP 를 만드는 Task 가 함께 만든다 — RED 를 그 파일로 관측하기 때문이다.
         T22 는 검증기·기대값·러너를 만들고 변경범위 전용 21~25 를 더한다.
```
...
`[X]` **변경범위 시나리오의 `@업무ID` 는 `RSV` Work 여야 한다.** 표는 처음에 `23`·`24` 를 `T014` 로 적었는데
`T014` 의 Work 는 `tests/00b` 가 만든 `RCP` 라 SP 가 `502 WrongStatus` 로 끊는다. `T020` 의 `RSV` Work 를 쓴다.

`21`~`25` 는 Test ID 가 없다 — `05` §9.11 변경범위 Cardinality 를 덮는 형상 전용 시나리오이고,
...
- Produces: RS0 + RS1 `(오늘날짜 DATE, 요일명 NVARCHAR(10), 휴무일명 NVARCHAR(100), 운영시작시각 TIME(0), 운영종료시각 TIME(0), 업무일여부 BIT, 운영시간내여부 BIT, 현재업무가능 BIT, 차단코드 INT, 차단메시지 NVARCHAR(300))`

**금지사항:** Parameter를 추가하지 않는다(0개). 업무시간 밖이라고 실패시키지 않는다 — 조회는 항상 성공한다.

- [x] **Step 1: RED — 테스트 먼저**

| Test ID | 인자 | 기대 RS0 `결과코드` | 설명 |
|---|---|---:|---|
...
`[X]` 초안의 예시 본문은 `EXEC [dbo].[USP_HC_공통업무상태_조회] (인자 없음);` 이었다 —
...
"01_공통업무상태": {
  ...
  "rs0Code": 0,
  "resultSets": [
    { "columns": ["성공여부","결과코드","결과메시지","오류항목","서버시각"], "rows": 1 },
    { "columns": ["오늘날짜","요일명","휴무일명","운영시작시각","운영종료시각",
                  "업무일여부","운영시간내여부","현재업무가능","차단코드","차단메시지"], "rows": 1 }
  ]
}
```
...
DECLARE @적용후 VARCHAR(100) = /* @Before 와 같은 식 */ NULL;
...
`@적용후` 는 `@Before` 와 **같은 식을 그대로 한 번 더 쓴다.** 헬퍼 함수로 묶지 않는다 — 스칼라 UDF 는
...
Expected: exit **1**, `Msg 2812` — "USP_HC_공통업무상태_조회 저장 프로시저를 찾을 수 없습니다".
...
`UFN_HC_일정확인` 의 `휴무일명`·`업무일여부` 는 **요청일** 기준인데, 여기서는 요청일 = 오늘이므로 그대로 사용할 수 있다.

`[X]` **`운영시간내여부` 를 `업무가능코드` 로 역산하면 안 된다.** TVF의 `업무가능코드` 는 `오늘업무일=0 → 308` 이 `운영시간내여부=0 → 309` 보다 **먼저** 걸리므로, 휴무일이나 일요일에는 시각과 무관하게 `308` 이 나온다. 초안의 `CASE WHEN s.업무가능코드 = 309 THEN 0 ELSE 1 END` 는 **`2026-12-25`(금, 휴무일) 새벽 3시에 `운영시간내여부=1`** 을 보고한다. `현재업무가능=0` 이라 업무는 막히지만 화면에 표시되는 값이 거짓이다. **SP 가 `@서버시각` 으로 직접 계산한다.**
...
WHERE p.name = N'USP_HC_공통업무상태_조회' ORDER BY r.column_ordinal;"
```
...
git commit -m "feat(phase4): USP_HC_공통업무상태_조회 구현 및 RS0 패턴 확립"
```
...
**Interfaces:**
- Produces: RS0 + RS1 11컬럼 `(수검자ID, 차트번호, 성명, 주민번호, 생년월일, 성별, 휴대전화, 전화번호, 이메일, 우편번호, 주소)`, 정렬 `성명 ASC, 생년월일 ASC, 차트번호 ASC`

**금지사항:** 이름 `%검색어%` 포함검색 금지 — **접두검색** `성명 LIKE @성명 + '%'` 만 사용. 조건 없는 전체조회 금지.

**Parameter (05 §7.2 그대로 5개):** `@차트번호 NVARCHAR(100)`, `@성명 NVARCHAR(100)`, `@주민번호 VARCHAR(13)`, `@생년월일 VARCHAR(8)`, `@휴대전화 VARCHAR(13)` — 전부 NULL 허용.
...
| `SEL-003` | `N'T001', NULL, NULL, NULL, NULL` | `0` | 차트번호 정확검색 |
| `SEL-004` | `N'ZZZZ9999', NULL, NULL, NULL, NULL` | `0` | 조회 0건은 성공 |
| `SEL-005` | `NULL, NULL, '12345', NULL, NULL` | `101` | 주민번호 형식 101 |
...
"02_수검자목록_조건없음": { "sp": "USP_HC_수검자목록_조회", "rs0Success": 0, "rs0Code": 103 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"03_수검자목록_ChartNo": { "sp": "USP_HC_수검자목록_조회", "rs0Success": 1, "rs0Code": 0 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"04_수검자목록_0건": { "sp": "USP_HC_수검자목록_조회", "rs0Success": 1, "rs0Code": 0 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"05_수검자목록_주민번호형식": { "sp": "USP_HC_수검자목록_조회", "rs0Success": 0, "rs0Code": 101 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
```
...
2. @주민번호, @휴대전화 에서 '-' 제거
3. @주민번호 가 NOT NULL 인데 13자리 숫자가 아니면  → 101 BadValue, 오류항목='주민번호'
4. @생년월일   가 NOT NULL 인데 8자리 숫자·실제 날짜가 아니면 → 101, 오류항목='생년월일'
5. 5개가 모두 NULL 이면                                  → 103 NeedSearchCondition, 오류항목=NULL
6. RS0 성공 + RS1 조회
```
...
WHERE (@차트번호      IS NULL OR p.[차트번호]      =  @차트번호)
  AND (@성명         IS NULL OR p.[[성명]]         LIKE @성명 + N'%')
  AND (@주민번호 IS NULL OR p.[주민번호] =  @주민번호)
  AND (@생년월일     IS NULL OR p.[생년월일]     =  @생년월일)
  AND (@휴대전화  IS NULL OR p.[CelNumberS]   =  @휴대전화)
...
```text
수검자ID ← 수검자ID       차트번호 ← 차트번호        성명 ← 성명
주민번호 ← 주민번호 생년월일 ← 생년월일      성별 ← 성별
휴대전화 ← CelNumber     전화번호 ← TelNumber        이메일 ← EMail
우편번호 ← 우편번호           주소 ← 주소
```
...
- [x] **Step 4: Commit** — `feat(phase4): USP_HC_수검자목록_조회 구현`

**완료조건:** SP 배포 exit 0 + 시나리오 4개 단독 실행 exit 0. 계약 판정은 `T22` 다.

---

## Task T17: `[dbo].[USP_HC_수검자상세_조회]`

**목적:** `@수검자ID` 로 수검자 1행 상세를 반환한다.
...
**Interfaces:** Produces RS0 + RS1 14컬럼 `(… , 비고 NVARCHAR(MAX), 최종수정일시 DATETIME)`, 정확히 1행.

**허용 결과코드:** `0, 100, 200` (`05` §13).

- [x] **Step 1: RED**

| Test ID | 인자 | 기대 RS0 `결과코드` | 설명 |
|---|---|---:|---|
| `SEL-006` | `NULL` | `100` | 수검자ID 필수 100 |
...
```json
// tools/expected-contracts.json 발췌. rs0Code 는 확정값이다. resultSets 는 05 §7~§12 가 정의한 RS 형상을 옮겨 적는다.
"06_수검자상세_PatientId없음": { "sp": "USP_HC_수검자상세_조회", "rs0Success": 0, "rs0Code": 100 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"07_수검자상세_미존재": { "sp": "USP_HC_수검자상세_조회", "rs0Success": 0, "rs0Code": 200 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"08_수검자상세_정상": { "sp": "USP_HC_수검자상세_조회", "rs0Success": 1, "rs0Code": 0 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
```

- [x] **Step 2: 구현 명세**

```text
1. @수검자ID IS NULL       → 100 MissingValue, 오류항목='수검자ID'
2. Patient 미존재            → 200 PatientNotFound, 오류항목='수검자ID'
3. RS0 성공 + RS1 1행
```

RS1 컬럼: `수검자ID, 차트번호, 성명, 주민번호, 생년월일, 성별, 휴대전화(←CelNumber), 전화번호(←TelNumber), 이메일(←EMail), 우편번호, 주소, 상세주소, 비고, 최종수정일시`

- [x] **Step 3: GREEN + Commit** — `feat(phase4): USP_HC_수검자상세_조회 구현`

**완료조건:** SP 배포 exit 0 + 시나리오 3개 단독 실행 exit 0. 계약 판정은 `T22` 다.

---

## Task T18: `[dbo].[USP_HC_수검자유효업무_조회]`
...
**Interfaces:** Produces RS0 + RS1 7컬럼 `(업무ID, 예약일, 시간대코드, 상태코드, 상태명, 오늘여부, 행버전 BINARY(8))`

**허용 결과코드:** `0, 100, 200, 701`
...
SET NOCOUNT ON;
EXEC [dbo].[USP_HC_수검자유효업무_조회] @P0;
```

```json
// tools/expected-contracts.json 발췌. rs0Code 는 확정값이다. resultSets 는 05 §7~§12 가 정의한 RS 형상을 옮겨 적는다.
"09_수검자유효업무_0건": { "sp": "USP_HC_수검자유효업무_조회", "rs0Success": 1, "rs0Code": 0 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"10_수검자유효업무_2건701": { "sp": "USP_HC_수검자유효업무_조회", "rs0Success": 0, "rs0Code": 701 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
```
...
1. @수검자ID IS NULL   → 100
2. Patient 미존재        → 200
3. 위 조건 COUNT >= 2   → 701 WorkDataError, 오류항목='업무ID'
4. 그 외                → RS0 성공 + RS1 0행 또는 1행
상태명  RSV=N'예약' / RCP=N'접수완료' / CNR=N'예약취소' / CNC=N'접수취소'
오늘여부     CASE WHEN 예약일 = @오늘날짜 THEN 1 ELSE 0 END
```

- [x] **Step 3: GREEN + Commit** — `feat(phase4): USP_HC_수검자유효업무_조회 구현`

**완료조건:** SP 배포 exit 0 + 시나리오 2개 단독 실행 exit 0. `10_수검자유효업무_2건701` 의 RS0 에 `701` 이 실제로 보여야 한다. 계약 판정은 `T22` 다.

---

## Task T19: `[dbo].[USP_HC_예약접수목록_조회]`
...
**Interfaces:** Produces RS0 + RS1 11컬럼, 정렬 `예약일 ASC, 시간대코드 ASC, 성명 ASC, 업무ID ASC`

**허용 결과코드:** `0, 101, 103, 104`

- [x] **Step 1: RED**

| Test ID | 인자 | 기대 RS0 `결과코드` | 설명 |
|---|---|---:|---|
| `SEL-011` | `'2026-12-01', '2026-11-01', NULL, NULL, NULL` | `104` | 시작일>종료일 104 |
...
```json
// tools/expected-contracts.json 발췌. rs0Code 는 확정값이다. resultSets 는 05 §7~§12 가 정의한 RS 형상을 옮겨 적는다.
"11_예약접수목록_날짜역전": { "sp": "USP_HC_예약접수목록_조회", "rs0Success": 0, "rs0Code": 104 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"12_예약접수목록_조건없음": { "sp": "USP_HC_예약접수목록_조회", "rs0Success": 0, "rs0Code": 103 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"13_예약접수목록_날짜범위": { "sp": "USP_HC_예약접수목록_조회", "rs0Success": 1, "rs0Code": 0 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
```

- [x] **Step 2: 구현 명세**

```text
1. 문자열 정규화, @상태코드 는 UPPER
2. @상태코드 가 NOT NULL 인데 RSV/RCP/CNR/CNC 가 아니면 → 101, 오류항목='상태코드'
3. @시작일 > @종료일                          → 104 BadDateRange, 오류항목='시작일'
4. @시작일/@종료일/@차트번호/@성명/@상태코드 가 **전부 NULL** → 103
   — `05` §8.1 은 "**상태코드=NULL은** 조회조건으로 보지 않음" 이라고 NULL 을 한정했다.
     값이 있는 `@상태코드` 는 실질 조건으로 센다. 초안은 한정어를 지워
...
- [x] **Step 3: GREEN + Commit** — `feat(phase4): USP_HC_예약접수목록_조회 구현`

**완료조건:** SP 배포 exit 0 + 시나리오 3개 단독 실행 exit 0. 계약 판정은 `T22` 다.

---

## Task T20: `[dbo].[USP_HC_예약접수상세_조회]`
...
**허용 결과코드:** `0, 100, 500, 701`

**금지사항:** `RS4` 를 5행이 아닌 개수로 반환하지 않는다. 업무시간 밖이라고 실패시키지 않는다 — `허용여부=0` 으로 반환한다.
...
```json
// tools/expected-contracts.json 발췌. rs0Code 는 확정값이다. resultSets 는 05 §7~§12 가 정의한 RS 형상을 옮겨 적는다.
"14_예약접수상세_미존재": { "sp": "USP_HC_예약접수상세_조회", "rs0Success": 0, "rs0Code": 500 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"15_예약접수상세_NEX0행701": { "sp": "USP_HC_예약접수상세_조회", "rs0Success": 0, "rs0Code": 701 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"16_예약접수상세_정상": { "sp": "USP_HC_예약접수상세_조회", "rs0Success": 1, "rs0Code": 0 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
```
...
```sql
[정원]     = 20
[현재인원] = (SELECT COUNT(*) FROM 예약접수 x
                 WHERE x.[예약일] = w.[예약일]
                   AND x.TimeSlotCode    = w.TimeSlotCode
                   AND x.StatusCode IN ('RSV','RCP'))
[잔여자리]    = CASE WHEN 20 - [현재인원] < 0 THEN 0 ELSE 20 - [현재인원] END
```

`RS2` = `ExamSourceCode='NEX'` 인 Detail 을 `검사코드` 와 JOIN, `검사항목코드 ASC`.
`RS3` = `ExamSourceCode='AEX'` 인 Detail, `추가검사코드 ASC`.

`RS4` 는 **고정 5행**을 `VALUES` 로 만들고 각 행에 허용조건을 평가한다.

```sql
SELECT
      [업무동작코드]    = CAST(a.[결과코드] AS VARCHAR(30))
    , [허용여부]       = CAST(... AS BIT)
    , [사유코드]    = CAST(... AS INT)
    , [사유메시지] = CAST(... AS NVARCHAR(300))
FROM [dbo].[예약접수] w
CROSS JOIN (VALUES ('EDIT_RESERVATION'),('CANCEL_RESERVATION'),('START_RECEPTION'),
                   ('EDIT_EXTRA'),('CANCEL_RECEPTION')) a([결과코드])
CROSS APPLY [dbo].[UFN_HC_일정확인](@서버시각, w.[예약일], w.[시간대코드], 'RECEPTION') s
WHERE w.[업무ID] = @업무ID
...
```

`[X]` **상관 인자를 받는 TVF 는 `CROSS APPLY` 여야 한다.** 초안은 `CROSS JOIN [dbo].[UFN_HC_일정확인](…, w.예약일, …)` 였는데, 일반 `JOIN` 형태의 TVF 호출은 같은 `FROM` 절 다른 테이블의 컬럼을 인자로 받을 수 없어 **`Msg 4104 — 다중 파트 식별자 "w.예약일"를 바인딩할 수 없습니다`** 가 난다. 게다가 `w` 가 `FROM` 절에 존재하지도 않았다. `T13`·`T14` 는 `CROSS APPLY`/`OUTER APPLY` 를 올바로 썼는데 여기만 어긋났다.
...
- [x] **Step 3: GREEN + Commit** — `feat(phase4): USP_HC_예약접수상세_조회 구현 (RS0~RS4)`

**완료조건:** SP 배포 exit 0 + 시나리오 3개 단독 실행 exit 0. `RS4` 5행은 `T22` 파서가 검증한다.

---

## Task T21: `[dbo].[USP_HC_예약가능정보_조회]`

**목적:** 예약 화면이 필요로 하는 모든 사전정보를 **6개 Result Set** 으로 반환한다. 15개 SP 중 가장 복잡하다.

**관련 Baseline 위치:** `05` §9 전체 (§9.2 signature, §9.3 NULL 조합, §9.4 변경범위, §9.5 RS 순서, §9.6~§9.11 각 RS, §9.12 저장가능, §9.13 평가순서).
...
**허용 결과코드:** `0, 100~102, 200, 500~502, 601, 700~701`

**금지사항:** 휴무일·정원마감·TGT 비대상을 SP 실패로 만들지 않는다 — `RS0.성공여부=1, 결과코드=0` + `RS1.저장가능=0` + `차단코드` 로 반환한다 (`05` §3.3). `변경범위=NONE` 에 `결과코드=1` 을 쓰지 않는다 (`결과코드=1` 은 Write SP No-op 전용).

- [x] **Step 1: RED — NULL 조합 오류 6종 + 변경범위 Cardinality**

| Test ID | 인자 | 기대 RS0 `결과코드` | 설명 |
|---|---|---:|---|
| `SEL-017` | `@Pn, NULL, 0x0000000000000001, 'NORMAL', '2026-11-16', 'AM', 0,0,0,0,0,0,0` | `102` | 업무ID NULL + 행버전 있음 102 |
...
```json
// tools/expected-contracts.json 발췌. rs0Code 는 확정값이다. resultSets 는 05 §7~§12 가 정의한 RS 형상을 옮겨 적는다.
"17_예약가능정보_RowVersion단독": { "sp": "USP_HC_예약가능정보_조회", "rs0Success": 0, "rs0Code": 102 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"18_예약가능정보_AEX_NULL": { "sp": "USP_HC_예약가능정보_조회", "rs0Success": 0, "rs0Code": 100 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"19_예약가능정보_WALKIN_날짜불일치": { "sp": "USP_HC_예약가능정보_조회", "rs0Success": 0, "rs0Code": 102 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
"20_예약가능정보_휴무일": { "sp": "USP_HC_예약가능정보_조회", "rs0Success": 1, "rs0Code": 0 },   // resultSets 는 T22 Step 2 스키마에 SP 계약대로 채운다
```

- [x] **Step 2: 구현 — `05` §9.13 평가순서를 그대로 따른다**

`[X 실측]` 계획서 순서표는 `05` §9.3 오류조합 6종을 4단계에서 한 번에 판정하는데, 구현은 5종만 4단계에 두고 `기존 날짜 유지 + 시간대코드 NULL` 1종을 6단계(존재·소유·상태·동시성) 뒤로 미뤘다 ― 그 조합은 Work 행이 있어야 판정할 수 있다.

```text
 1. @서버시각 캡처
 2. 필수값        @수검자ID / @예약구분 / @예약일 가 NULL → 100
                 AEX 7 BIT 중 하나라도 NULL → 100
                 @업무ID NOT NULL 인데 @행버전 NULL → 100
                 ※ 초안은 @예약일·@예약구분 을 필수값 목록에서 빠뜨렸다.
                   @예약구분 NOT IN ('NORMAL','WALKIN') 는 NULL 에 대해 UNKNOWN 이라
                   101 도 발생하지 않아 NULL 이 변경범위 계산까지 흘러들어간다.
                   05 §9.6 RS1 의 예약구분·예약일 는 NOT NULL 이고
                   05 §5 는 "1. 필수값 누락 → 2. 값 형식·허용값" 순서를 고정했다.
 3. 허용값        @예약구분 ∉ {NORMAL, WALKIN} → 101
 4. Parameter 조합 05 §9.3 오류표 6종 → 102
 5. Patient 존재  → 200
 6. @업무ID 있으면  Work 존재(500) → Patient 일치(501) → 상태코드=RSV(502) → 행버전(601)
 7. 변경범위 계산     예약일변경여부 / 시간대변경여부 / 추가검사변경여부 (§28.1 EXCEPT 양방향)
 8. 변경범위=NONE     → RS0 결과코드=0, RS1 저장가능=0 차단코드=0, RS2~RS5 전부 0행 후 종료
 9. ALL 이면        검사 Master 구성 확인 → 700 ExamSetupError
10. EXTRA/SLOT_EXTRA 이면 AEX Master + 저장 NEX 무결성 → 700 / 701
11. 현재 공통 업무 가능 여부 (UFN_HC_일정확인, @마감구분='NONE')
12. ALL/SLOT/SLOT_EXTRA 이면 다른 유효업무(2건↑ 701, 1건 다른업무ID) → 일정 → AM/PM 정원
13. ALL 이고 일정 진행 가능하면 TGT → NEX → AEX
14. EXTRA/SLOT_EXTRA 이면 저장 NEX 기준 AEX
15. RS0 ~ RS5 반환
```

`RS2` `적용후인원` (`05` §9.7 / 스펙 §30.1):

RS2는 AM·PM 두 행을 반환하며, **각 행을 "이 시간대를 선택한다면" 이라는 가정으로** 평가한다.

```sql
-- 표시용 [현재인원] 는 05 §9.7 대로 현재 Work 를 포함한 실제 인원이다
[현재인원]  = (SELECT COUNT(*) FROM 예약접수
                  WHERE [예약일] = @예약일
                    AND TimeSlotCode    = s.[시간대코드]          -- 'AM' / 'PM'
...
ExcludingSelf = [현재인원]
                - CASE WHEN @업무ID IS NOT NULL
                        AND EXISTS (SELECT 1 FROM 예약접수
                                     WHERE [업무ID] = @업무ID
                                       AND [예약일] = @예약일
                                       AND TimeSlotCode    = s.[시간대코드]
                                       AND StatusCode IN ('RSV','RCP'))
                       THEN 1 ELSE 0 END

[적용후인원]    = ExcludingSelf + 1
[잔여자리]     = CASE WHEN 20 - [적용후인원] < 0 THEN 0 ELSE 20 - [적용후인원] END
SlotFull      = ([적용후인원] > 20)
```

이 하나의 식이 `05` §9.7의 세 경우를 모두 만족한다.

| 경우 | `ExcludingSelf` | `적용후인원` | `05` §9.7 기대값 |
|---|---|---|---|
| 신규예약 (`@업무ID` NULL) | `현재인원` | `현재인원 + 1` | `현재인원 + 1` ✓ |
| 기존 Work 가 같은 시간대 유지 | `현재인원 - 1` | `현재인원` | `현재인원` ✓ |
| 기존 Work 가 다른 시간대 으로 이동 | `현재인원` | `현재인원 + 1` | `현재인원 + 1` ✓ |

**주의:** 20/20 시간대 을 그대로 유지하는 Work 는 `ExcludingSelf=19`, `적용후인원=20`, `선택가능=1` 이 된다. **21로 계산하면 안 된다.**

`RS1` `차단코드` 우선순위 (`05` §9.6):

```text
308/309 → 306 → 선택 시간대 의 300~305 → (시간대코드 미선택이고 AM/PM 모두 불가면) 307
        → 400/401 → 요청한 무효 AEX 410~412 (추가검사코드 ASC 첫 건)
```

`저장가능` 는 `05` §9.12의 변경범위별 조건식을 그대로 구현한다.

변경범위별 RS Cardinality (`05` §9.11) — 반드시 지킨다:
...
- [x] **Step 5: Commit** — `feat(phase4): USP_HC_예약가능정보_조회 구현 (RS0~RS5)`
...
- Create: `tests/contract/21_예약가능정보_Scope_ALL.sql` ~ `tests/contract/25_예약가능정보_Scope_NONE.sql` (변경범위 전용 5개. `01`~`20` 은 `T15`~`T21` 이 이미 만들었다)
...
각 파일은 SP를 한 번 호출하기만 한다. `01`~`20` 은 `T15`~`T21` 이 이미 만들었으므로 여기서는 변경범위 전용 `21`~`25` 만 만든다. 예: `tests/contract/01_공통업무상태.sql`
...
`SELECT_예약가능정보` 는 변경범위별로 5개 파일을 더 만든다 (`ALL` / `SLOT` / `EXTRA` / `SLOT_EXTRA` / `NONE`) — `05` §9.11 Cardinality 전용이라 Test ID 가 없다.
...
```json
{
  ...
  "06_예약접수상세": {
    "resultSets": [
      { "columns": ["성공여부","결과코드","결과메시지","오류항목","서버시각"], "rows": 1 },
      { "columns": ["업무ID","수검자ID","차트번호","성명","생년월일","성별","휴대전화",
                    "예약일","시간대코드","상태코드","상태명",
                    "정원","현재인원","잔여자리","행버전"], "rows": 1 },
      { "columns": ["검사항목코드","검사항목명","국가검사구분","국가검사규칙코드"], "rowsMin": 8, "rowsMax": 11 },
      { "columns": ["추가검사코드","검사항목코드","검사항목명"], "rowsMin": 0, "rowsMax": 7 },
      { "columns": ["업무동작코드","허용여부","사유코드","사유메시지"], "rows": 5 }
    ]
  }
}
```
...
`[X]` 파서는 컬럼명·RS 개수·행수뿐 아니라 **RS0 첫 행의 `성공여부`·`결과코드` 값을 읽어야 한다.** 스펙 §36.4 가 요구하는 것은 형상 일치가 아니라 *"관측 `결과코드` = 기대값"* 과 *"관측 `결과코드` ∈ `05` §13 허용집합"* 두 가지다. `expected-contracts.json` 의 각 항목은 `sp`·`rs0Success`·`rs0Code` 를 갖고, 허용집합은 `tools/allowed-codes.json` 에서 읽는다(`T36` Step 5).

RS0 첫 데이터행은 위에서 만든 `sets[0]` 의 헤더 바로 다음 줄이다 — `|` 로 분리해 `columns` 와 짝지으면 `성공여부`·`결과코드` 를 이름으로 꺼낼 수 있다.
...
const text = buf.slice(0, 2).equals(Buffer.from([0xff, 0xfe]))
  ? buf.slice(2).function toString() { [native code] }('utf16le')
  : buf.function toString() { [native code] }('utf8');
...
// RS0 의 첫 데이터행은 따로 보관한다 — 스펙 §36.4 는 형상이 아니라 결과코드 값 일치를 요구한다.
const sets = [];
...
if (expected.rs0Success !== undefined && Number(obsSuccess) !== expected.rs0Success) {
  bad(`FAIL ${key} RS0.성공여부 관측 ${obsSuccess} != 기대 ${expected.rs0Success}`); fail++;
}
if (expected.rs0Code !== undefined && Number(obsCode) !== expected.rs0Code) {
  bad(`FAIL ${key} RS0.결과코드 관측 ${obsCode} != 기대 ${expected.rs0Code}`); fail++;
}
...
if (fs.existsSync(allowedPath)) {
  ...
  if (Array.isArray(allowed) && !allowed.includes(Number(obsCode))) {
    bad(`FAIL ${key} RS0.결과코드 ${obsCode} 가 ${expected.sp} 의 허용집합 밖`); fail++;
  }
} else {
...
if (fail === 0) say(`PASS ${key} Result Set 계약 일치 (${sets.length}개 RS · RS0 결과코드=${obsCode})`);
process.exit(fail === 0 ? 0 : 1);
```

- [x] **Step 4: RED — 기대값을 일부러 틀리게 두고 실패를 확인**

`expected-contracts.json` 의 `01_공통업무상태` RS1 컬럼에서 `오늘날짜` 를 `Todayx` 로 바꾼 뒤 실행한다.
...
**완료조건:** RED에서 실제 FAIL 관측 + 스펙 §45.2 의 `SEL` 전건이 계약 판정으로 PASS + 변경범위 5개 시나리오 PASS.

`[X]` 초안은 시나리오를 SP 단위 11개로 셌다. 그렇게 세면 `SEL` 20건 중 9건이 판정되지 않는다 — 위 **계약 시나리오 규약** 표가 단일 출처다.

---

## Task T22b: `[dbo].[USP_HC_변경이력_조회]` (`SP-LOG-01`)

**목적:** 대상 행 하나의 변경기록을 최신순으로 낸다. `00` CP-06 이 변경기록을 **열람용**으로 규정한 것을 받는 유일한 조회 SP 다.

**관련 Baseline 위치:** `05` §8.3 (입력·RS·정렬·계약 경계), `05` §13 (허용 결과코드 `0`·`100`·`101`), `04` §8.6.3·§8.6.4, `03` §23 (`DLG-LOG-01`).
...
**Interfaces:**
- Parameter 2개: `@대상테이블 NVARCHAR(10)`, `@대상키 BIGINT` (둘 다 `NOT NULL`)
- Produces: RS0 + RS1 `(이력ID BIGINT, 기록일시 DATETIME2(0), 조작자명 NVARCHAR(50), 컬럼명 NVARCHAR(30), 변경전 NVARCHAR(4000), 변경후 NVARCHAR(4000))`
- 정렬 `기록일시 DESC, 이력ID DESC` — `IX_변경이력_TARGET` 의 Key(`대상테이블, 대상키, 기록일시 DESC`)가 술어와 정렬을 그대로 덮는다
...
|---|---|---|---|
| `SEL-021` | `N'수검자'`, 변경이력 최초 `수검자` 대상키 | `성공여부=1, 결과코드=0` | 정상 조회. RS1 최소 1행 |
| `SEL-022` | `N'예약접수'`, `-1` | `성공여부=1, 결과코드=0` | 기록 0건 → RS1 **0행**. `200` 이 아니다 |
| `SEL-023` | `N'완료이력'`, `1` | `결과코드=101` | `CK_변경이력_TARGET_TABLE` 도메인 밖 |
| `SEL-024` | `NULL`, `1` | `결과코드=100` | 필수값 누락 |
...
```text
정규화       @대상테이블 LTRIM/RTRIM, 빈 문자열 → NULL. 한글 값이라 UPPER 하지 않는다
필수값       @대상테이블 NULL → 100 / @대상키 NULL → 100
값 형식      @대상테이블 NOT IN (N'수검자', N'예약접수') → 101
RS0          결과코드 <> 0 이면 RS0 만 내고 RETURN
...
## `[X 실측 2026-09-08]` `SEL-025` — 정원이 조회와 저장에서 갈리는 것을 막는다

정원 `20` 은 조회 SP 8곳·저장 SP 2곳에 리터럴로 박혀 있고 **어떤 시험도 둘의 일치를 보지 않았다**.
한쪽만 고치면 화면은 "자리 있음" 을 보여주고 저장은 `305` 를 낸다 — 조용히 갈린다.

설정 테이블로 빼는 것은 오히려 기준선 위반이다 (`00` RP-03 이 정책 수치로 확정했고 관리자 CRUD 는 `00` §8.2 밖).
대신 **양쪽을 실행으로 고정**했다.

```text
SEL-025   조회 SP 의 RS2 정원 = 20 (AM·PM 동일)   scripts/verify-contract-all.sh · 시각 무관
CON-002   19/20 동시예약 → 최종 20                     저장 쪽 경계 · 창 안
CON-008   20/20 에서 WalkIn 신규 → 305                 저장 쪽 경계 · 창 안
```

`정원` 는 `@수검자ID` 를 요구하므로 계약 루프 **앞**(픽스처가 살아 있을 때)에서 잰다 —
뒤에 두었더니 `099_cleanup` 이 지운 뒤라 `100 수검자ID` 가 나왔다(실측).
