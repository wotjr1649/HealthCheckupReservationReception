...
| 산출물 | 시트 | 실측 |
|---|---|---|
| `04_…DB설계서.xlsx` | 논리ERD · 물리ERD · 테이블 6 · 컬럼 48 · 제약 43 · 인덱스 8 · 명명규칙 12 | FAIL 0 |
...
### 14.1 ERD (2026-09-08 추가)

초판 `build_04.js` 는 기준선 `04` §4.5(논리 ERD)를 **통째로 읽지 않았다.** 사용자가 지적했다.

```text
논리 ERD   04 §4.1 Entity · §4.4 Cardinality · §4.5 mermaid 에서 그린다
물리 ERD   기준선에 없다 — §7 요약과 §8 컬럼·키에서 유도한다
```

`exceljs 4.4` 는 도형을 못 그리고 이 환경에 래스터라이저가 없어 한글이 든 PNG 를 만들 수 없다.
**셀로 그렸다** — 테두리·채움·병합만 쓰므로 선택·검색·인쇄가 된다.

`[!]` **점선은 관계가 아니다.** §4.5 산문이 "검사코드는 코드로 참조하므로 FK 관계선이 없다" 고
적은 것을 눈에 보이게 옮긴 것이고, 그림과 범례가 그렇게 이름 붙인다. 실선은 FK 2개뿐이다.

`[X]` **열 너비 `9` 는 파일에 안 남는다.** `exceljs` 가 자기 기본값과 같다고 보고 `<col>` 을
생략하는데 Excel 실제 기본은 8.43 이다(실측·재현). 그림이 조용히 어긋나므로 `emit()` 이
왕복 대조하게 만들고 음성시험으로 확인했다 — 9 로 되돌리면 `FAIL 3열`.

좌표는 손으로 뒀다. 테이블이 6개가 아니거나 §4.4 관계가 2건이 아니면 `erd_04.js` 가 먼저 멈춘다.
`node tools/docgen/xlsx/inspect_erd.js` 가 만들어진 시트를 텍스트로 되그려 배치를 판정한다 —
그림은 오류 없이 만들어져도 상자가 겹칠 수 있다.
