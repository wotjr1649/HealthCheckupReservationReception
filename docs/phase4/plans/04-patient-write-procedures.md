...
## 공통 — 주민번호 파생값 산출 (두 SP 동일)

`05` §10.1의 7번째 자리 표를 그대로 구현한다.

```sql
-- @주민번호 는 이미 정규화(13자리 숫자)되었다고 가정
DECLARE @주민7번째자리 CHAR(1) = SUBSTRING(@주민번호, 7, 1);
-- [X] 1800년대(@주민7번째자리 IN ('9','0')) 분기를 두면 안 된다. CK_수검자_SOCIAL_FORMAT 이 7번째 자리를
--     1~8 로 묶으므로(04 §8.1.3) SP 가 통과시켜도 저장 단계에서 계산열 NOT NULL 이 Msg 515 를
--     내며 죽는다. ResultCode 가 아니라 예외로 튀어 RS0 계약이 깨진다. 여기서 101 로 거부한다.
DECLARE @출생세기 VARCHAR(2) =
    CASE WHEN @주민7번째자리 IN ('1','2','5','6') THEN '19'
         WHEN @주민7번째자리 IN ('3','4','7','8') THEN '20'
         ELSE NULL END;
DECLARE @성별 CHAR(1) =
    CASE WHEN @주민7번째자리 IN ('1','3','5','7') THEN 'M'
         WHEN @주민7번째자리 IN ('2','4','6','8') THEN 'F'
         ELSE NULL END;
DECLARE @생년월일 VARCHAR(8) = @출생세기 + SUBSTRING(@주민번호, 1, 6);

-- 검증
IF @출생세기 IS NULL OR @성별 IS NULL
   OR TRY_CONVERT(DATE, @생년월일, 112) IS NULL
BEGIN SET @결과코드 = 101; SET @오류항목 = '[주민번호]'; END
```

- 체크디지트와 실제 행정번호 존재 여부는 **검증하지 않는다** (`00` §2.1, `04` §3.5).
- UI가 계산한 생년월일/성별를 Parameter로 받지 않는다.
- `[!]` **이 블록의 `@생년월일` 는 저장용이 아니라 후보검색용이다.** *"이름 + 산출 생년월일 동일 후보"* 판정은
  아직 존재하지 않는 행의 생년월일을 필요로 하므로 SP 가 변수로 파생한다. **저장값은 계산열이 만든다** ―
  `INSERT`/`UPDATE` 컬럼 목록에 `[생년월일]`·`[성별]` 을 넣지 않는다(`Msg 271`).
- `@성별` 는 검증용이다. 저장에도 검색에도 쓰지 않는다.
...
## Task T23: `[dbo].[USP_HC_수검자_등록]`

**목적:** 신규 수검자를 등록하고, 동일 주민번호·중복후보를 계약대로 처리한다.

**관련 Baseline 위치:** `05` §10.1 (입력·RS·결과표·검증순서), `00` EP-03~09, `04` §3.6 (차트번호 Sequence), 스펙 §21.2·§24.1.
...
**Interfaces:**
- Produces: RS0 + RS1 `(수검자ID BIGINT, 차트번호 NVARCHAR(100), 성명 NVARCHAR(100), 주민번호 VARCHAR(13), 생년월일 VARCHAR(8), 성별 CHAR(1), 휴대전화 VARCHAR(13), 최종수정일시 DATETIME)`
- Parameter 12개: `@차트번호자동발급여부 BIT`, `@차트번호 NVARCHAR(100)`, `@성명 NVARCHAR(100)`, `@주민번호 VARCHAR(13)`, `@휴대전화 VARCHAR(13)`, `@전화번호 VARCHAR(13)`, `@이메일 VARCHAR(200)`, `@우편번호 VARCHAR(10)`, `@주소 NVARCHAR(200)`, `@상세주소 NVARCHAR(200)`, `@비고 NVARCHAR(MAX)`, `@유사수검자확인여부 BIT`

**applock:** `HC|SSN|{sha256}` → `HC|CHART|{차트번호 또는 발급후보}` (**수동·자동 모두**, 스펙 §12.6·§22)

**허용 결과코드:** `0, 2, 100~102, 201~203, 206, 308~309`
...
`[X 실측]` `PWR-013` 은 계획서가 `결과코드=101` 을 기대했으나 구현·계약은 `결과코드=2` 다 — `@주민번호 VARCHAR(13)` 경계에서 14자리가 절단되어 `PWR-001` 이 만든 행과 같은 값이 된다(`tests/contract/PWR-013_주민번호_14자리_경계절단.sql`).

`[X 실측]` 업무시간 가드는 `[Active]` 가 아니라 `[사용여부]` 이고, 창 밖에서 `SKIP` + `RETURN` 하는 대신 `PWR-OFF`(창 밖 호출이 DB 를 바꾸지 않는다)를 실제로 판정한다.

`[X]` **`INSERT … EXEC` 는 쓸 수 없다.** `INSERT_수검자` 는 성공 시 RS0 + RS1 을 반환하므로 `Msg 213` 이다(스펙 §33.1a). RS0 `결과코드` 판정은 **계약 시나리오**로, DB 에 실제로 남은 결과 판정은 **이 파일**로 나눈다.

**계약 시나리오** — 각 행이 `tests/contract/<Test ID>.sql` 한 파일(`EXEC` 한 번, 단언 없음)과 `tools/expected-contracts.json` 한 항목이 된다.

Parameter 순서(`05` §8.1): `@차트번호자동발급여부, @차트번호, @성명, @주민번호, @CelNumber, @TelNumber, @이메일, @우편번호, @주소, @Job, @비고, @유사수검자확인여부`

| Test ID | 인자 | 기대 RS0 | 설명 |
|---|---|---|---|
| `PWR-001` | `1, NULL, N'신규수검자', '9001011000018', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `성공여부=1, 결과코드=0` | 신규등록 성공 (자동 차트번호) |
| `PWR-002` | `1, NULL, N'신규수검자', '9001011000018', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `성공여부=1, 결과코드=2` | 동일 주민번호 + **동일 이름** → 기존 수검자 반환 |
| `PWR-003` | `1, NULL, N'다른이름', '9001011000018', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `성공여부=0, 결과코드=202` | 동일 주민번호 + 다른 이름 (실패인데 RS1 동반) |
| `PWR-004` | `1, NULL, N'짧은번호', '900101100001', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `결과코드=101` | 주민번호 12자리 |
| `PWR-005` | `1, NULL, N'잘못된날짜', '9002311000012', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `결과코드=101` | 존재하지 않는 생년월일 (02-31) |
| `PWR-007` | `0, NULL, N'차트없음', '9101011000015', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `결과코드=102` | `차트번호자동발급여부=0` 인데 `차트번호` NULL |
| `PWR-008` | `0, N'T001', N'중복차트', '9101011000015', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `결과코드=201` | 수동 차트번호 중복 |
| `PWR-009` | `1, NULL, N'테스트일구', '0610024000015', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `결과코드=203` | `T001` 과 이름·생년월일 동일, 주민번호는 다름 → 유사후보 미확인 |
| `PWR-010` | `1, NULL, N'테스트일구', '0610024000015', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 1` | `성공여부=1, 결과코드=0` | 같은 인자에 `@유사수검자확인여부=1` → 별도 등록 |
| `PWR-013` | `1, NULL, N'열네자리', '90010110000188', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `결과코드=101` | 주민번호 14자리 (`05` §17.6) |
| `PWR-014` | `1, NULL, N'비숫자', '900101A000018', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0` | `결과코드=101` | 7번째 자리가 `'A'` — 비숫자 포함 (`05` §17.6) |
...
-- PWR-011  [생년월일]/[성별] 가 SP 산출값과 일치한다 (Fixture 가 직접 지정한 값과 같아야 한다)
EXEC [dbo].[USP_HC_수검자_등록] 1, NULL, N'산출확인', '9001011000018', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0;
IF EXISTS (SELECT 1 FROM [dbo].[수검자]
            WHERE [주민번호] = '9001011000018'
              AND [생년월일] = '19900101' AND [[성별]] = 'M')
    PRINT 'PASS PWR-011 [생년월일]/[성별] 가 주민번호 산출값과 일치';
ELSE BEGIN PRINT 'FAIL PWR-011'; SET @Fail += 1; END

-- PWR-012  자동 발급 [차트번호] 형식 = '[코드]' + 6자리
IF EXISTS (SELECT 1 FROM [dbo].[수검자]
            WHERE [주민번호] = '9001011000018'
              AND [차트번호] LIKE '[코드][0-9][0-9][0-9][0-9][0-9][0-9]'
              AND LEN([차트번호]) = 7)
    PRINT 'PASS PWR-012 자동 [차트번호] 형식 [코드] + 6자리';
...
IF NOT EXISTS (SELECT 1 FROM [dbo].[수검자]
                WHERE [[성명]] IN (N'짧은번호', N'잘못된날짜', N'차트없음', N'중복차트', N'열네자리', N'비숫자'))
    PRINT 'PASS PWR-004/005/007/008/013/014 실패 경로가 행을 남기지 않았다';
ELSE BEGIN PRINT 'FAIL 실패 경로가 행을 남겼다'; SET @Fail += 1; END
```

`[R3]` 이 SP 는 Parameter 목록 **맨 끝**에 `@조작자명 NVARCHAR(50)` 을 받는다(`05` §19.2). 아래 시험의 모든 호출은 마지막 인자로 `@조작자명 = N'TEST'` 를 명시 전달한다 — `05` §2.1 이 선택 Parameter 의 생략을 금지한다.
...
```sql
-- PWR-030  USP_HC_수검자_등록 가 변경이력 1행을 남긴다 (성공·업무실패 각각)
...
`[X 실측]` Parameter 는 12개가 아니라 14개다 — R3 이 `@B형간염제외여부` 와 맨 끝 `@조작자명` 을 더했다.

`[X 실측]` 1번의 "휴대전화/전화번호 에서 '-' 제거" 는 구현하지 않았다 — `05` §2.2 는 '-' 제거를 검색값으로 한정했고 `04` §8.1.2 는 표시값 그대로 저장한다.

```text
[Transaction 밖]
 1. 문자열 정규화: LTRIM/RTRIM, 빈 문자열 → NULL, 휴대전화/전화번호 에서 '-' 제거
 2. @성명 IS NULL 또는 공백                        → 100 MissingValue, 오류항목='성명'
 3. @주민번호 13자리 숫자 아님                  → 101 BadValue, 오류항목='주민번호'
 4. 7번째 자리 코드 / 6자리 날짜 유효성 → 생년월일·성별 산출
    실패 시                                        → 101, 오류항목='주민번호'
 5. @차트번호자동발급여부=1 인데 @차트번호 NOT NULL          → 102 BadRequest, 오류항목='차트번호'
    @차트번호자동발급여부=0 인데 @차트번호 NULL              → 102 BadRequest, 오류항목='차트번호'
 6. @유사수검자확인여부 IS NULL                 → 100

[Transaction 안]
 7. applock  HC|SSN|{sha256(@주민번호)}
 8. applock  HC|CHART|{@차트번호}          (수동 입력)
 ...
 9. UFN_HC_일정확인(@서버시각, @오늘날짜, 'AM', 'NONE') → 현재업무가능=0 이면 업무가능코드(308/309)
10. 동일 주민번호 기존 Patient 조회
      있고 성명 동일  → 결과코드=2  ExistingPatient, 성공여부=1, RS1=기존 1행, COMMIT 후 반환
      있고 성명 다름  → 결과코드=202 SameNumberDifferentName, 성공여부=0, RS1=기존 1행
11. @유사수검자확인여부=0 이고 (성명 + 산출 생년월일) 동일 후보가 있으면
                        → 결과코드=203 SimilarPatient, 성공여부=0, RS1=후보 N행
12. 수동 차트번호 가 이미 존재      → 201 ChartNoUsed, 오류항목='차트번호'
13. 자동 차트번호 발급 (아래 Step 3)
14. 수검자 1행 INSERT, SCOPE_IDENTITY() 로 수검자ID 확보
15. COMMIT → RS0 성공 + RS1 신규 1행
```

`202`/`203` 은 **실패지만 RS1을 동반하는 유일한 예외**다 (`05` §3.5).

- [x] **Step 3: 자동 차트번호 발급 루프**
...
```sql
---- [2] Transaction 밖 ------------------------------------------------------
DECLARE @순번값 BIGINT = NULL, @후보차트번호 NVARCHAR(100) = NULL, @Exhausted BIT = 0;
IF @차트번호자동발급여부 = 1
BEGIN
    BEGIN TRY
        SET @순번값 = NEXT VALUE FOR [dbo].[SEQ_HC_CHART_NO];
        SET @후보차트번호 = N'[코드]' + RIGHT(N'000000' + CONVERT(NVARCHAR(6), @순번값), 6);
    END TRY
    BEGIN CATCH
        IF ERROR_NUMBER() = 11728 SET @Exhausted = 1;   -- Sequence MAXVALUE 도달
        ELSE THROW;
    END CATCH
    IF @Exhausted = 1 BEGIN SET @결과코드 = 206; SET @오류항목 = '[차트번호]'; END
END
...
WHILE @후보차트번호 IS NOT NULL AND @Exhausted = 0
BEGIN
    ...
    BEGIN TRAN;
        ...
        IF @잠금결과 < 0 BEGIN ROLLBACK; IF @잠금결과 = -3 THROW 50002, N'잠금 교착', 1; ELSE THROW 50001, N'잠금 실패', 1; END

        SET @자원차트번호 = N'HC|CHART|' + @후보차트번호;
        ...
        IF NOT EXISTS (SELECT 1 FROM [dbo].[수검자] WHERE [차트번호] = @후보차트번호)
        BEGIN
...
EXEC [dbo].[USP_HC_수검자_등록] 1, NULL, N'실패유도', '900101100001', NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0;
...
git commit -m "feat(phase4): USP_HC_수검자_등록 구현 및 등록 경계 테스트 12건"
```
...
## Task T24: `[dbo].[USP_HC_수검자정보_수정]`

**목적:** 수검자 정보를 수정하고, 주민번호 변경 시 활성 업무 존재를 차단하며, `최종수정일시` 를 단조 증가시킨다.
...
**Interfaces:**
- Produces: RS0 + RS1 `(수검자ID BIGINT, 차트번호 NVARCHAR(100), 최종수정일시 DATETIME)`
- Parameter 12개: `@수검자ID BIGINT`, `@최종수정일시 DATETIME`, `@차트번호 NVARCHAR(100)`, `@성명 NVARCHAR(100)`, `@주민번호 VARCHAR(13)`, `@휴대전화`, `@전화번호`, `@이메일`, `@우편번호`, `@주소`, `@상세주소`, `@비고`

**applock:** `HC|SSN|{sha256}` → `HC|CHART|{차트번호}` → `HC|PAT|{수검자ID}` — **조건 없이 항상** (스펙 §24.1)

`[X]` **"변경 시에만" 으로 두면 안 된다.** 변경 여부를 알려면 현재 행을 먼저 읽어야 하고, 자연스러운 구현이 `PAT`(랭크 3)을 먼저 잡은 뒤 `SSN`(랭크 1)을 잡게 되어 **§23 전역 순서가 뒤집힌다.** 게다가 자원명은 **요청값** `@주민번호`·`@차트번호` 로 바로 만들 수 있으므로 사전조회가 애초에 필요 없다.

**허용 결과코드:** `0, 1, 100~102, 200~201, 204~205, 600, 308~309`

**금지사항:** 주민번호 변경으로 기존 Work의 TGT/NEX/AEX를 자동 재판정하거나 취소하지 않는다. 생년월일/성별를 Parameter로 받지 않는다.
...
**계약 시나리오** — Parameter 순서(`05` §8.2): `@수검자ID, @최종수정일시, @차트번호, @성명, @주민번호, @CelNumber, @TelNumber, @이메일, @우편번호, @주소, @Job, @비고`

| Test ID | 인자 요지 | 기대 RS0 | 설명 |
|---|---|---|---|
| `PWR-020` | `T015` 의 현재값 12개를 그대로 | `성공여부=1, 결과코드=1` | No-op |
| `PWR-021` | `@최종수정일시 = '2000-01-01'` | `결과코드=600` | stale `최종수정일시` |
| `PWR-022` | 현재 `@Led` + `@성명 = N'이름변경됨'` | `성공여부=1, 결과코드=0` | 실제 변경 |
| `PWR-023` | `F001` + 새 `@주민번호` (RSV 보유) | `결과코드=205` | 주민번호 변경 + **RSV** 활성 Work |
| `PWR-024` | `F001` + `@차트번호 = N'F001X'` (RSV 보유) | `성공여부=1, 결과코드=0` | 차트번호 변경은 활성 Work 가 있어도 **차단하지 않는다** (`00` EP-08) |
| `PWR-025` | `T015` + `T001` 의 `주민번호` | `결과코드=204` | 다른 Patient 의 주민번호 |
| `PWR-026` | `@수검자ID = -1` | `결과코드=200` | 미존재 Patient |
| `PWR-027` | `T014` + 새 `@주민번호` (**RCP** 보유) | `결과코드=205` | 주민번호 변경 + **RCP** 활성 Work (`05` §17, 스펙 §33.4) |
| `PWR-028` | `T014` + `@차트번호 = N'T014X'` (RCP 보유) | `성공여부=1, 결과코드=0` | 차트번호 변경은 RCP 가 있어도 차단하지 않는다 |
...
SELECT @Pid = [수검자ID], @Led = [최종수정일시], @Ssn = [주민번호], @Nm = [[성명]]
  FROM [dbo].[수검자] WHERE [차트번호] = 'T015';

-- PWR-020  No-op 은 [최종수정일시] 를 건드리지 않는다
SET @Before = @Led;
EXEC [dbo].[USP_HC_수검자정보_수정] @Pid, @Led, N'T015', @Nm, @Ssn, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
IF ((SELECT [최종수정일시] FROM [dbo].[수검자] WHERE [수검자ID] = @Pid) = @Before)
    PRINT 'PASS PWR-020 No-op 은 [최종수정일시] 불변';
ELSE BEGIN PRINT 'FAIL PWR-020 No-op 이 [최종수정일시] 를 바꿨다'; SET @Fail += 1; END

-- PWR-022  실제 변경은 [최종수정일시] 를 반드시 증가시킨다
--   DATETIME 의 해상도는 약 3.33ms 다. @Before 를 한 번만 읽고 '>' 로 비교한다.
SELECT @Led = [최종수정일시] FROM [dbo].[수검자] WHERE [수검자ID] = @Pid;
SET @Before = @Led;
EXEC [dbo].[USP_HC_수검자정보_수정] @Pid, @Led, N'T015', N'이름변경됨', @Ssn, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
IF ((SELECT [최종수정일시] FROM [dbo].[수검자] WHERE [수검자ID] = @Pid) > @Before
    AND (SELECT [[성명]] FROM [dbo].[수검자] WHERE [수검자ID] = @Pid) = N'이름변경됨')
    PRINT 'PASS PWR-022 실제 변경 시 [최종수정일시] 단조증가 + 값 반영';
...
DECLARE @Nf NVARCHAR(100) = (SELECT [[성명]] FROM [dbo].[수검자] WHERE [수검자ID] = @Pf);
...
EXEC [dbo].[USP_HC_수검자정보_수정] @Pf, @Lf, N'F001', @Nf, @NewSsn, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
...
EXEC [dbo].[USP_HC_수검자정보_수정] @Pf, @Lf, N'F001X', @Nf, @Sf, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
...
-- 되돌린다 — 뒤 테스트가 F001 을 [차트번호] 로 찾는다
SELECT @Lf = [최종수정일시] FROM [dbo].[수검자] WHERE [수검자ID] = @Pf;
EXEC [dbo].[USP_HC_수검자정보_수정] @Pf, @Lf, N'F001', @Nf, @Sf, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
...
DECLARE @Nr NVARCHAR(100) = (SELECT [[성명]] FROM [dbo].[수검자] WHERE [수검자ID] = @Pr);
...
ELSE
BEGIN
    EXEC [dbo].[USP_HC_수검자정보_수정] @Pr, @Lr, N'T014', @Nr, @NewSsn, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
    ...
    EXEC [dbo].[USP_HC_수검자정보_수정] @Pr, @Lr, N'T014X', @Nr, @Sr, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
    ...
    SELECT @Lr = [최종수정일시] FROM [dbo].[수검자] WHERE [수검자ID] = @Pr;
    EXEC [dbo].[USP_HC_수검자정보_수정] @Pr, @Lr, N'T014', @Nr, @Sr, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
END
...
SELECT @Led = [최종수정일시] FROM [dbo].[수검자] WHERE [수검자ID] = @Pid;
EXEC [dbo].[USP_HC_수검자정보_수정] @Pid, @Led, N'T015', N'이름변경됨', @S1, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
...
`[X 실측]` Parameter 12개 → 14개(`@B형간염제외여부`·`@조작자명`)이고, 9번 No-op 의 NULL-safe INTERSECT 비교도 `B형간염제외여부` 를 포함한다.

```text
[Transaction 밖]
 1. 문자열 정규화
 2. 필수값: @수검자ID, @최종수정일시, @차트번호, @성명, @주민번호 → NULL 이면 100
 3. @주민번호 형식·날짜·세기/성별 → 생년월일·성별 산출. 실패 시 101

[Transaction 안]
 4. applock  HC|SSN|{sha256(@주민번호)}   (조건 없이 항상 — 요청값으로 자원명을 만든다)
 5. applock  HC|CHART|{@차트번호}              (조건 없이 항상 — 요청값으로 자원명을 만든다)
 6. applock  HC|PAT|{@수검자ID}
 7. Patient 존재                          → 없으면 200
 8. 저장된 최종수정일시 <> @최종수정일시   → 600 PatientChanged, 오류항목='최종수정일시'
 9. 12개 입력이 현재값과 전부 동일         → 결과코드=1 NoChange, UPDATE 없이 COMMIT,
                                             RS1 에 기존 최종수정일시 반환
    ※ 비교는 반드시 **NULL-safe** 여야 한다 (스펙 §28.2).
       <> 로 비교하면 NULL <> 'x' 가 UNKNOWN 이라 "변경 없음" 으로 오판하고
       사용자가 입력한 비고·삭제한 이메일 이 조용히 소실된다.
       ...
       비고 는 NVARCHAR(MAX) 라 INTERSECT 피연산자가 될 수 없어 분리 비교한다.
10. 공통 업무 가능 여부                    → 308 / 309
11. 차트번호 변경 시 다른 Patient 고유성    → 201
12. 주민번호 변경 시 다른 Patient 고유성 → 204 SocialNumberUsed
13. 주민번호 변경 시 해당 Patient 의 StatusCode IN ('RSV','RCP') 존재 → 205 SocialChangeBlocked
14. 최종수정일시 단조증가 계산 후 조건부 UPDATE
15. COMMIT → RS0 + RS1
```

- [x] **Step 3: `최종수정일시` 단조증가 + 조건부 UPDATE**
...
UPDATE [dbo].[수검자]
   SET [차트번호] = @차트번호, [성명] = @성명, [주민번호] = @주민번호
     -- [생년월일]·[성별] 은 계산열이라 여기에 쓸 수 없다. @주민번호 를 바꾸면 자동으로 다시 유도된다.
     , [휴대전화] = @휴대전화
     , [전화번호] = @전화번호
     , [이메일] = @이메일, [우편번호] = @우편번호, [주소] = @주소
     , [상세주소] = @상세주소, [비고] = @비고
     , [최종수정일시] = @새최종수정일시
 WHERE [수검자ID]    = @수검자ID
   AND [최종수정일시] = @기존최종수정일시;

IF @@ROWCOUNT = 0 BEGIN SET @결과코드 = 600; SET @오류항목 = '[최종수정일시]'; END
```
...
`@휴대전화` 검색은 `REPLACE(p.[휴대전화],'-','') = @휴대전화` 으로 하며 인덱스 seek 이 아니다
...
`[X 실측 2026-09-07]` **`PWR-032` 신설 — 대소문자만 바꾼 수정이 조용히 버려졌다.**
정렬이 `Korean_Wansung_CI_AS` 라 No-op 판정의 `INTERSECT` 가 `'a@B.com'` 과 `'a@b.com'` 을
같다고 본다(실측). `결과코드=1` 이 돌아오고 편집이 사라졌다 — NULL 함정을 막으려던 그 자리에
정렬 함정이 남아 있었다. **감사 필터도 같은 CI 비교**였으므로 둘을 함께 바이트 비교로 바꿨다.

```sql
-- COLLATE 는 06 §9.2 허용목록 밖이다. VARBINARY 변환은 허용목록 안이고 정렬과 무관하다.
SELECT CONVERT(VARBINARY(400), @기존이메일) INTERSECT SELECT CONVERT(VARBINARY(400), @이메일)
-- 감사 필터도 같은 기준으로. 한쪽만 고치면 저장은 되는데 이력에 한 줄도 안 남는다.
```

`PWR-032` 는 저장값과 감사행을 **둘 다** 보고, `PWR-032b` 는 진짜 무변경이 여전히 No-op 인지 본다.
저장값 비교도 CI 라 `=` 로는 구분되지 않아 시험 쪽도 VARBINARY 로 본다.
...
sqlcmd -S '.\SQLEXPRESS' -E -d HealthCheckupReservationReceptionDb -b -I -h -1 -W -Q "
SET NOCOUNT ON;
DECLARE @P BIGINT=(SELECT 수검자ID FROM 수검자 WHERE 차트번호='T015');
DECLARE @S VARCHAR(13)=(SELECT 주민번호 FROM 수검자 WHERE 수검자ID=@P);
DECLARE @L0 DATETIME=(SELECT 최종수정일시 FROM 수검자 WHERE 수검자ID=@P);  -- 최초 1회만 읽는다
DECLARE @L DATETIME=@L0, @i INT=0, @Nm NVARCHAR(100);
WHILE @i<5
BEGIN
  SET @Nm = N'반복' + CONVERT(NVARCHAR(3), @i);   -- 매 회 실제 변경이어야 No-op 이 아니다
  EXEC dbo.USP_HC_수검자정보_수정 @P, @L, N'T015', @Nm, @S, NULL,NULL,NULL,NULL,NULL,NULL,NULL;
  SELECT @L = 최종수정일시 FROM 수검자 WHERE 수검자ID=@P;   -- SP 가 갱신한 값을 다음 호출에 넘긴다
  SET @i+=1;
END
SELECT CASE WHEN @L > @L0 THEN 'PASS 4MS-001 5회 연속 수정 후 최종수정일시 증가'
            ELSE 'FAIL 4MS-001 최종수정일시 미증가' END
     + '  최초=' + CONVERT(VARCHAR(30), @L0, 121) + '  최종=' + CONVERT(VARCHAR(30), @L, 121);
SELECT 'final=' + CONVERT(varchar(30), 최종수정일시, 121) FROM 수검자 WHERE 수검자ID=@P;"
```

Expected: `PASS 4MS-001`, 최종 `최종수정일시` > 최초.

`[X]` **초안의 루프는 보정 유무를 구분하지 못했다.** 매 회 `최종수정일시` 를 **다시 읽어서** 전달했으므로, `+4ms` 보정이 없어 `@새최종수정일시 = @기존최종수정일시` 이 되어도 조건부 `UPDATE` 는 `@@ROWCOUNT=1` 로 성공하고 `600` 이 나지 않는다. 통과해도 아무 증거가 아니었다. **최초 값을 한 번만 읽고, SP 가 갱신한 값을 이어받으며, 최종 > 최초를 단언**해야 단조증가를 실증한다.
...
cd /d/AIDEV/HealthCheckupReservationReception
git add database/deploy/05_Procedures_Patient_Write.sql database/tests/05_Patient_Write_Tests.sql
git commit -m "feat(phase4): USP_HC_수검자정보_수정 구현 및 수정·동시성 테스트 7건"
```
...
```sql
-- PWR-031  USP_HC_수검자정보_수정 가 변경이력 1행을 남긴다 (성공·업무실패 각각)
DECLARE @H0 INT = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상테이블] = N'수검자');
--   … 성공 호출 1회(값이 바뀌는 것) + 업무실패 호출 1회를 수행한다 …
--   EAV 는 성공한 변경만 남기므로 업무실패 호출은 행을 만들지 않는다.
DECLARE @H1 INT = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상테이블] = N'수검자');
