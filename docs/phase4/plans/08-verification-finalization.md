...
- 문서 버전      v0.4 → v1.0 (SQL 실행검증 완료 시점에 CANDIDATE 해제) → v1.1 (R4 한글화 반영, §46) → v1.2 (R5 — §42 의 G00·G13·G16 정정 + 회차 C) → v1.3 (R6 — §42 의 미래 단정 제거) → v1.4 (R7 — 계약 재설계: 휴무일 CRUD SP 4개 · 행버전 통일 · 공휴일 Seed 2년치) → v1.5 (R8 — 리뷰 정정: 선거일 Seed · No-op 바이트 비교 · 게이트 fail-open 셋)
