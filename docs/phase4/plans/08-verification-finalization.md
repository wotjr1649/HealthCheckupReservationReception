...
`[X 실측]` **R3 반영으로 두 단언이 넓어졌다.** `VER-005` 는 `PK 7 / FK 2 / UQ 2 / UX 1 / NCI 5`, `VER-007` 은 `CHECK 28 / DEFAULT 10 / Seed Exam 19 / Holiday 41` 을 함께 센다 (R13 반영) (R7 반영. 그 전에는 `CHECK 24 / DEFAULT 8 / Holiday 2` 였다). 계획 초안의 `NCI 4` 는 `IX_변경이력_TARGET` 신설로 5 가 됐다.
...
- 문서 버전      v0.4 → v1.0 (SQL 실행검증 완료 시점에 CANDIDATE 해제) → v1.1 (R4 한글화 반영, §46) → v1.2 (R5 — §42 의 G00·G13·G16 정정 + 회차 C) → v1.3 (R6 — §42 의 미래 단정 제거) → v1.4 (R7 — 계약 재설계: 휴무일 CRUD SP 4개 · 행버전 통일 · 공휴일 Seed 2년치) → v1.5 (R8 — 리뷰 정정: 선거일 Seed · No-op 바이트 비교 · 게이트 fail-open 셋) → v1.6 (R9 — Seed 건수 사본 제거: 06 §33·§44 두 문장과 verify-contract-all.sh 의 0/41) → v1.7 (R10 — 03 재봉인에 딸린 §4.1 회차 갱신. SQL·Seed·계약은 한 줄도 바뀌지 않았다) → v1.8 (R11 — 03·05 재봉인에 딸린 §4.1 회차 갱신. SQL·Seed·계약은 한 줄도 바뀌지 않았다) → v1.9 (R12 — 00·03·05 재봉인에 딸린 §4.1 회차 갱신에 더해, 이 문서가 두 번째로 갖고 있던 수검자 Write 계약을 05 와 맞췄다: §21.2 두 행 · §15.2 fixture 이유 · §33.2a · §43 의 12·23) → v1.10 (R13 — 04 재봉인에 딸린다: 운영시간·마감시각을 04 §8.7 [운영기준] 으로 옮기면서 §14a 신설 · §33.2a 업무일 분기 · §42 회차 R13 · §43 의 1·12·14·21) → v1.11 (R14 — R13 잔재: 세 문서 본문의 옛 테이블 수와 05 머리말의 옛 SP 수. 수를 세는 게이트 V25 를 놓았다) → v1.12 (R15 — 남은 위험 닫기: §42 창 안 회차를 논거로 해소 · §43-22 를 Phase 5 범위에서 해소 · 새 게이트 G17·V26 등재) → v1.13 (R16 — 수검자 목록의 최소 1개 조건을 걷는다: 03 §5.3 · 04 §11.3 · 05 §7.2 재봉인 + §43-31 한계 등재. 동결을 연 첫 회차) → v1.14 (R17 — 차트번호·이름 포함검색: 03 §5.3 · 04 §11.3·§8.1.5 · 05 §7.2 재봉인 + §43-32 한계 등재) → v1.15 (R18 — SELECT_예약접수상세 에 RS5 추가검사구성 7행 추가: 05 §8.2 재봉인. DLG-RCP-02 가 AEX 를 읽는 길이 없었다. 기존 RS0~RS4 는 한 칸도 안 바뀐다) → v1.16 (R19 — R18 이 NOT RUN 으로 미룬 전체 회귀를 실제로 돌려 §42 에 회차 R19 를 등재하고(exit 0 · PASS 411 · FAIL 0 · SKIP 0 · NOT RUN 1), §1 문서정보에 R4 값으로 굳어 있던 버전·기준일·기준선 ID 사본을 걷었다. 계약·SQL·Seed·시험은 한 줄도 바뀌지 않았다) → v1.17 (R20 취소 SP 통합) → v1.18 (R21 — 수검자상세·수검자유효업무를 수검자목록 RS1 로 들인다. SP 19 → 17) → v1.19 (R22 — 자체휴무일 등록·수정을 저장 하나로. SP 17 → 16) → v1.20 (R24 — §18 SP Matrix 의 거짓 수치 정정 + 게이트 V27 신설. 계약·SQL·시험은 한 줄도 안 바뀐다)
