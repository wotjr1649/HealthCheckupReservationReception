...
`[X 실측]` **R3 반영으로 두 단언이 넓어졌다.** `VER-005` 는 `PK 6 / FK 2 / UQ 2 / UX 1 / NCI 5`, `VER-007` 은 `CHECK 26 / DEFAULT 10 / Seed Exam 19 / Holiday 41` 을 함께 센다 (R7 반영. 그 전에는 `CHECK 24 / DEFAULT 8 / Holiday 2` 였다). 계획 초안의 `NCI 4` 는 `IX_변경이력_TARGET` 신설로 5 가 됐다.
