...
|---|---|---|---|---|
| `CON-001` | 동일 SocialNumber 동시등록 | `INSERT_수검자` | `INSERT_수검자` (동일 SSN) | 해당 SSN의 `수검자` 행 = **1** **그리고 로그에 `Msg 2627` 0건** |
...
DECLARE @Nex NVARCHAR(100) = (SELECT [국가검사항목] FROM [dbo].[예약접수] WHERE [수검자ID] = @P);
DECLARE @Now INT = (SELECT COUNT(*) FROM [dbo].[UFN_HC_국가검사구성](@P, @ReservationDate));
DECLARE @Saved INT = CASE WHEN LEN(ISNULL(@Nex, N'')) = 0 THEN 0
                          ELSE LEN(@Nex) - LEN(REPLACE(@Nex, N',', N'')) + 1 END;
...
```text
AM Slot 을 쓰면   09:00 ~ 10:50      (마감 11:00, 경계 10분 여유)
PM Slot 을 쓰면   11:00 ~ 15:50      (마감 16:00, 경계 10분 여유)   <- 창이 넓다
```

`CON-005`·`CON-008` 은 **PM Slot** 으로 구성한다. AM 으로 두면 하루에 두 시간도 안 되는 창에
...
| 3 | `CON-004` 인자 부족 | `UPDATE_수검자정보` 는 **전체치환형**이라 `@LastEditDate`·`@ChartNo`·`@Name` 이 전부 필수다. 주민번호만 넣으면 `100` 으로 끝난다 | 현재 값을 barrier 앞에서 읽어 넘겨 **주민번호만** 바꾼다 |
...
| 15 | ~~**접수취소 후 당일 재접수 불가**~~ `[한계가 아니었다 — 2026-09-08]` | — | **오판이었다.** 16:00 이후엔 접수완료도 당일예약도 `304` 라 그 시간에 오접수가 **생길 수 없고**, 다음 날 이후 예약은 `CutoffTime NULL` 로 열려 있다(실측). 막히는 것은 당일 복구뿐이며 그것은 접수마감 정책이 의도한 바다. 각 규칙은 문서화돼 있었는데 그 **조합**을 잘못 읽어 구멍으로 분류했다 (§44.8-9) |
...
| 21 | **정원 20 이 조회 SP 와 저장 SP 에 따로 박혀 있다** `[완화 2026-09-08]` | 조회 SP 8곳·저장 SP 2곳에 리터럴로 있다. 한쪽만 고치면 화면은 "자리 있음" 을 보여주고 저장은 `305` 를 낸다 | 설정 테이블은 오히려 기준선 위반이다(`00` RP-03 이 정책 수치로 확정 · 관리자 CRUD 는 §8.2 밖). 대신 **양쪽을 실행으로 고정**했다 — `SEL-025` 가 조회 SP 의 `Capacity` 를 매 회귀에서(시각 무관) 보고, `CON-002`(19/20→20)·`CON-008`(20/20→305)이 저장 쪽을 경계에서 본다. 한쪽만 바뀌면 둘 중 하나가 반드시 깨진다 |
| 22 | **`700` 과 `701` 의 상대 순서가 SP 마다 반대다** | `UPDATE_예약변경` 은 `700 → 701`, `UPDATE_접수추가검사` 는 `701 → 700` 이다. Master 손상 시 같은 원인에 다른 Code 가 나간다 | `05` §5 가 둘을 한 칸("Master·Aggregate 데이터 구성")에 묶어 두어 어느 쪽도 위반이 아니다. 각 SP 는 자기 계약(§11.2 vs §12.2)의 문장 순서를 정확히 따랐다 — **계약 문서가 갈라져 있다** |
| 23 | **"무변경 성공" 의 업무시간 취급이 SP 마다 다르다** | 창 밖에 무변경 저장을 누르면 수검자수정·예약변경·추가검사는 `Code=1` 로 성공하고, 동일 주민번호 재등록은 `308`/`309` 로 실패한다 | 셋은 `05` §10.2/§11.2/§12.2 를, 하나는 §10.1 을 각각 정확히 따른 결과다. 구현 버그가 아니라 계약의 비대칭이다 |
...
| 9 | **없는 구멍을 한계로 등재했다** | `§43-15`(접수취소 후 당일 재접수 불가)를 "복구 경로가 없다" 로 적었는데, 16:00 이후엔 접수완료도 당일예약도 `304` 라 **그 시간에 오접수가 생길 수 없고**, 다음 날 이후 예약은 `CutoffTime NULL` 로 열려 있다(실측). 각 규칙은 문서화돼 있었고 **그 조합을 잘못 읽었다** — 시나리오를 끝까지 실행해 보지 않고 규칙 두 개를 머리로 합친 결과다 | 항목을 취소선 처리하고 이 줄을 남긴다. 규칙을 조합해 결론을 내면 **그 조합을 실제로 돌려 본다** |
...
| `SEL` | `001`~`025` | 25 | `tests/04_Select_SP_Tests.sql` + `tests/contract/` | 8개 SELECT SP — DB 상태 단언은 `tests/04`, RS0 Code·RS 형상은 `contract/` (§33.1a) | G09 |
