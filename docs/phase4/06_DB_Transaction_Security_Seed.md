...
| 21 | **정원 20 이 조회 SP 와 저장 SP 에 따로 박혀 있다** | `04_Procedures_Select.sql` 5곳과 `06_Procedures_Reservation_Write.sql` 2곳에 리터럴로 있다. 한쪽만 고치면 화면은 "자리 있음" 을 보여주고 저장은 `305` 를 낸다 | `00` RP-03 이 정책 수치로 확정했고 관리자 CRUD 가 §8.2 범위 밖이라 설정 테이블은 오히려 기준선 위반이다. 다만 **어떤 시험도 두 값의 일치를 보지 않는다** |
