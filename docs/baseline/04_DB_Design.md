...
| 5 | `생년월일` | `VARCHAR(8)` | X | `주민번호`에서 DB 유도 (`PERSISTED` 계산열) | `yyyyMMdd` |
| 6 | `성별` | `CHAR(1)` | X | `주민번호`에서 DB 유도 (`PERSISTED` 계산열) | `M` / `F` |
...
| 13 | `B형간염제외여부` | `BIT` | X | `0` / 사용자 입력 | B형간염(`EX010`) 검사 제외 여부. `1`이 제외 |
| 14 | `비고` | `NVARCHAR(MAX)` | O | 사용자 입력 | 메모 |
| 15 | `생성일시` | `DATETIME` | X | `GETDATE()` | 생성시각 |
| 16 | `최종수정일시` | `DATETIME` | X | `GETDATE()` | 마지막 수정시각 및 수정 동시성 기준값 |

컬럼 순서는 **업무 데이터를 앞으로** 둔다. 검진 업무가 읽고 쓰는 값(`B형간염제외여부`까지)이 먼저 오고,
자유 서술인 `비고`와 DB가 채우는 `생성일시`·`최종수정일시`가 뒤에 모인다. §8.2.2도 같은 규칙을 따른다.
...
| 6 | `국가검사항목` | `NVARCHAR(100)` | X | 저장 SP 조립 | NEX 검사항목코드를 오름차순 쉼표 연결 |
| 7 | `추가검사항목` | `NVARCHAR(50)` | O | 저장 SP 조립 | AEX 검사항목코드를 오름차순 쉼표 연결 |
| 8 | `생성일시` | `DATETIME2(0)` | X | `SYSDATETIME()` | 업무행 생성시각 |
| 9 | `최종수정일시` | `DATETIME2(0)` | X | `SYSDATETIME()` | 최근 변경·접수·취소·AEX 변경시각 |
| 10 | `행버전` | `ROWVERSION` | X | SQL Server 자동생성 | 낙관적 동시성 토큰 |

검사구성은 이 행의 컬럼이다. 별도 Detail 테이블을 두지 않는다.

§8.1.2와 같은 규칙으로 **업무 데이터를 앞으로** 둔다. 업무를 식별하고 상태를 정하는 값에 검사구성 2개까지가
연속된 앞부분이고, 감사·동시성 컬럼(`생성일시`·`최종수정일시`·`행버전`)이 뒤에 모인다.
...
| 컬럼 추가·삭제·이름·타입·NULL 변경 필요 | **없음 — 48컬럼 확정** |
| PK/FK/UQ/CK/DF 변경 필요 | **없음 — PK 6 / FK 2 / UQ 2 / UX 1 / CK 24 / DF 8** |
