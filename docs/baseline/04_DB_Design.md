...
- **R16 개정 범위:** §11.3 의 검색연산 계약에서 **수검자 목록의 「최소 1개 조건」과 「조건 없는 전체조회 금지」를 걷는다**(`03` §5.3 재봉인에 딸린다). 이름 `%검색어%` 포함검색 금지는 그대로다 — 그 둘은 한 줄에 같이 적혀 있었지만 서로 다른 것을 막는다. 전체조회 금지가 실제로 막던 사고는 `%` 한 글자로 `103` 가드를 우회해 전건이 주민번호와 함께 나온 것이었고(`04_Procedures_Select.sql` 의 `[X]` 주석), 그 경로는 메타문자 이스케이프가 따로 막는다. **전체조회가 주민번호를 전건 내보내는 것 자체는 `06` §43 에 한계로 등재했다**(2026-09-10 사용자 결정). 테이블·컬럼·제약·Index·Sequence 는 한 줄도 바뀌지 않는다
...
- 수검자 Master의 컬럼 수는 **§8.1.2가 단일 출처**다. 여기에 값을 적지 않는다 — R3에서 16이 되었다고 적어 두었다가 R7에서 거짓이 되었다(ROOT `AGENTS.md` §6). 줄어든 경위는 R2의 29개에서 UI·계약 어디에서도 읽지 않는 13개를 삭제하고 `B형간염제외여부` 1개를 신설한 뒤 R3 재봉인에서 `CelNumberS` 1개를 마저 삭제한 것이고, R7에서 `행버전` 1개가 더해졌다.
...
| 7 | `운영기준` | 운영시간 Master | 운영시간과 마감시각 넷. 1행 고정이며 값의 소유자는 `00` 이다 |
...
`검사코드`는 검사구성 문자열이 코드로 참조하므로 FK 관계선이 없다. `휴무일`은 날짜로만 조회되는 독립 Master이고 `변경이력`도 Foreign Key를 갖지 않는다. `운영기준`도 그렇다 — 1행짜리 운영시간 Master이며 어느 업무 행과도 관계를 맺지 않는다.
...
| 7 | `운영기준` | `기준ID` | - | 운영시간·마감시각 1행 고정. 값의 소유자는 `00` 이다 |
...
|---|---|---|---|
| `UQ_수검자_CHART_NO` | `차트번호` | - | 차트번호 고유성. 조회는 포함검색이라 이 Index 를 타지 않는다 |
| `UQ_수검자_SOCIAL_NUMBER` | `주민번호` | - | 주민번호 테스트값 정확조회·고유성 |
| `IX_수검자_NAME_BIRTHDAY` | `성명, 생년월일` | `수검자ID, 차트번호, 성별, 휴대전화` | 이름+생년월일 중복후보. 이름 조회는 포함검색이라 이 Index 를 타지 않는다 |
...
`00` CP-04 의 **운영시간**과 `00` §3장의 **마감시각 넷**을 담는 단일행 기준정보다.
`UFN_HC_일정확인`(§Rule)과 `USP_HC_공통업무상태_조회`가 이 값을 읽는다.

`[!]` **정책을 설정으로 바꾸는 것이 아니다.** 수치의 소유자는 여전히 `00` 이고, 이 테이블은 그 값을
Rule 이 읽을 수 있는 자리에 **옮겨 놓은 것**이다. 셋이 그것을 강제한다.

```text
관리 CRUD SP 를 만들지 않는다        00 §8.2 밖을 그대로 유지한다 — 화면에서 바꿀 수 없다
게이트가 Seed = 00 을 판정한다        verify-operating-baseline.sh. 00 §7.4 의 180일을 HOL-G4 가
                                      지키는 것과 같은 방식이다
게이트가 회차 끝의 복원을 판정한다     시험이 창을 넓혔다 되돌리지 않으면 이후 회차가
                                      **다른 제품을 시험한 PASS** 가 된다 (06 §43-14)
```

`[!]` **왜 리터럴이면 안 되는가.** `05` §7.1 은 `운영시작시각`·`운영종료시각`을 `SP-COM-01` 의
Result Set 컬럼으로 이미 정해 두었다 — 계약이 처음부터 *"운영시각은 DB 가 쥐고 화면에 건네주는 값"*
으로 모델링했다는 뜻이다. 지금까지는 SP 가 그 값을 리터럴로 **지어내서** 돌려주고 있었다.
저장하는 편이 계약과 더 가깝다. 정원 20 은 그런 Result Set 컬럼이 아니라는 점에서 다르다(`06` §43-21).

### 8.7.2 컬럼 (7)

네 마감 컬럼의 값은 `00` §3장 표가 정한다 — 그 표의 「당일예약 마감」이 `일반예약*마감`,
「접수 마감」이 `접수*마감` 이다. 아래 표의 설명 칸에 절 번호를 적지 않는 것은 공개본이
그 참조를 따라갈 수 없기 때문이다(`verify_output.js`).

| No | 컬럼 | 타입 | NULL | Default / 생성 | 설명 |
|---:|---|---|:---:|---|---|
| 1 | `기준ID` | `TINYINT` | X | Seed 고정값 `1` | PK. 항상 1행이다 |
| 2 | `운영시작시각` | `TIME(0)` | X | Seed | 운영시간의 시작 (`00` CP-04) |
| 3 | `운영종료시각` | `TIME(0)` | X | Seed | 운영시간의 끝. 이 시각부터 업무 불가 (`00` CP-04) |
| 4 | `일반예약AM마감` | `TIME(0)` | X | Seed | 일반 당일예약의 오전 마감 |
| 5 | `일반예약PM마감` | `TIME(0)` | X | Seed | 일반 당일예약의 오후 마감 |
| 6 | `접수AM마감` | `TIME(0)` | X | Seed | 접수의 오전 마감. WalkIn 당일예약도 이 값을 쓴다 |
| 7 | `접수PM마감` | `TIME(0)` | X | Seed | 접수의 오후 마감. WalkIn 당일예약도 이 값을 쓴다 |

**감사 컬럼(`생성일시`·`최종수정일시`)과 `행버전`을 두지 않는다.** 셋 다 값을 바꾸는 주체가 있을 때
필요한 것인데 이 테이블에는 CRUD SP 가 없다. `변경이력.대상테이블`도 `수검자`·`예약접수` 둘뿐이라
(§8.6.3) 기록할 곳이 없다. `휴무일`이 R7 에 감사 컬럼을 얻은 것은 화면 CRUD 대상이 되었기 때문이며
(§8.4.2) 이 테이블은 그렇지 않다.

`WalkIn` 당일예약과 접수가 같은 두 값을 쓰는 것은 `00` §3장 표가 그렇게 정했기 때문이다 —
`UFN_HC_일정확인`의 `@마감구분`이 `NORMAL`·`RECEPTION` 둘뿐인 것과 같은 이유다.
토요일 오후는 마감이 "해당 없음"이라 컬럼을 두지 않는다. `시간대운영 = 0` 이 그 자리를 대신한다.

### 8.7.3 Key / Constraint / Index

| 구분 | 이름 | 정의 |
|---|---|---|
| PK | `PK_운영기준` | `기준ID` Clustered |
| CK | `CK_운영기준_SINGLETON` | `기준ID = 1` |
| CK | `CK_운영기준_HOURS` | `운영시작시각 < 운영종료시각` |

마감시각에는 CHECK 를 두지 않는다. **입력 주체가 없으므로 입력 검증이 지킬 것이 없고**, 시험이 창을
넓힐 때 네 값을 한 UPDATE 로 옮기지 못하면 걸리는 함정만 남는다. 마감이 운영시간 안에서만 적용된다는
`00` §3 의 규칙은 스키마가 아니라 `UFN_HC_일정확인`이 지킨다 — 운영시간 판정이 마감 판정보다 앞이다.

추가 Index 를 두지 않는다. 1행 테이블이고 접근은 언제나 `기준ID = 1` 정확조회다.

`[X]` **행이 없으면 fail-closed 여야 한다.** `UFN_HC_일정확인`은 이 값을 스칼라 하위쿼리로 읽으므로
0행이면 `NULL` 이 되고, `현재시각 >= NULL` 은 UNKNOWN 이라 `운영시간내여부 = 0` 으로 떨어진다.
**열리는 쪽이 아니라 닫히는 쪽으로 무너진다.** 그 반대였다면 Seed 누락이 조용히 모든 시각을 업무시간으로
만들었을 것이다.
...
- 빈 문자열은 `NULL`로 정규화한다. `[R16]` **수검자 목록은 조건이 하나도 없으면 전체를 조회한다**(`03` §5.3). 예약·접수 Workbench 는 그대로 최소 1개 조건을 요구한다.
...
| PK/FK/UQ/CK/DF 변경 필요 | **없음 — PK 7 / FK 2 / UQ 2 / UX 1 / CK 28 / DF 10** |
