...
- **R22 개정 범위:** 2026-09-14 사용자 지시. **`SP-HOL-02` `자체휴무일_등록` 과 `SP-HOL-03` `자체휴무일_수정` 을 `SP-HOL-02` `[dbo].[USP_HC_자체휴무일_저장]` 하나로 합친다.** 둘은 같은 한 행을 쓰고 잠금 자원(`HOL|휴무일자`)·Transaction 형태·Result Set 이 같았으며, 다른 것은 **그 행이 이미 있어야 하는가**뿐이었다. 그 갈래를 `@휴무동작코드`(`CREATE_HOLIDAY`/`UPDATE_HOLIDAY`)가 고른다. `[!]` **동작을 현재 상태에서 유도하지 않는다** — R20 이 `@업무동작코드` 를 둔 이유 그대로이고, 유도하면 `[추가]` 가 남의 행을 덮어써 `801` 이 뜻을 잃는다. 고치는 곳은 §1.3 표·§12.6·§12.7·§13 이고 **절 번호를 밀지 않는다**. 허용 결과코드에 `101` 이 하나 생긴다 — `@휴무동작코드` 가 만든 새 경로이며 계약시험 `HOL-008` 이 잰다. 다른 SP 는 한 줄도 바뀌지 않는다
- **R21 개정 범위:** 2026-09-14 사용자 지시. **`SP-PAT-02` `SELECT_수검자상세` 와 `SP-PAT-05` `SELECT_수검자유효업무` 를 `SP-PAT-01` `SELECT_수검자목록` RS1 안으로 들인다.** 셋 다 같은 수검자 한 행을 읽던 SELECT 이고, 화면은 목록을 받은 뒤 **행을 고를 때 한 번 · 모달을 열 때 또 한 번** 같은 사람을 다시 묻고 있었다. RS1 이 11칸에서 19칸이 되며 그중 여덟이 사라진 두 SP 가 주던 것이다. `[!]` **판정이 화면으로 오지 않는다** — `유효*` 네 칸을 채우는 기준(`예약일 >= DB 현재일 AND 상태 IN (RSV, RCP)`)은 RP-06 그대로이고 SP 안에 있다. 고치는 곳은 §1.3 표·§7.2·§7.3·§7.4·§13·부록 기능표이며 **§7.3·§7.4 를 지우고 절 번호를 밀지 않는다**(§8 이하가 그대로다). 다른 SP 의 Parameter·Result Set·ResultCode·검증순서는 한 줄도 바뀌지 않는다
...
| SP-PAT-01 | `[dbo].[USP_HC_수검자목록_조회]` | SELECT | 수검자 검색·상세·유효업무 |
...
| SP-HOL-02 | `[dbo].[USP_HC_자체휴무일_저장]` | SAVE | `@휴무동작코드` 로 자체휴무일 1행 생성·수정 |
...
| `상세주소` | `NVARCHAR(200)` | O | `상세주소` |
| `비고` | `NVARCHAR(MAX)` | O | `비고` |
| `B형간염제외여부` | `BIT` | X | `B형간염제외여부` |
| `행버전` | `BINARY(8)` | X | `행버전` |
| `유효업무ID` | `BIGINT` | O | `예약접수.업무ID` |
| `유효예약일` | `DATE` | O | `예약접수.예약일` |
| `유효시간대코드` | `CHAR(2)` | O | `예약접수.시간대코드` |
| `유효상태코드` | `CHAR(3)` | O | `예약접수.상태코드` |

`[R21]` **뒤 여덟 칸은 SP 두 개가 하던 일이다** (2026-09-14 사용자 지시). `상세주소`부터 `행버전`
까지 넷은 `SELECT_수검자상세`(옛 SP-PAT-02)가, `유효*` 넷은 `SELECT_수검자유효업무`(옛 SP-PAT-05)가
주던 것이고, 두 SP 는 이 회차에 없어졌다. 화면이 목록을 한 번 받으면 **행을 고를 때도 모달을 열
때도 다시 묻지 않는다** — `03` §5.5 의 우측 상세와 `DLG-PAT-01`·`WF-RSV-01` 의 진입값이 전부 이
한 행이다.

`유효*` 네 칸은 `00` RP-06 의 유효업무다:
...
없으면 네 칸이 전부 `NULL` 이다. **판정을 SP 가 낸다는 뜻이지 「예약 가능」을 낸다는 뜻이 아니다** —
가능/불가 문구는 화면이 만든다(`03` §5.5). RP-06 이 유효업무를 최대 1건으로 두므로 이 조인은
수검자 1행을 늘리지 않는다. 2건 이상은 불변조건 위반이며 `SP-RSV-02` 가 저장 시점에 `701` 로 막는다.
...
정확히 7행을 반환한다. `RS3`과 달리 **저장 여부와 무관하게 AEX 7종 전부**를 낸다.
...
```text
UFN_HC_추가검사확인(수검자ID, 예약일, 업무ID, 저장검사사용여부=1, 요청선택여부 7개=0)
선택여부 = 예약접수.추가검사항목에 그 검사항목코드가 있는가
```

- `저장검사사용여부=1`이므로 성별·중복 판정의 근거는 **그 Work에 저장된 NEX**다(§6.4.3). 예약일 기준 TGT 재판정(`400`/`401`)은 하지 않는다 — 이미 성립한 Work다.
- `요청선택여부`를 0으로 넘긴다. `사유코드`는 `요청선택여부`에 의존하지 않으므로(§6.4.3) 판정 결과가 같고, 이 Result Set은 **고르기 전 상태**를 낸다.
- `RS3`과 겹치는 것은 의도한 것이다. `RS3`은 저장된 구성을 **읽는** 화면(`WF-WRK-01`·`DLG-RCP-01`)이, `RS5`는 **고치는** 화면(`DLG-RCP-02`)이 쓴다. §9.9·§9.10이 `SELECT_예약가능정보`에서 같은 방식으로 갈라져 있다.
- 이 Result Set은 상태코드를 가리지 않는다. `RSV` Work에서도 7행이 오며, `DLG-RCP-02` 진입 가능 여부는 `RS4`의 `EDIT_EXTRA`가 판정한다.
...
- **공통 업무 가능조건(`308`·`309`)을 적용하지 않는다.** 수검자 Master는 검진 업무가 아니라 기준정보이며(`00` §1.1 · EP-01), 운영시간 안으로 묶으면 정비가 필요한 바로 그 시각에 막힌다 — 오늘 접수한 수검자의 오타를 18:00 이후에 고칠 수 없게 된다. `SP-HOL-02`·`SP-HOL-04`가 §12에서 같은 이유로 이미 그렇게 되어 있다. 예약·접수 Write는 이 예외의 대상이 아니다.
...
## 12.6 `[dbo].[USP_HC_자체휴무일_저장]`

`[R22]` **등록과 수정이 하나다** (2026-09-14 사용자 지시). 둘은 같은 한 행을 쓰고 잠금 자원·
Transaction 형태·Result Set 이 같았으며, 다른 것은 **그 행이 이미 있어야 하는가**뿐이었다.
그 갈래를 `@휴무동작코드` 가 고른다. 절 번호는 밀지 않는다 — 옛 §12.7 자리는 비워 둔다.

### 입력

| Parameter | 타입 | NULL | 의미 |
|---|---|:---:|---|
| `@휴무동작코드` | `VARCHAR(30)` | X | `CREATE_HOLIDAY` / `UPDATE_HOLIDAY` |
| `@휴무일자` | `DATE` | X | PK. 바꾸지 않는다 |
| `@휴무일명` | `NVARCHAR(100)` | X | |
| `@사용여부` | `BIT` | X | |
| `@비고` | `NVARCHAR(500)` | O | |
| `@행버전` | `BINARY(8)` | △ | `UPDATE_HOLIDAY` 에만 필수다. `CREATE_HOLIDAY` 에는 아직 행이 없다 |

`휴무구분`은 Parameter가 아니다. 이 SP는 `자체휴무일`만 만든다(`00` HOL-05).

`휴무일자`는 PK이며 바꾸지 않는다. 날짜를 옮기려면 삭제 후 등록이다(`03` §24.5).

`[!]` **동작을 현재 상태에서 유도하지 않는다.** 「행이 있으면 수정, 없으면 등록」으로 하면
`[추가]` 를 누른 사용자가 남의 행을 조용히 덮어쓴다 — `801` 이 막던 것이 그것이다. `SP-WRK-03`
(`@업무동작코드`)이 R20 에서 같은 판단을 했고, 이 SP 도 같은 방식이다.

### 동작코드별 갈래

| `@휴무동작코드` | 기존 행 | 결과 |
|---|---|---|
| `CREATE_HOLIDAY` | 없다 | `INSERT` · `결과코드=0` |
| `CREATE_HOLIDAY` | 있다 | `결과코드=801` · 아무것도 바꾸지 않는다 |
| `UPDATE_HOLIDAY` | 없다 | `결과코드=800` |
| `UPDATE_HOLIDAY` | 자체휴무일 아님 | `결과코드=802` |
| `UPDATE_HOLIDAY` | 행버전 불일치 | `결과코드=601` |
| `UPDATE_HOLIDAY` | 값이 같다 | `결과코드=1` · 행을 갱신하지 않고 기존 `행버전` 반환 |
| `UPDATE_HOLIDAY` | 값이 다르다 | `UPDATE` 문 실행 · `결과코드=0` |

허용 밖 코드는 `결과코드=101` 이고 `오류항목` 은 `휴무동작코드`다.

### Result Set

```text
RS0 처리결과
RS1 휴무일결과      결과코드 0 또는 1 일 때만
```
...
```text
필수값 (휴무동작코드 → 휴무일자 → 휴무일명 → 사용여부 → UPDATE_HOLIDAY 면 행버전)
→ 휴무동작코드 허용값
→ 잠금(HOL|휴무일자) 후 기존 행 1회 조회
→ CREATE_HOLIDAY : 기존 행 없음
→ UPDATE_HOLIDAY : 휴무일 존재 → 휴무구분 = 자체휴무일 → 행버전 → 실제 변경 여부
...
```text
0, 1, 100, 101, 601, 800, 801, 802
```

`[R22]` **`101` 이 생겼다.** 합치기 전 두 SP 에는 `BadValue` 가 나올 경로가 없어 선언하지
않았는데(`05` §18 *"사용되지 않는 ResultCode 0개"*), `@휴무동작코드` 가 생기며 그 경로가
생겼다. 계약시험 `HOL-008` 이 그 갈래를 재고 있다.

---

## 12.7 (비어 있음 — `[R22]` §12.6 으로 합쳤다)

자체휴무일 **수정** 계약이 여기 있었다. R22 에서 등록과 하나가 되어 §12.6 이 둘 다 갖는다.

`[!]` **절 번호를 비워 두고 밀지 않는다.** 밀면 이 번호를 가리키는 바깥 참조가 조용히
어긋난다 — `03` §18 · `06` · `winforms` 주석이 `05` §12.7 을 이름으로 부르고 있다.
§12.3(접수취소)이 R20 에서 같은 모양으로 남았다.
...
| SAVE_자체휴무일 | 0, 1, 100, 101, 601, 800, 801, 802 |
...
|---|---|---|---|
| F-PAT-001 | P01-01 | WF-PAT-01, DLG-PAT-02 | SELECT_수검자목록 |
| F-PAT-002 | P01-02~03 | DLG-PAT-01, DLG-PAT-03 | INSERT_수검자 |
| F-PAT-003 | P01-04~05 | DLG-PAT-01 | UPDATE_수검자정보 |
| F-RSV-001 | P02-01~04 | WF-RSV-01 | SELECT_수검자목록, SELECT_예약가능정보, INSERT_예약 |
...
| F-RCP-001 | P03-01~03 | DLG-RCP-01 | SELECT_예약접수상세, UPDATE_접수완료 |
...
| F-COM-009 | 기준정보 | DLG-HOL-01 | SELECT_휴무일목록, SAVE_자체휴무일, DELETE_자체휴무일 |
