...
| SELECT_휴무일목록 | 0, 101, 104 |
| INSERT_자체휴무일 | 0, 100~101, 801 |
| UPDATE_자체휴무일 | 0, 1, 100~101, 601, 800, 802 |
...
`00`·`01`·`03`은 화면·C# 계층의 영문 이름으로 호출계약을 적고, `04`·`05`는 DB 계약의 한글 이름을 쓴다. §16.2가 정한 대로 **C# 식별자는 영문 PascalCase를 유지**하므로 두 이름은 계속 공존한다. 아래 26종이 그 전건이며, 이 표 밖에서 두 이름이 만나는 곳은 없다.
