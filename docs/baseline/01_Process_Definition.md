...
- **문서 버전:** v2.1  (v2.0 → R3. v2.1 → R11 수검자 동시성 기준)
- **기준일:** 2026-09-04
- **기준선 ID:** `HC-RSV-RCP-20260909-R11`  (직전 `HC-RSV-RCP-20260904-R3`)
- **R11 개정 범위:** `P01-04` 의 동시성 기준을 `LastEditDate` 에서 `RowVersion` 으로 고친다 — **한 줄뿐이다.** `05` R7 이 수검자 동시성 토큰을 `행버전` 으로 통일하고 `600` 을 `601` 에 흡수했을 때 이 문서가 따라오지 않았고, 번호가 작은 쪽이 이기는 규칙(ROOT `AGENTS.md` §1) 때문에 가장 높은 자리에 옛 문장이 남아 있었다. 프로세스 단계·분기·상태전이는 바뀌지 않는다
...
> **최종 판정: GO — 본 문서는 `HC-RSV-RCP-20260909-R11` 기준선의 최종 프로세스 정의서이며 이후 READ-ONLY로 사용한다.**
