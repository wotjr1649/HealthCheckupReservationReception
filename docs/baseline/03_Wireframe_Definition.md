...
- **문서 버전:** v1.10  (v1.5 → R8 리뷰 정정. v1.6 → R10 §1.4 기동 크기. v1.7 → R11 수검자 동시성값. v1.8 → R12 공통 업무불가가 Action을 닫지 않는다. v1.9 → R16 수검자 조회에 최소 조건이 없다. v1.10 → R17 차트번호·이름 포함검색)
- **기준일:** 2026-09-09
...
- **R16 개정 범위:** §5.3 의 「최소 1개 조건이 있어야 조회한다」를 걷는다. 조건이 하나도 없으면 **전체 목록**을 조회하고, 화면을 열 때 한 번 그렇게 채운다. 2026-09-10 사용자 결정이며, 빈 Grid 가 *"무엇을 검색해야 하는지 모르겠다"* 로 읽힌 것이 이유다. §7.2 는 *"조회계약은 5.3과 동일하다"* 라 적으므로 `DLG-PAT-02` 도 함께 따라온다. `04` §11.3 · `05` §7.2 가 같은 회차에서 열린다. **예약·접수 Workbench(§9.3)는 그대로다** — 그쪽은 날짜로 열려 있어 전건이 업무량과 함께 자란다. 화면 ID·Navigation·Action·입력/ReadOnly 범위·나머지 Validation 은 바뀌지 않는다
- **R12 개정 범위:** 공통 업무불가가 **Ribbon Action을 닫지 않도록** 고친다 — §1.3 한 줄과 §5.2·§9.6·§9.7 세 표의 `공통 업무불가` 행이다. `00` §1.1 이 그 조건의 적용 대상을 예약·접수로 한정했고, 9장 9항이 최종 권한을 Stored Procedure에 두었으므로 **화면이 미리 닫으면 그 판정을 받아볼 수 없다.** 화면 ID·Navigation·Action 목록·입력/ReadOnly 범위·Validation 의미는 바뀌지 않는다 — 닫히던 것이 열릴 뿐이고 예약·접수 Write는 저장 시점에 DB가 `308`/`309`로 막는다
- **R11 개정 범위:** 수검자 Edit 의 숨은 동시성값을 `LastEditDate` 에서 `RowVersion` 으로 고친다 — §6.4 와 §16 두 줄뿐이다. 같은 회차에서 `01` §P01-04 의 같은 문장도 함께 고쳤다. `05` R7 이 수검자 동시성 토큰을 `행버전` 으로 통일했을 때 §5.6 만 따라오고 이 둘이 남았고, 그래서 **이 문서가 자기 자신과 어긋나 있었다**(§5.6 No 3 은 `LastEditDate` 를 *"동시성 기준이 아니다"* 라 적는다). 설계 선택이 아니라 갱신 누락의 정정이다. 화면 ID·Navigation·Action·입력/ReadOnly 범위·Validation 의미는 바뀌지 않는다
...
- 공통 업무불가는 **상태로 표시하되 Ribbon Action을 막지 않는다**(`00` §1.1). 화면이 미리 닫으면 저장 시점의 DB 판정을 사용자가 받아볼 수 없고, 수검자·휴무일 기준정보는 애초에 `308`/`309`의 대상이 아니다.
...
**공통 업무불가는 이 표에 행을 갖지 않는다** — Action을 닫지 않기 때문이다. 상태는 §1.3의 상태영역에만 나타나고 판정은 저장 시점에 DB가 한다(`00` §1.1 · 9장 9항). 수검자 등록·수정은 기준정보 정비라 애초에 `308`/`309`의 대상이 아니며(`05` §10.1 · §10.2), `[변경이력]`은 조회 Action이라 데이터를 바꾸지 않는다(§23.4).
...
- **조건이 하나도 없으면 전체 목록을 조회한다.** `[R16]` 화면을 열면 조건 없이 한 번 조회해 목록을 채운다.
- 입력된 조건은 AND로 결합한다.
...
- 기대 RowVersion 불일치 시 저장하지 않고 최신값을 다시 조회
...
**공통 업무불가는 이 표에 행을 갖지 않는다**(§1.3). 다만 예약 Write는 `308`/`309`의 대상이므로 저장 시점에 DB가 막고 화면은 그 결과를 Blocking으로 보인다(§15.1).
...
`[변경이력]`은 §9.6과 같다 — 상태와 무관하게 행이 선택되면 열린다(§23.4). **공통 업무불가도 §9.6과 같다** — 이 표에 행을 갖지 않고, 접수 Write는 저장 시점에 DB가 막는다.
...
- 수검자 Edit Modal은 원본 `RowVersion`을 숨은 값으로 유지한다.
...
`[!]` **§1.3의 공통 업무조건이 이 화면에는 적용되지 않는다.** 공통 업무 가능조건은 검진 업무(예약·접수)를 대상으로 하며 기준정보 정비에는 걸지 않는다(`00` §1.1). 정비를 운영시간 안으로 묶으면 *내일이 휴무가 되었다*를 오늘 18:00 이후에 넣을 수 없다 — 정비가 필요한 바로 그 시각에 막히는 셈이다. `SP-HOL-02`~`04`도 `308`·`309`를 반환하지 않으며, R12 부터 같은 기준정보인 수검자 Write 둘도 그렇다(`05` §10.1 · §10.2 · §12.6~§12.8). `[R12]` **공통 업무불가로는** 어느 화면도 Action을 닫지 않는다 — §1.3. 행 선택·Work 상태로 닫는 것은 그대로다(§5.2 · §9.6 · §9.7).
...
> **최종 판정: GO — 본 문서는 최종 UI 구현 기준이며 이후 READ-ONLY로 사용한다.** 기준선 회차는 여기 적지 않는다 — `06` §4.1 표가 단일 출처다(ROOT `AGENTS.md` §6).
