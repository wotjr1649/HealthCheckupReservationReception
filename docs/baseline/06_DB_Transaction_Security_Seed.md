...
- **문서 버전:** v1.15  (`[R12]` 회차별 버전을 여기 적지 않는다 — §4.1 표가 단일 출처이고 무엇을 고쳤는지는 아래 `RN 개정 범위` 가 적는다. 걷어낸 사본은 R6 부터 한 칸씩 밀려 있었고 아무 게이트도 그것을 보지 않았다. ROOT `AGENTS.md` §6)
- **기준일:** 2026-09-08  ·  **실행검증일:** 2026-09-07(R3 회귀) · 2026-09-08(R4 회귀 · R5 회귀 회차 `C`) · 2026-09-09(R12 회귀 회차 `R12` 창 밖) · 2026-09-10(R13 회귀 회차 `R13` 창 밖)
- **기준선 ID:** `HC-RSV-RCP-20260911-R18`  (직전 `HC-RSV-RCP-20260910-R17`)
- **R18 개정 범위:** `05` 재봉인에 딸린다. `05` §8.2 `SELECT_예약접수상세` 가 `RS5 추가검사구성` 을 얻는다 — `03` §12 `DLG-RCP-02` 가 AEX 를 고치라고 요구하는데 그 일곱을 읽는 길이 계약에 없었다. 이 문서에서 바뀌는 것은 **§4.1 회차표뿐**이다: Transaction 경계·잠금 순서·검증순서·허용 ResultCode·Seed·시험 아키텍처는 한 줄도 바뀌지 않는다. `[!]` **기존 Result Set 을 고치지 않고 뒤에 더했으므로** 이미 `RS0`~`RS4` 를 읽는 쪽(C# Repository · `tools/expected-contracts.json`)은 깨지지 않는다 — 계약시험의 기대값에는 새 절을 한 벌 더한다. `[!]` **전체 회귀는 돌리지 않았다(`NOT RUN`)** — `database/scripts/test.sh` 가 DB 를 DROP 해 개발 중인 데이터와 넓혀 둔 운영시간 창(§4.8 인계문서)을 지우기 때문이다. 대신 배포 후 `SELECT_예약접수상세` 를 직접 불러 RS0~RS4 가 회차 전과 한 바이트도 다르지 않고 RS5 가 7행인 것을 확인했다
...
| `06_DB_Transaction_Security_Seed.md` | v1.15 | FINAL / GO / READ-ONLY | `HC-RSV-RCP-20260911-R18` |
