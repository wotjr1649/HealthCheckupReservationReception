...
- **기준일:** 2026-09-08  ·  **실행검증일:** 2026-09-07(R3 회귀) · 2026-09-08(R4 회귀 · R5 회귀 회차 `C`) · 2026-09-09(R12 회귀 회차 `R12` 창 밖) · 2026-09-10(R13 회귀 회차 `R13` 창 밖) · 2026-09-11(R19 회귀 회차 `R19` 창 안) · 2026-09-14(R20 회귀 회차 `R20` 창 안 · R21 · R22)
- **기준선 ID:** `HC-RSV-RCP-20260914-R22`  (직전 `HC-RSV-RCP-20260914-R21`)
- **R22 개정 범위:** `05` 재봉인과 한 쌍이다. 2026-09-14 사용자 지시로 **자체휴무일 등록·수정을 `USP_HC_자체휴무일_저장` 하나로 합쳤다** — 둘은 같은 한 행을 쓰고 잠금 자원·Transaction·Result Set 이 같았다. 여기서 고치는 것은 §1 문서정보 · §4.1 회차표 · §18 SP Matrix 두 행 → 한 행과 개수뿐이다. Transaction 형태(`sp_getapplock HOL|휴무일자` · `XACT_ABORT ON`)와 감사 규칙은 한 줄도 바뀌지 않는다 — 휴무일 SP 는 원래 `변경이력` 을 남기지 않는다(§18 각주)
...
- **R16 개정 범위:** `03`·`04`·`05` 재봉인에 딸린다. **수검자 목록의 「최소 1개 조건」을 걷었다** — 조건이 하나도 없으면 전체를 조회하고, 화면은 열릴 때 그렇게 한 번 채운다 (2026-09-10 사용자 결정; 빈 Grid 가 *"무엇을 검색해야 하는지 모르겠다"* 로 읽혔다). 이 문서에서 바뀌는 것은 §4.1 회차표와 §43 의 새 한계 하나뿐이다 — Transaction·잠금·Seed·시험 아키텍처는 한 줄도 바뀌지 않는다. `[!]` **동결(`winforms/scripts/verify-db-frozen.sh`)을 연 첫 회차다.** 게이트가 요구한 대로 회차 이름을 붙이고 매니페스트를 같은 커밋에 넣었다 — 동결은 *고치지 마라* 가 아니라 *고쳤으면 회차를 붙여라* 였고, 실제로 그렇게 작동했다
- **R15 개정 범위:** `06` 만 연다. **남은 위험을 닫는 회차다** — 계약·SQL·Seed·시험은 한 줄도 바꾸지 않는다. ① §42 의 「창 안 회차 미실행」을 논거로 닫는다(그 회차가 더할 정보가 `RUL-T02`·`T03` 와 `OFF-309-02` 가 이미 재는 한 가지뿐이다). ② §43-22 의 `700`/`701` 순서 비대칭을 Phase 5 범위에서 닫는다(`07` §6.1 이 둘을 한 칸으로 표현하므로 화면이 가르지 않는다 — `05` 를 열어 SP 를 고치면 DB 변경이 되고 그것은 이 시점의 방침과 정면으로 부딪힌다). ③ 새 게이트 둘을 §42 에 등재한다 — `G17`(라이브 DB ↔ `deploy/`)과 `V26`(정책 시각값 ↔ `00`). 회차 `R13` 의 증거가 그대로 유효하므로 §42 에 회차를 더하지 않는다
- **R14 개정 범위:** `04`·`05` 재봉인에 딸린다. 본문 두 곳의 옛 테이블 수를 걷는다 — §13 의 `변경이력` 예외 문장과, `04` §3.9 를 그대로 인용한 §16 의 한 줄이다. 인용은 원문을 따라가야 하므로 함께 열었다. §4.1 회차표도 갱신한다. **SQL·Seed·시험·계약을 한 줄도 바꾸지 않았으므로 회차 `R13` 의 증거가 그대로 유효하고, §42 에 회차를 더하지 않는다** — 판정한 것은 문서 게이트 셋이다(`verify-baseline.sh` · `verify-docs.js` · `verify_output.js`). R10·R11 이 같은 성격이었다
- **R13 개정 범위:** `04` 재봉인에 딸린다. 운영시간과 마감시각 넷이 `UFN_HC_일정확인`·`USP_HC_공통업무상태_조회` 안에 리터럴로 박혀 있어 **시험이 그 창을 만들 수 없었다**. `04` §8.7 이 그 여섯 값을 테이블 `운영기준` 으로 옮기면서 이 문서에서 바뀌는 것은 **시험 아키텍처**다 — §14a(새 Seed 한 행), §33.2a(업무시간 분기 → 업무일 분기), §42(회차 `R13`), §43 의 1·12·14·21 이다. Transaction 경계·잠금 순서·검증순서·허용 ResultCode·`검사코드`/`휴무일` Seed 는 한 줄도 바뀌지 않았다. `[!]` **정책 수치는 여전히 `00` 이 소유한다** — 이 문서도 `deploy/02_Seed.sql` 도 그 값을 두 번째로 적지 않으며, 둘이 같은지는 `verify-operating-baseline.sh` 가 매 회차 판정한다 (ROOT `AGENTS.md` §6)
- **R12 개정 범위:** `00`·`03`·`05` 재봉인에 딸린다. §4.1 회차표에 더해 **이 문서가 두 번째로 갖고 있던 수검자 Write 계약**을 `05` 와 맞춘다 — §21.2 Transaction Matrix 의 `INSERT_수검자`·`UPDATE_수검자정보` 두 행(검증순서·허용 ResultCode), §15.2 fixture 표의 이유 칸, §33.2a 의 업무시간 분기, §43 의 12·23 이다. `[X]` **`05` 만 고치고 이 문서를 안 열었다면 계약이 두 곳에서 서로 다른 말을 하게 된다** — ROOT `AGENTS.md` §6 이 경고한 그 함정이고 실제로 R12 작업 중에 한 번 밟았다가 되잡았다. Transaction 경계·잠금 순서·Seed·보안 계약은 한 줄도 바뀌지 않았다
- **R11 개정 범위:** `01`·`03`·`05` 재봉인에 딸린다. §4.1 의 `01`·`03`·`05`·`06` 네 행만 바꾼다 — 회차의 단일 출처가 여기이므로 그 둘이 R11 이 되면 이 표가 거짓이 된다. **SQL·Seed·계약·시험을 한 줄도 바꾸지 않았으므로 회차 `R9` 의 증거가 그대로 유효하고, DB 를 재구축하는 전체 회귀는 돌리지 않았다.** 판정한 것은 문서 게이트 셋이다 — `verify-baseline.sh` · `verify-docs.js` · `verify_output.js`
...
|---|---|
| 문서명 | `06_DB_Transaction_Security_Seed.md` |
| 버전·상태·기준선 ID | **§4.1 표가 단일 출처다** — 여기에 두 번 적지 않는다 (ROOT `AGENTS.md` §6). 예전에 적어 두었다가 `v1.1` · `HC-RSV-RCP-20260908-R4` 에 굳은 채 R18 까지 왔다 |
| 기준일·실행검증일 | **머리말이 단일 출처다** — 같은 이유로 여기 적지 않는다 |
...
| 파일 | 버전 | 상태 | 기준선 ID |
|---|---|---|---|
| `00_Project_Policy.md` | v2.2 | FINAL / GO / READ-ONLY | `HC-RSV-RCP-20260909-R12` |
| `01_Process_Definition.md` | v2.1 | FINAL / GO / READ-ONLY | `HC-RSV-RCP-20260909-R11` |
...
R16 은 `03`·`04`·`05`·`06` 넷을 열었다. 수검자 목록의 「최소 1개 조건」을 걷는 회차이고, **동결을 연 첫 사례**다 — `01`·`02` 는 그대로다.
R15 는 `06` 만 열었다. 남은 위험을 닫는 회차이고 계약·SQL·Seed·시험을 한 줄도 바꾸지 않는다 — `04`·`05` 는 R14 표기를 유지한다.
R14 는 `04`·`05`·`06` 셋을 열었다. R13 이 테이블을 하나 더했는데 세 문서 본문의 옛 테이블 수 열여섯 곳이 그대로였고, `05` 는 머리말이 말하는 외부 호출 SP 수가 R7 이후 §1.3 과 어긋난 채였다. **수를 세는 게이트가 없던 자리다** — `V25` 를 그 자리에 놓았다.
R13 은 `04`·`05`·`06` 셋을 열었다. 운영시간·마감시각 여섯 값이 SP 안 리터럴이라 시험이 창을 만들 수 없었고, 그래서 예약·접수 성공 경로가 **하루 아홉 시간 안에서만** 판정됐다. `04` §8.7 이 값을 테이블로 옮기고, `05` §2.4·§6.1.4·§7.1 이 *SP 가 그 값을 어디서 읽는지* 를 적고, 이 문서가 시험 아키텍처를 따라간다.
`01` 은 R11, `00`·`03` 은 R12, `02` 는 R7 표기를 유지한다 — 열지 않은 문서는 회차가 올라가지 않는다 (ROOT `AGENTS.md` §2.2).
...
- `[X]` **`변경이력`은 예외다.** 이 문서 초안이 clean-create 의 근거로 든 *"보존할 운영 데이터가 없다"* 가 그 테이블에는 성립하지 않는다. 감사 기록은 배포로 지워지지 않아야 하므로 `DROP` 대상에서 빼고 `IF OBJECT_ID(...) IS NULL` 가드로 만든다(`04` §8.6.3). 물리 테이블 중 유일한 예외이고, 나머지에는 `IF NOT EXISTS` 가드를 두지 않는다.
...
- `04` §3.9: *"실제 DB Script 작성 전에 서버 버전과 Database Compatibility Level을 **확인하되**, 이 확인은 물리 테이블 논리모델을 변경하는 사유가 아니다"* — 확인을 요구했을 뿐 110 강제를 요구하지 않는다.
...
`00` HOL-06 이 정한 등재 범위 **2026-01-01 ~ 2027-12-31** 을 편다. 법정공휴일·대체공휴일 40행과
자체휴무일 1행, 합계 **41행**이다.
...
- **임시공휴일만** 넣지 않는다. 지정 시점이 유동적이라 다음 Seed 갱신에서 반영한다(`03` §24.7).
- `[X]` **선거일은 넣는다.** 처음에 "임시공휴일·선거일" 을 함께 뺐는데 그 근거가 선거일에는 성립하지 않는다 —
  임기만료에 의한 선거일은 「공직선거법」이 날짜를 정하고 규정 제2조가 공휴일로 삼는다. 유동적인 것은 임시공휴일뿐이다.
  그 결과 `2026-06-03`(수) 제9회 전국동시지방선거일이 빠져 **전국이 쉬는 평일을 업무 가능일로 판정**하고 있었다.
  2027 에는 해당 선거가 없다 — 대통령선거가 궐위로 2025-06-03 에 앞당겨졌고 국회의원선거는 2028 이다.
...
| `2026-06-03` | 수 | 제9회 전국동시지방선거일 | 법정공휴일 |
...
`00` §7.4 가 등재 범위와 경고 임계(잔여 180일)를 정한다. 이 Seed 의 공휴일 최종일은 `2027-12-27` 이다. 게이트는 **선언이 아니라 Seed 의 그 값**으로 잔여를 센다 —
`00` 한 줄만 늘려 게이트를 침묵시킬 수 없게 하기 위해서다.
...
# 14a. `운영기준` Seed 1행 `[R13]`

`04` §8.7 의 테이블이다. `기준ID = 1` 한 행뿐이고 `CK_운영기준_SINGLETON` 이 그것을 강제한다.

`[!]` **여섯 값을 여기 적지 않는다.** `00` CP-04 가 운영시간을, `00` §3장 표가 마감 넷을 소유한다.
이 문서에 베끼면 `00` 이 바뀌는 날 조용히 거짓이 되고, 그 실패 방식은 이 저장소에서 이미 세 번
나왔다 (ROOT `AGENTS.md` §6 · 이 문서 §13 의 `휴무일 2건` · §42 의 `6/6`).

판정하는 것은 `scripts/verify-operating-baseline.sh` 다.

```text
OPR-G1  00 CP-04 에서 운영시작·종료를 읽는다        못 읽으면 FAIL 이다 (통과가 아니다)
OPR-G2  00 §3 마감표에서 넷을 읽는다                 못 읽으면 FAIL 이다
OPR-G3  deploy/02_Seed.sql 의 값이 00 과 같다        → "설정" 이 아니라 정책이다
OPR-G4  배포된 [운영기준] 1행이 00 과 같다           → 회차가 넓힌 창이 되돌아왔다
```

`[X]` **G4 는 복원 판정을 겸하며, 그것이 이 회차가 새로 만든 실패 방식이다.** R13 부터 시험이
창을 옮겼다 되돌린다(§33.2a). 되돌리지 못한 채 끝난 회차는 **다음 회차를 다른 제품의 시험으로
만든다** — §43-14 가 배포 원본 수정을 금지한 바로 그 이유다. `test.sh` 가 회차 안에서 두 번 묻는다:
쓰기 시험·계약 게이트 뒤 한 번, 동시성 뒤 한 번. 뒤의 `verify-red`·`clean-rebuild` 가 DB 를 다시
만들기 전이어야 잔여가 지워지지 않는다.

`[I]` **관리 CRUD SP 를 만들지 않는다.** 이 값을 바꾸는 SP 가 생기면 `05` §1.3 의 외부 호출 SP
목록에 나타나야 하고 `UIDB-002` 가 그것을 본다. 만들지 않았으므로 `00` §8.2 밖에 그대로 있다
(§43-21 이 정원 20 에 대해 세운 기준을 이 값이 어떻게 지나는지는 그 항목에 적었다).

`[I]` **행이 없으면 열리는 쪽이 아니라 닫히는 쪽으로 무너진다.** `UFN_HC_일정확인` 이 스칼라
서브쿼리로 읽으므로 값이 `NULL` 이면 비교가 `UNKNOWN` 이고 `운영시간내여부 = 0`, 즉 `309` 다.
Seed 누락이 업무를 열어 주는 일은 구조적으로 없다.
...
| 대상 | 방식 | 이유 |
|---|---|---|
| 수검자 · 완료이력 · **기존 예약 19건** | **dbo 직접 `INSERT`** | 예약 Write SP는 `308/309`(업무일·운영시간)를 검증하므로 **업무시간 밖에는 fixture 생성 자체가 실패**한다. 직접 INSERT는 시간 무관·결정적·빠르다. `[R12]` 수검자 Write 는 이제 그 검사를 하지 않지만 fixture 는 그대로 직접 INSERT 다 — 결정적이고 빠른 것이 여전히 이유이며, `수검자ID`·`행버전` 을 시험이 직접 쥐어야 한다 |
...
결과메시지 문자열 비교로 C# 분기
Catalog 에 없는 실패를 기존 업무코드에 억지 매핑
```
...
|---|---|---|---|---|---|---|---|---|---|---|
| `INSERT_수검자` | Trim/NULL화/UPPER, 주민번호 13자리·날짜·세기·성별 검증, 생년월일/성별 산출 | 없음 | 잠금 직전 | `SSN` → `CHART`(수동) | 없음 | 동일 SSN 조회 → 성명+생년월일 후보 → 차트번호 고유성 → Sequence 발급 (`[R12]` 공통 업무가능 없음) | `수검자` 1행 INSERT + `변경이력` 컬럼 수만큼(트랜잭션 밖) | O | RS0(0 또는 2) + RS1 수검자결과 | 100~102, 201~203, 206 (202/203은 RS1 동반) |
| `UPDATE_수검자정보` | 동일 | `수검자ID`(입력) | 잠금 직전 | **`SSN` → `CHART` → `PAT`** (조건 없이 항상, §24.1) | 없음 | Patient 존재 → `행버전` → 실제 변경 여부(**NULL-safe**, §28.2) → 차트번호/SSN 고유성 → SSN 변경 시 RSV/RCP 부재 (`[R12]` 공통 업무가능 없음) | `수검자` 1행 UPDATE (No-op이면 없음) + `변경이력` 바뀐 컬럼 수만큼(트랜잭션 밖) | O | RS0(0 또는 1) + RS1(수검자ID, 차트번호, 행버전) | 100~102, 200~201, 204~205, 601 |
...
ResultCode Catalog는 `05` §4.2가 고정하며 잠금실패 코드가 없다. **개수를 여기 적지 않는다** — `정확히 38개`라 적어 두었다가 R7이 `800~899` 대역을 열며 거짓이 되었다(ROOT `AGENTS.md` §6). 인계 프롬프트 §5.9는 *"Catalog에 없는 실패를 억지로 기존 업무코드에 매핑하지 않는다"*고 못박았고, `05` §3.6은 *"예상하지 못한 SQL/시스템 오류 → THROW"*이다. 따라서 **`THROW`가 `05` 계약을 변경하지 않는 유일한 정합적 처리**다.
...
tests/02_Seed_Tests.sql       Exam 19행 / Holiday 41행 / 주민번호 체크디지트 무효
...
## 33.2a 업무일 분기 — 모든 Write 테스트 파일 공통 `[I]` `[X 수정 2차]` `[R13]`

`[R12]` **수검자 Write 둘은 이 절의 대상이 아니다.** `05` §10.1·§10.2 에서 공통 업무 가능조건이 빠졌으므로 `PWR-*` 는 업무시간 가드 없이 **언제나 실행**한다. 아래는 예약·접수 Write(`RWR-*`·`CWR-*`)에 대한 것이다.

예약·접수 Write SP는 검증순서에서 `308`/`309`(공통 업무 가능)를 **업무 Rule보다 먼저** 판정한다. 따라서 업무시간 밖에 실행하면 `300`·`305`·`411`·`0` 등을 기대하는 단언이 **전부 `308`/`309`를 받아 FAIL**한다. 초안은 `CWR-006` 한 곳에만 가드를 두고 나머지 Task의 기대 PASS 수를 무조건값으로 적었다.
...
`[R13]` **시각 축이 사라졌다.** 위 표의 `창 밖` 은 R12 까지 *요일·휴무일·시각* 셋을 뭉친 것이었다.
`04` §8.7 이 운영시간을 테이블로 꺼내면서 시각은 시험이 **만들 수 있는 조건**이 되었고, 남은
갈림은 요일·휴무일뿐이다. 그 둘은 `00` 이 **날짜로** 정한 것이라 넓히지 않는다 — 일요일·휴무일
회차에는 여전히 위의 `창 밖` 분기가 돌고, 그때도 판정하는 것은 `SKIP` 이 아니라 불변조건이다.

```text
PWR-*                 **언제나 실행**          [R12] 수검자 Write 는 308/309 를 내지 않는다
업무일                 RWR·CWR 실행            [R13] 시험이 창을 넓혔다 되돌린다
업무일 아님            RWR·CWR 는 불변조건만    성공 경로는 NOT RUN — 요일은 넓히지 않는다
OFF-308-*             **언제나 실행**          시험이 오늘을 휴무일로 심는다
OFF-309-*             **언제나 실행**          [R13] 시험이 창을 '지금이 아닌 1초' 로 좁힌다
```

`[R13]` **`OFF-309` 가 `OFF-308` 과 같은 방식이 되었다.** 둘의 비대칭은 규칙이 아니라 *휴무일은
데이터고 운영시간은 리터럴이다* 라는 우연이었다. 이제 둘 다 자기 조건을 자기가 만들고, **한
회차 안에서 `308` 과 `309` 가 함께 판정된다** — R12 까지는 어느 시각에 돌려도 한쪽만 볼 수 있었다.

```text
창 좁히기   오전이면 23:00:00~23:00:01 · 오후면 11:00:00~11:00:01
            둘 다 고정값이라 CK_운영기준_HOURS(시작 < 종료)를 항상 지나고,
            배치가 정오나 자정을 넘겨도 고른 창이 지금을 품지 않는다
```

`[!]` **넓힌 창은 같은 파일에서 되돌린다.** 되돌리지 못한 채 끝나면 다음 회차가 다른 제품을
시험한 `PASS` 를 낸다 — §43-14 가 배포 원본 수정을 금지한 그 이유가 여기에도 그대로 걸린다.
차이는 하나다: 배포 원본은 아무도 안 보지만, 이 테이블은 §14a 의 `OPR-G4` 가 회차마다 본다.
...
verify-contract-all.sh 가 루프 뒤에 한 번 더 지우고 **잔여 0건**을 판정한다.
**Seed 총계를 여기서 세지 않는다** — `휴무일 2건` 이라 적어 두었다가 R8 에서 41 이 되며 거짓이 되었고,
그 사이 아무 게이트도 이 문장을 보지 않았다. 총계는 `SED-008`·`VER-007`·`RBD-004` 가 판정한다 (ROOT `AGENTS.md` §6).
```

`[실측 2026-09-07 19:36]` `PASS OFF-308-01 … RS0 결과코드=308` · `PASS OFF-308-CLEAN 잔여 0건 · Seed 2건`.

`tests/05`·`06`·`07`·`08` 머리에 다음을 둔다. `[R13]` 창을 먼저 넓히고, 남은 갈림만 잰다.

```sql
UPDATE [dbo].[운영기준] SET [운영시작시각] = '00:00:00', [운영종료시각] = '23:59:59'
 WHERE [기준ID] = 1;   -- 파일 끝에서 읽어 둔 값으로 되돌린다
...
`[I]` 시간 경계(`CWR-009`)는 `UFN_HC_일정확인` 의 `@서버시각` 주입으로 **결정적으로** 시험한다. `[R13]` Write SP 경로도 더 이상 실제 시각에 의존하지 않는다 — 시나리오가 `운영기준` 의 마감을 옮겼다 되돌린다(§33.2a). 재는 것이 달라진 것은 아니다: 경계는 여전히 `RUL-T07`·`T08`·`T11`·`T12` 가 진짜 값으로 재고, `CWR-006`·`CWR-009` 는 *마감 전/후에 무엇이 일어나는가* 를 잰다.
...
`05` §13 허용 집합은 `tools/allowed-codes.json` 에 SP별 배열로 둔다. `05` §18이 *"사용되지 않는 ResultCode 0개"*를 PASS로 확정했으므로, **Catalog 중 한 번도 관측되지 않은 코드 목록**도 함께 보고한다.
...
| `RBD-004` | 배포 직후 객체 인벤토리 | Table 7 / TVF 4 / SP 20 / Sequence 1 / PK 7 / FK 2 / UQ 2 / UX 1 / NCI 5 / Trigger 0 |
| `RBD-005` | **연속 2회 Rebuild** 후 **정렬된 객체·Seed 덤프를 `diff`** | 차이 0줄 |
| `RBD-006` | 2회 Rebuild 후 Seed 행수 + **19행 전건 값** | Exam 19 / Holiday 41, 값까지 동일 |
...
`SYNC-001`~`004`·`SYNC-SELFTEST`(R15 의 `G17`) 다섯 · `V25`·`V26`(R14·R15) 둘 ·
`02b_수검자목록_차트번호_포함검색`(R17) 하나다. **시험이 늘어 수치가 올랐고 줄어든 것은 없다.**
이 회차가 한 일은 수를 늘린 것이 아니라 R18 이 `NOT RUN` 으로 미룬 자리를 닫은 것이다.

`[I]` **회차 `R12` 는 창 밖 수치가 처음으로 올라간 회차다.** 같은 창 밖인 `R9` 와 견주면
`PASS 227 → 273`(+46) · `SKIP 78 → 56`(−22) 이다. 수검자 Write 계약이 공통 업무 가능조건에서
풀려(`05` §10.1·§10.2) `PWR-001`~`031` 과 `PWR-*` 계약시험이 **창 밖에서 처음 판정됐기** 때문이다.
`SKIP` 은 `PASS` 가 아니므로 이 22건은 그동안 아무것도 증명하지 못하고 있었다.
`OFF-309-01` 은 뜻이 뒤집혀(`§43` 12) 업무시간 밖 수검자등록이 `309` 가 아니라 결과코드 `2` 를
낸다는 것을 재고, `309` 자체는 `OFF-309-02`(예약)가, `308` 은 `OFF-308-01`(예약)이 계속 판정한다.

`[X]` **회차 `R12` 는 창 안에서 돌지 않았다.** 그때는 `CON-001`~`008` 과 예약·접수 성공 경로가
업무시간 안에서만 판정됐고 그 회차의 `NOT RUN 9` 가 그것이다. R12 는 그 경로를 한 줄도 바꾸지
않았으므로 회차 `R9` 창 안의 증거가 그대로 유효했지만, 그 주장 자체가 확인되지 않은 채였다.

`[해소 2026-09-10 · R15]` **그 자리는 회차 `R13` 이 메웠고, 남은 한 가지는 다른 시험이 잰다.**
`R13` 이 `CON-001`~`008` 과 성공 경로 전건을 **창 밖에서** 판정했으므로(`PASS 403 · SKIP 0`)
"창 안에서만 판정된다" 는 조건 자체가 사라졌다. 남는 차이는 하나뿐이다 — 그 회차는 창을 **넓힌**
설정으로 돌았으므로 *진짜 `09:00~18:00` 에서 창 판정이 통과를 낸다* 는 것은 재지 않았다.
그것을 재는 시험은 따로 있고 **매 회차 돈다.**

```text
RUL-T02  09:00:00.0000000  -> 현재업무가능=1     운영시작 경계에서 창이 열린다
RUL-T03  17:59:59.9999999  -> 현재업무가능=1     운영종료 직전까지 열려 있다
RUL-T01  08:59:59.9999999  -> 309               그 앞은 닫혀 있다
RUL-T04  18:00:00.0000000  -> 309               그 시각부터 닫힌다
OFF-309-02                  -> RS0 결과코드=309   창 밖 Write 가 실제로 309 를 낸다
```

`RUL-T01`~`T04` 는 `UFN_HC_일정확인` 에 `@서버시각` 을 주입해 **진짜 값으로** 네 경계를 전부 재고,
그 진짜 값은 `04` §8.7 `운영기준` 에서 오며 `OPR-G3`·`G4` 가 그것이 `00` 과 같음을 매 회차 판정한다.
`V26` 은 그 값이 문서 사슬에서도 `00` 을 따라오는지 본다. **네 겹이 겹쳐 있으므로 창 안 회차가
따로 더할 것이 없다** — 평일 낮에 한 번 돌리면 좋지만, 돌리지 않았다고 비는 자리는 없다.

`[I]` **회차 `R13` 이 이 표에서 가장 강한 회차다 — 그리고 창 밖에서 돌았다.**
창 밖끼리 견주면 `R9` 가 `PASS 227 · SKIP 78 · NOT RUN 9`, `R12` 가 `273 · 56 · 9`, `R13` 이
**`403 · 0 · 1`** 이다. 창 **안** 최고 회차(`R9` 창 안 `399 · 2 · 1`)보다도 높다. 남은 `NOT RUN 1` 은
`RBD-001` 하나이며 그것은 인스턴스가 1개라 **구조적으로 불가능**한 음성 시험이다(§43-8).

세 가지가 이 회차에서 처음이다.

```text
예약·접수 Write 성공 경로와 RWR/CWR 계약 전건이 창 밖에서 판정됐다
OFF-308 과 OFF-309 가 한 회차 안에서 함께 판정됐다 — 둘의 배타성이 사라졌다
CON-001~008 이 창 밖에서 판정됐다 (2026-09-10 00시대 · 목 · 운영시작 9시간 전)
```

`04` §8.7 이 운영시간·마감시각을 테이블로 꺼냈기 때문이며, 시험은 그 창을 넓혔다 되돌린다(§33.2a).
되돌아온 것은 `OPR-G1`~`G4` 가 이 회차 안에서 두 번 판정했다(§14a).

`[!]` **`CON-001`~`008` 에는 아직 넓히지 않은 게이트가 하나 남아 있다.** barrier 가 `WAITFOR TIME`
이라 하루를 넘기면 timeout 이 없어 약 24시간 정지한다 — 그래서 `23` 시에 시작한 회차는 여덟을
`NOT RUN` 으로 남긴다(실측: 같은 트리를 2026-09-09 23:5x 에 돌린 예비 회차가 `PASS 386 · NOT RUN 9`
였다). **업무시간과 무관한, 넓힐 수 있는 창이 아닌 도구의 한계**다. 요일·휴무일도 마찬가지로 넓히지
않는다 — `00` 이 **날짜로** 정한 것이기 때문이다(§33.2a).
...
`PASS 389 · FAIL 0` 안에 실행되지 않은 시나리오가 하나 있었다는 뜻이다. R8 이 토큰을 고치고,
하니스가 `THROW 50001/50002` 가 없는 비정상 종료를 FAIL 로 읽게 했다(`§38`). 회차 `R8` 은 8개 시나리오
전부 `session A exit=0 / session B exit=0` 이다.
...
| G04 | Object Inventory | `05` §1.3·§1.4 가 선언한 객체가 전건 실재 | **PASS** (회차 `R8`) | `VER-001`~`VER-004` · `RBD-004` 지문. **수치를 여기 적지 않는다** — `SP 16` 이라 적어 두었다가 R7 에서 거짓이 되었다 (ROOT `AGENTS.md` §6) |
| G05 | Schema | `04` §8·§10.1 이 선언한 컬럼·Key·제약이 실제 DB 와 **양방향 차집합 0** | **PASS** (회차 `R8`) | `SCH-001`~`SCH-019` · `DOC-001`~`DOC-009`. **수치를 여기 적지 않는다** — `48컬럼·제약 24·Default 8` 이라 적어 두었다가 R7 에서 세 값이 **전부** 거짓이 되었다. 실측은 게이트가 낸다 (ROOT `AGENTS.md` §6) |
| G06 | 금지 객체 | Trigger 0 / TVP 0 / **업무 테이블을 지우는 SP 0** / 추가 Table 0 | **PASS** (회차 `R8`) | `VER-006` · `SCH-010`·`SCH-011`·`SCH-012`. R7 에서 `DELETE_자체휴무일` 이 생겨 조건을 좁혔다 — 금지의 뜻은 *업무 데이터를 지우지 않는다* 였다(§18) |
| G07 | Seed | `06` §13·§14 가 선언한 Seed 가 전건 값까지 일치 · 공휴일 등재 만료 전 | **PASS** (회차 `R8`) | `VER-007` · `RBD-006` · R7 부터 `verify-holiday-seed.sh` 가 §14 표와 `00` §7.4 임계를 함께 판정한다 |
| G08 | Rule | TGT/NEX/AEX/HOL 경계 전건 + `CORRUPT-3` 재검증금지 | **PASS** | `tests/03_Rule_Tests.sql` 51건 |
| G09 | SP Contract | `05` §7~§12 Parameter 와 `sys.parameters` 가 `EXCEPT` 양방향 0 · SP 전건 후속 RS · RS0 결과코드 가 `05` §13 허용집합 내 | **PASS** (회차 `R8`) | `SCH-019` · `V17` · `V18` · `HOL-001`~`007`. **건수를 여기 적지 않는다** — `Parameter 99`·`16/16` 이 R7 에서 거짓이 되었다 (ROOT `AGENTS.md` §6). R7 은 휴무일 SP 4개의 계약 시험이 0건인 채 `PASS` 였다 — R8 이 7건을 신설하고서야 "SP 전건" 이 참이 되었다 | `SCH-019` · `V17` · `V18`. **건수를 여기 적지 않는다** — `Parameter 99`·`16/16` 이 R7 에서 거짓이 되었다 (ROOT `AGENTS.md` §6) |
| G10 | Rollback | 부분저장 0건 | **PASS** | `RBK-001`~`RBK-007` · `RBK-008` 은 T-SQL 로 RS 개수를 셀 수 없어 `V18` 이 정적으로 판정 |
| G11 | Concurrency | `CON-001`~`008` + `rc=1` 1건 이상 + `Msg 1205`·`50002` 각 0건 + **세션 비정상 종료 0건** | **PASS** (회차 `R8`) | **8/8** · `applock rc=1` 시나리오마다 1~2건 · `1205`·`50002`·`50001`·`2627`·`결과코드 100` 각 **0건** (회차 `A`) |
...
| **G17** | **라이브 DB ↔ `deploy/`** `[R15]` | `SYNC-001` 모듈 이름 양방향 0 · `SYNC-002` 본문 일치 · `SYNC-003` 배포목록 == 파일 · `SYNC-004` App.config 가 가리키는 DB == 잰 DB | **PASS** (회차 `R15`) | `scripts/verify-live-sync.sh` — 회귀 안에서 rebuild 직후 매 회차 실행. **이 축은 R15 전까지 비어 있었다**: `G05` 는 `04` ↔ DB 를 보고 `RBD-005` 는 rebuild 두 번을 견주므로 *지금 붙어 있는 DB 가 배포 원본과 같은가* 를 묻는 것이 없었다. winforms 의 `TestCategory=Db` 시험이 rebuild 없이 살아 있는 DB 에 붙으므로 실제 위험이었다. 음성시험 확인: 배포 원본에 주석 한 줄을 더하면 그 모듈을 지목하고, App.config 의 Data Source 를 바꾸면 `SYNC-004` 가 잡는다. `[!]` `05` §1.1 은 Database 만 확정하고 **서버·인스턴스는 선언하지 않는다** — `CFG-002` 도 Initial Catalog 만 보므로 화면이 게이트가 재지 않은 DB 를 볼 수 있는 자리가 남아 있었다. 그 자리를 `SYNC-004` 가 닫는다. 모듈이 아닌 **테이블·제약·Index** 는 `G05` 가 같은 자리에서 본다 |
...
| # | 한계 | 영향 | 완화 |
|---:|---|---|---|
| 1 | ~~**업무시간 밖 Write SP 성공 경로 검증 불가**~~ `[해소 2026-09-10 · R13]` | 창 밖에서는 Write SP가 `308`/`309`로 끝나 성공 경로를 볼 수 없었다 | **해소했다.** 뿌리는 "시각이 SP 안 리터럴이라 시험이 창을 만들 수 없다" 였다. `04` §8.7 이 여섯 값을 `운영기준` 으로 꺼냈고, 시험이 창을 넓혔다 되돌린다(§33.2a) — 회차 `R13`(창 밖)에서 `SKIP 0` 이다. production SP 에 시간 주입 backdoor 는 **여전히 없다**: 바뀐 것은 SP 가 아니라 값이 사는 곳이다. 앞의 두 겹은 그대로 남아 이 겹을 교차 검증한다 — ① `RUL-T01`~`T12` 가 `@서버시각` 주입으로 **진짜 값의** 경계를 재고, ② 일요일·휴무일에는 `tests/05`~`08` 이 여전히 불변조건 분기를 판정한다 |
...
| 12 | **`OFF-309-01`·`02` 는 창 밖에서만 판정된다** | 창 안 회차에서는 `SKIP` 이고, `SKIP` 은 `PASS` 가 아니다 | **정의상의 배타성**이다 — 업무시간 안에서 `309` 는 나올 수 없다. 창 밖 회차에서 `PASS` 로 판정되며, 두 회차를 합치면 빈 구멍이 없다. §42 가 두 회차를 함께 근거로 든다. `[R12]` `OFF-309-01` 은 뜻이 뒤집혔다 — 이제 **업무시간 밖 수검자등록이 `309` 가 아니라 업무 결과를 낸다**는 것을 재며, `309` 자체는 `OFF-309-02`(예약)가 계속 판정한다. `[R13]` **더 이상 창 밖에서만 판정되지 않는다.** `OFF-309-02` 가 `운영기준` 의 창을 '지금이 아닌 1초' 로 좁혀 자기 조건을 자기가 만든다 — `OFF-308-01` 이 휴무일을 심는 것과 같은 방식이다. 배타성이 사라졌으므로 "두 회차를 합치면" 이라는 단서도 필요 없다 |
...
| 14 | ~~**시각 의존 경로 검증은 사람이 시계를 옮겨야 한다**~~ `[해소 2026-09-10 · R13]` | `CON-005`·`CON-008`·`CWR-006`·`CWR-009` 등 성공 경로는 특정 시각창에서만 성립했다 | **해소했다.** 옛 완화가 금지한 우회는 *배포 원본의 TVF 상수를 고치는 것* 이었고 **그 금지는 지금도 유효하다** — R13 은 그것을 하지 않았다. 값을 `04` §8.7 의 **테이블 행**으로 옮겼고, 회차는 배포 원본이 아니라 그 행을 옮겼다 되돌린다. 차이는 셋이다: ① `rebuild` 가 매번 `00` 값으로 되돌려 놓는다(`OPR-G3`), ② 되돌아왔는지를 회차마다 `OPR-G4` 가 **본다** — 배포 원본을 고치는 우회에는 그것을 보는 눈이 없었다, ③ 값의 소유자는 여전히 `00` 이고 관리 CRUD SP 가 없다(§14a · §43-21). `SYSDATETIME()` 이 시각의 유일한 입구인 것도 그대로이므로 OS 시각을 옮기는 옛 방법도 계속 성립한다 — 다만 더는 필요하지 않다. 남은 것은 **요일·휴무일과 자정 가드** 셋이며 이것들은 넓히지 않는다(§42) |
...
| 21 | **정원 20 이 조회 SP 와 저장 SP 에 따로 박혀 있다** `[완화 2026-09-08]` | 조회 SP 8곳·저장 SP 2곳에 리터럴로 있다. 한쪽만 고치면 화면은 "자리 있음" 을 보여주고 저장은 `305` 를 낸다 | 설정 테이블은 오히려 기준선 위반이다(`00` RP-03 이 정책 수치로 확정 · 관리자 CRUD 는 §8.2 밖). 대신 **양쪽을 실행으로 고정**했다 — `SEL-025` 가 조회 SP 의 `정원` 를 매 회귀에서(시각 무관) 보고, `CON-002`(19/20→20)·`CON-008`(20/20→305)이 저장 쪽을 경계에서 본다. 한쪽만 바뀌면 둘 중 하나가 반드시 깨진다. `[R13]` **운영시간·마감시각은 테이블로 옮겼는데 정원 20 은 안 옮긴 이유가 여기 있다.** 이 칸이 세운 기준은 *"설정 테이블"* 자체를 금지한 것이 아니라 **`00` 이 정책 수치로 확정한 값에 관리자 CRUD 를 붙이는 것**을 금지한 것이다(`00` §8.2). `운영기준` 은 셋을 다 지나간다 — ① 관리 CRUD SP 를 만들지 않았다(`05` §1.3 의 SP 목록에 없고 `UIDB-002` 가 그것을 본다), ② `OPR-G3` 가 Seed 값 == `00` 을 매 회차 판정한다, ③ `OPR-G4` 가 회차 끝에 되돌아왔는지 판정한다. 정원 20 은 ①~③ 을 붙여도 **얻는 것이 없다** — 조회·저장 양쪽이 이미 `SEL-025`·`CON-002`·`CON-008` 로 고정돼 있고, 시험이 그 값을 옮겨야 할 이유가 없다. 운영시간은 달랐다: 옮기지 못해 하루 아홉 시간 밖이 전부 `SKIP` 이었다(§43-1) |
| 32 | **수검자 조회가 Index seek 을 잃었다** `[R17 · 사용자 결정]` | 차트번호·이름을 포함검색(`LIKE '%값%'`)으로 열면서 앞의 `%` 때문에 `UQ_수검자_CHART_NO` 와 `IX_수검자_NAME_BIRTHDAY` 를 타지 못한다. 두 조건 중 하나만 넣어도 `수검자` 전건 scan 이다 | 2026-09-10 사용자 결정으로 **연다** — *일부만 아는 값으로 찾는 것* 이 접수 업무의 실제 모양이다. `04` §11.3 이 이름 포함검색을 금지했던 근거는 원래 둘이었는데, *가드 우회로 전건+주민번호 노출* 은 R16 이 전체조회를 열며 이미 사라졌고 남은 것이 이 seek 하나였다. `[I]` **주민번호·생년월일·휴대전화는 정확검색 그대로**라 `UQ_수검자_SOCIAL_NUMBER`·`IX_수검자_BIRTHDAY` seek 은 살아 있다 — 신원값으로 한 사람을 집는 경로는 안 느려진다. `[!]` **운영 전 다시 본다**: 수검자가 수만 건이 되면 접두검색(`LIKE '값%'`)으로 좁히거나 전용 Index 를 얹는 선택지가 있다. `§43-31`(전체조회의 주민번호 노출)과 같은 자리에서 함께 결정한다 |
| 31 | **수검자 전체조회가 주민번호를 전건 내보낸다** `[R16 · 사용자 결정]` | `03` §5.3 의 「최소 1개 조건」을 걷어 조건 없는 호출이 전체를 돌려준다. `05` §7.2 의 `RS1` 은 `주민번호` 13자리를 그대로 싣고(*"화면 표시 위치에서는 전체 13자리를 제공한다"*), 화면을 열 때마다 그 Result Set 이 클라이언트로 온다 | 2026-09-10 사용자 결정으로 **연다.** 과제는 `00` §2.1 이 임의 시험값만 쓰게 해 실피해가 없고, Grid 는 `주민번호` 를 기본 숨김 컬럼으로 둔다(`03` §17.3 · §18). `[!]` **운영 전 반드시 다시 본다** — `§43-20`(주민번호 평문 보존)과 같은 자리이고, 그때 결정할 것은 셋이다: 전체조회에 상한을 둘지, `RS1` 에서 `주민번호` 를 뺄지, 권한으로 막을지(`§43-7` 이 계정·권한을 폐기해 지금은 막을 수단이 없다). `[I]` 이름 `%검색어%` 포함검색 금지는 **걷지 않았다** — `04` §11.3 의 `[X]` 사고(가드 우회로 전건+주민번호 노출)가 막던 그 경로는 메타문자 이스케이프가 지킨다 |
| 22 | ~~**`700` 과 `701` 의 상대 순서가 SP 마다 반대다**~~ `[Phase 5 범위에서 해소 2026-09-10 · R15]` | `UPDATE_예약변경` 은 `700 → 701`, `UPDATE_접수추가검사` 는 `701 → 700` 이다 | `05` §5 가 둘을 한 칸("Master·Aggregate 데이터 구성")에 묶어 두어 어느 쪽도 위반이 아니다 — 각 SP 는 자기 계약(§11.2 vs §12.2)의 문장 순서를 정확히 따랐다. **화면이 둘을 가르지 않으므로 소비자에게는 차이가 없다**: `07` §6.1 이 `700`·`701` 을 한 칸에 묶어 *"Blocking · 사용자가 고칠 수 없는 데이터 구성 오류"* 로 같게 표현한다. 순서를 통일하려면 `05` 를 열고 SP 두 개의 본문을 고쳐야 하는데, **그것은 DB 변경이고 얻는 것이 없다** — 계약도 위반이 아니고 화면도 가르지 않는다. 실무에서 두 코드를 다르게 쓰기로 하는 날 다시 연다 |
| 23 | ~~**"무변경 성공" 의 업무시간 취급이 SP 마다 다르다**~~ `[해소 2026-09-09 · R12]` | 창 밖에 무변경 저장을 누르면 수검자수정·예약변경·추가검사는 `결과코드=1` 로 성공하고, 동일 주민번호 재등록은 `308`/`309` 로 실패했다 | **해소했다.** 비대칭의 뿌리는 "수검자 Master 를 검진 업무로 취급한 것" 이었다 — `05` §12 의 휴무일 SP 넷은 같은 기준정보인데 처음부터 `308`/`309` 를 적용하지 않았고, `01` 도 P01(수검자)에 업무일 분기를 두지 않았다. R12 가 `00` §1.1 에 적용 대상을 적고 `05` §10.1·§10.2 에서 그 줄을 빼면서 넷이 같은 규칙을 따르게 되었다 |
...
| 내용 정합성 | 문서 완결(23장, 말미 "최종 판정: GO"). 정원 20 / NEX 8~11 / AEX 7종 / 주민번호 13자리 / `수검자` 컬럼(수는 `04` §8.1.2가 단일 출처) / 예약변경 자기 Work 오탐 방지 / 취소 복원 없음 — 전부 `04`·`05`와 모순 없음 |
| 사본 존재 | `docs` 전체에서 0건 |
...
| # | 이탈 | 어떻게 green 이었나 | 조치 |
|---:|---|---|---|
| 1 | `OFF-308` 이 **한 번도 실행된 적이 없다** | 일요일·활성 휴무일을 기다리는 구성이라 평일 회차마다 `SKIP`. `SKIP` 은 집계에서 실패가 아니다 | 휴무일은 **데이터**다. 시험이 오늘을 활성 휴무일로 심고 지운다 → 아무 날 아무 시각에나 성립. 게이트가 루프 뒤에 잔여 0건을 확인 (총계는 SED-008·VER-007 이 본다) |
...
| 객체 | Table 7 / TVF 4 / SP 20 / Sequence 1 / PK 7 / FK 2 / UQ 2 / UX 1 / NCI 5 / CHECK 28 / Default 10 |
...
| `OFF` | `308`~`309` | 2 | `tests/contract/OFF-*` | `[R13]` 둘 다 **언제나** 판정된다 — `308` 은 시험이 오늘을 휴무일로 심고, `309` 는 `운영기준` 의 창을 '지금이 아닌 1초' 로 좁힌다. 한 회차 안에서 함께 판정되므로 배타성이 사라졌다 (§33.2a) | G09 |
