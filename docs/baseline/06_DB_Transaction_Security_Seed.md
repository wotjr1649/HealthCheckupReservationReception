...
- **문서 버전:** v1.11  (`[R12]` 회차별 버전을 여기 적지 않는다 — §4.1 표가 단일 출처이고 무엇을 고쳤는지는 아래 `RN 개정 범위` 가 적는다. 걷어낸 사본은 R6 부터 한 칸씩 밀려 있었고 아무 게이트도 그것을 보지 않았다. ROOT `AGENTS.md` §6)
...
- **기준선 ID:** `HC-RSV-RCP-20260910-R14`  (직전 `HC-RSV-RCP-20260910-R13`)
...
| `06_DB_Transaction_Security_Seed.md` | v1.11 | FINAL / GO / READ-ONLY | `HC-RSV-RCP-20260910-R14` |
...
`[X]` **창 안 회차를 아직 돌리지 않았다.** `CON-001`~`008` 과 예약·접수 성공 경로는 여전히
업무시간 안에서만 판정되며 이 회차의 `NOT RUN 9` 가 그것이다. R12 는 그 경로를 한 줄도 바꾸지
않았으므로 회차 `R9` 창 안의 증거가 그대로 유효하지만, **그 주장 자체는 창 안 회차로 확인해야 한다.**
