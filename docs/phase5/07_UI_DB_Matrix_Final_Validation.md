...
`[X]` **`03` §16 은 `LastEditDate` 를 수검자 Edit 의 숨은 값이라 적는다.** R7 이 수검자 동시성
토큰을 `최종수정일시` 에서 `행버전` 으로 바꿨고 `05` §16.5 가 *"R7 부터 Parameter·RS 컬럼이
아니다"* 라고 명시한다. **`05` 가 이긴다** — 수검자 Edit 도 `행버전` 을 보관한다. §14 `X-02`.
...
| ID | 위치 | 내용 | 이 문서의 처리 |
|---|---|---|---|
| `X-01` | `05` §4.2 vs §16.1 | `304` 의 C# Enum 이름이 `마감경과여부` 와 `CutoffPassed` 로 다르다 | §16.1 을 따른다 (§6.4) |
