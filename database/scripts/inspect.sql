...
DECLARE @업무ID  BIGINT        = NULL;   -- [3] 볼 업무. NULL 이면 검사구성이 가장 많은 업무
DECLARE @차트번호 NVARCHAR(100) = NULL;   -- [4] 볼 수검자. NULL 이면 업무가 가장 많은 수검자

IF @업무ID IS NULL
    SELECT TOP (1) @업무ID = [업무ID]
      FROM [dbo].[예약접수] ORDER BY LEN([국가검사항목]) DESC, [업무ID];

IF @차트번호 IS NULL
    SELECT TOP (1) @차트번호 = p.[차트번호]
