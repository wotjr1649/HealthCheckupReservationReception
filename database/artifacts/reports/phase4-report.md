...
`[!]` **회차 이름은 이 보고서의 것이다.** `06` §42 는 자기 표에서 `A`·`B`·`C` 로
매기며 그 셋은 여기의 `R3-안`·`R3-밖`·`R5-안` 이다. 예전에 양쪽이 문자를 독립적으로 매겨
같은 `C` 가 다른 회차를 가리켰다 — 그래서 여기서는 문자를 쓰지 않는다.

| 회차 | 시각 | 업무시간 | exit | PASS | FAIL | SKIP | NOT RUN | 로그 |
|---|---|---|---:|---:|---:|---:|---:|---|
| `R3-안` | 2026-09-07 12:19 | 안 | 0 | **357** | 0 | 2 | **1** | — |
| `R3-밖` | 2026-09-08 03:05 | 밖 | 0 | **185** | 0 | 78 | 9 | `artifacts/logs/full_test_run_off.log` |
| `R4-밖` | 2026-09-08 04:37 | 밖 | 0 | **185** | 0 | 78 | 9 | — |
| `R4-안` | 2026-09-08 12:59 | 안 | 0 | **357** | 0 | 2 | **1** | `artifacts/logs/full_test_run.log` |
| `R4G-밖` | 2026-09-08 05:12 | 밖 | 0 | **187** | 0 | 78 | 9 | `artifacts/logs/full_test_run_off.log` |
| `R5-안` | 2026-09-08 | 안 | 0 | **363** | 0 | 2 | **1** | `artifacts/logs/full_test_run_final.log` |

- 회차 `R3-안` 의 `SKIP` 2건은 `OFF-309-01`·`02` 뿐이다 — 업무시간 안에서 `309` 는 **정의상** 나올 수 없고, 회차 `R3-밖` 에서 `PASS` 로 판정된다.
- 회차 `R3-밖` 의 `SKIP` 78건은 업무시간 밖이라 성립하지 않는 Write SP 성공 경로다. 회차 `R3-안` 이 판정한다.

회차 `R4G-밖` 의 `187` 은 `R4-밖` 의 `185` 에 **R4-2·R4-3 게이트 두 줄이 더해진 것**이다. 처음에는 둘을
손으로 돌렸는데, 회귀 밖의 증거는 다음 회차에 썩는다 — `test.sh` 안으로 넣었다. 그래서
다음 창 안 회귀는 `357` 이 아니라 `359` 여야 맞다. **실측은 `363` 이었다**(회차 `R5-안`) — `V20`·`V21`·`V22` 와 `G13-c` 가 더 붙었다.

**R4 한글화는 판정 대상을 한 건도 줄이지 않았다.** 회차 `R4-밖`·`R4-안` 이 `R3-밖`·`R3-안` 과 `PASS`/`FAIL`/`SKIP`/`NOT RUN` 네 수치는 물론 **`SKIP`·`NOT RUN` 항목까지 같다** (`06` §46.4). R4 에서 새로 돈 것은 `verify-rs-contract` 170건과 `csharp-probe` 16건이다.

- 회차 `R3-안` 은 사용자가 머신 시각을 6시간 뒤로 옮겨 만든 창이다. **코드는 한 글자도 바꾸지 않았다** — `SYSDATETIME()` 이 시각의 유일한 입구다. 끝난 뒤 같은 크기로 되돌렸다.
- 두 회차 모두 최종 코드로 돌렸다. 회차 `R3-안` 의 `NOT RUN` 은 `RBD-001` 하나뿐이며 구조적으로 불가능하다.
...
**0건.** 모든 회차가 `FAIL 0` 이며 최종은 회차 `R5-안` 이다. 계획서 체크박스 미체크도 **0**이다.
...
11. **차트번호는 대소문자를 구분하지 않는다.** `UQ_수검자_CHART_NO` 가 CI·폭무시라 `c1` 과 `C1` 은
    ...
    사용자가 혼동할 여지가 줄어든다.
12. **마감 후 취소는 당일 복구가 안 된다 — 정책이다.** 16:00 이후엔 접수완료도 당일예약도 `304` 라
    그 시간에 오접수가 생길 수 없고, 이미 접수된 건을 취소하면 `CNC` 확정이 맞다(그날 예약이 이미 끝났다).
    확인 창에 **"오늘 접수는 마감됐습니다"** 를 보여주고, **다음 날 이후 예약은 지금도 된다**는 것을
    안내한다 — 다음 날 예약은 `CutoffTime NULL` 이라 시각과 무관하다(실측).
13. **주민번호가 `변경이력` 에 평문으로 남는다. 운영 전 보유기간·삭제 정책을 반드시 결정한다.**
    2026-09-08 사용자 결정으로 Phase 4 에서는 고치지 않았다 — `00` §2.1 이 임의 테스트값만 쓰게 해
    과제 범위에서 실피해가 없기 때문이다. 실제 주민번호를 넣는 순간 이 항목이 살아난다(`06` §43-20).
...
```text
artifacts/logs/full_test_run.log            회차 R4-안  (창 안 · 구 회차. R5 가 대체했다)
artifacts/logs/full_test_run_final.log      회차 R5-안  (창 안 · 최종)
artifacts/logs/full_test_run_off.log        회차 R3-밖 · R4G-밖 (창 밖)
