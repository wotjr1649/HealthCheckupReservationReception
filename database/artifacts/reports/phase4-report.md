...
|---|---|---|---:|---:|---:|---:|---:|---|
| `A` 창 안 | 2026-09-07 13:58 | 안 | 0 | 347 | 0 | 2 | 1 | `artifacts/logs/full_test_run.log` |
| `B` 창 밖 | 2026-09-07 20:14 | 밖 | 0 | 180 | 0 | 78 | 9 | `artifacts/logs/full_test_run_off.log` |
...
- 회차 `A` 는 `SCH-019` 신설과 `G00`·`G01`·`G05` 회귀 편입보다 앞선다. 그 셋은 회차 `B` 에서 판정됐다.
...
| G12 Security | **NOT RUN (범위 밖)** | 사용자 결정 (§32 · §43-7). `SEC-010` 만 범위 안이며 PASS |
...
```text
G12 Security   사용자 결정으로 범위 밖 (2026-09-07). SEC-010 만 범위 안이며 PASS
RBD-001        인스턴스 1개 — 음성 시험 구조적 불가
G13 (c)        자동 판정 불가 — REVIEWED
```
...
2. **User·GRANT 는 Phase 4 범위 밖**이다(사용자 결정). 운영 배포에서 `HC_APP_ROLE` 을 만들고 login 을 매핑하는 절차는 `06` §32.3 에 있다. `deploy/08_Security.sql` 은 placeholder 다.
...
5. **동시성 오류 두 개를 화면이 구분해야 한다.** `601`(다른 사용자가 먼저 변경 — 다시 읽고 재시도) 과 `50002`(교착 — 그대로 재시도).
