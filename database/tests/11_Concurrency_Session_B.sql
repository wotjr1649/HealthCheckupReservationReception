...
IF @Scen = 8
    -- 같은 Slot 의 RSV 하나. CONC2 것을 고정으로 집어 회차마다 같은 행을 쓴다.
    SELECT @W = w.[업무ID], @Rv = w.[행버전] FROM [dbo].[예약접수] w
     WHERE w.[수검자ID] = @P2 AND w.[예약일] = @Today AND w.[시간대코드] = 'PM' AND w.[상태코드] = 'RSV';
...
IF @Scen = 1
    EXEC [dbo].[USP_HC_INSERT_수검자] 1, NULL, N'동시등록비', @Ssn,
         NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, 1, N'CONC-B';
ELSE IF @Scen = 2
    -- A 와 **다른** 수검자로 **같은** Slot 을 노린다. 19/20 이라 한쪽만 들어가야 한다.
    EXEC [dbo].[USP_HC_INSERT_예약] @P2, 'NORMAL', '2026-11-16', 'AM', 0,0,0,0,0,0,0, N'CONC-B';
ELSE IF @Scen = 3
    -- A 와 **같은** 수검자로 **다른** Slot. RP-06 상 유효업무는 1건뿐이어야 한다.
    EXEC [dbo].[USP_HC_INSERT_예약] @P1, 'NORMAL', '2026-11-18', 'PM', 0,0,0,0,0,0,0, N'CONC-B';
ELSE IF @Scen = 4
    EXEC [dbo].[USP_HC_INSERT_예약] @P1, 'NORMAL', '2026-11-18', 'AM', 0,0,0,0,0,0,0, N'CONC-B';
ELSE IF @Scen = 5
    EXEC [dbo].[USP_HC_UPDATE_예약취소] @W, @Rv, N'CONC-B';
ELSE IF @Scen = 6
    ...
    EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @W, @Rv, 1,1,0,0,0,0,0, N'CONC-B';
ELSE IF @Scen = 7
    EXEC [dbo].[USP_HC_UPDATE_예약변경] @W, @Rv, '2026-11-18', 'AM', 0,0,0,0,0,0,0, N'CONC-B';
ELSE IF @Scen = 8
    EXEC [dbo].[USP_HC_UPDATE_접수완료] @W, @Rv, N'CONC-B';
