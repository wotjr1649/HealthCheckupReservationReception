...
BEGIN
    ...
    EXEC [dbo].[USP_HC_UPDATE_접수완료] @OW2, @ORv2, N'TEST';
    EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @OW, @ORv, 1,1,0,0,0,0,0, N'TEST';
    EXEC [dbo].[USP_HC_UPDATE_접수취소] @OW, @ORv, N'TEST';
...
DECLARE @W BIGINT, @Rv BINARY(8), @H0 INT, @H1 INT, @Cnt INT;

----------------------------------------------------------------------------
-- UPDATE_접수완료
----------------------------------------------------------------------------
-- CWR-001  미존재 WorkId 는 아무 행도 만들지 않는다
SET @Cnt = (SELECT COUNT(*) FROM [dbo].[예약접수]);
EXEC [dbo].[USP_HC_UPDATE_접수완료] -1, 0x0000000000000001, N'TEST';
IF ((SELECT COUNT(*) FROM [dbo].[예약접수]) = @Cnt)
...
EXEC [dbo].[USP_HC_UPDATE_접수완료] @Wf, @Rf, N'TEST';
...
EXEC [dbo].[USP_HC_UPDATE_접수완료] @Wp, @Rp, N'TEST';
...
EXEC [dbo].[USP_HC_UPDATE_접수완료] @Wcnr, @Rcnr, N'TEST';
...
BEGIN
    ...
    EXEC [dbo].[USP_HC_UPDATE_접수완료] @Wc2, @Rc2, N'TEST';
...
INSERT INTO [dbo].[예약접수] ([수검자ID], [예약일], [시간대코드], [상태코드], [국가검사항목], [추가검사항목])
VALUES (@P09, CONVERT(DATE, SYSDATETIME()), @Slot, 'RSV', @Basic, N'EX001');
DECLARE @Wc4 BIGINT = SCOPE_IDENTITY();
DECLARE @Rc4 BINARY(8) = (SELECT [행버전] FROM [dbo].[예약접수] WHERE [업무ID] = @Wc4);
EXEC [dbo].[USP_HC_UPDATE_접수완료] @Wc4, @Rc4, N'TEST';
...
-- CWR-003  stale RowVersion 은 전이시키지 않는다 (601 이 마감보다 앞이다)
...
BEGIN
    ...
    IF ((SELECT [상태코드] FROM [dbo].[예약접수] WHERE [업무ID] = @W) = 'RCP'
        ...
        AND (SELECT [시간대코드] FROM [dbo].[예약접수] WHERE [업무ID] = @W) = @Slot
    ...
    EXEC [dbo].[USP_HC_UPDATE_접수완료] @W, @Rv2, N'TEST';
...
BEGIN
    ...
    -- CWR-020  동일 AEX 집합은 No-op. RowVersion 이 바뀌지 않는다 (스펙 §28).
    EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @W14, @Rv, 1,0,0,0,0,0,0, N'TEST';
    ...
    EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @W14, @Rv, 1,1,0,0,0,0,0, N'TEST';
    ...
    SET @Rv = (SELECT [행버전] FROM [dbo].[예약접수] WHERE [업무ID] = @W14);
    EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @W14, @Rv, 0,0,0,0,0,0,0, N'TEST';
    ...
    SET @Rv = (SELECT [행버전] FROM [dbo].[예약접수] WHERE [업무ID] = @W14);
    EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @W14, @Rv, 0,0,1,0,0,0,0, N'TEST';
    IF ((SELECT [추가검사항목] FROM [dbo].[예약접수] WHERE [업무ID] = @W14) IS NULL)
        PRINT 'PASS CWR-023 성별 불충족 AEX 가 저장되지 않았다';
    ELSE BEGIN PRINT 'FAIL CWR-023 성별 불충족 AEX 가 저장됐다'; SET @Fail += 1; END

    -- CWR-026  stale RowVersion 은 아무것도 바꾸지 않는다
    SET @Rv = (SELECT [행버전] FROM [dbo].[예약접수] WHERE [업무ID] = @W14);
    EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @W14, 0x0000000000000001, 1,1,0,0,0,0,0, N'TEST';
    IF ((SELECT [행버전] FROM [dbo].[예약접수] WHERE [업무ID] = @W14) = @Rv)
...
BEGIN
    ...
    BEGIN
        ...
        EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @W11, @Rv11, 0,0,0,1,0,0,0, N'TEST';
...
EXEC [dbo].[USP_HC_UPDATE_접수추가검사] @Wrsv, @Rrsv, 1,0,0,0,0,0,0, N'TEST';
...
EXEC [dbo].[USP_HC_UPDATE_접수취소] @Wrsv, @Rrsv, N'TEST';
...
BEGIN
    ...
    -- CWR-041  stale RowVersion 은 취소하지 않는다
    EXEC [dbo].[USP_HC_UPDATE_접수취소] @W14, 0x0000000000000001, N'TEST';
    ...
    SET @H0 = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상키] = @W14);
    EXEC [dbo].[USP_HC_UPDATE_접수취소] @W14, @Rv, N'TEST';
    SET @H1 = (SELECT COUNT(*) FROM [dbo].[변경이력] WHERE [대상키] = @W14);
    ...
    EXEC [dbo].[USP_HC_UPDATE_예약변경] @W14, @Rv, '2026-11-18', 'AM', 0,0,0,0,0,0,0, N'TEST';
    IF ((SELECT [상태코드] FROM [dbo].[예약접수] WHERE [업무ID] = @W14) = 'CNC')
