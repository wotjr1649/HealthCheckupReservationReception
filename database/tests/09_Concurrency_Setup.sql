...
-- USP_HC_수검자정보_수정 가 새 주민번호를 검증하므로 무효값이면 시나리오가 성립하지 않는다.
...
----------------------------------------------------------------------------
-- CON-001  동일 주민번호 동시등록 : 그 주민번호가 없어야 한다. 위 DELETE 와 rebuild 가 보장한다.
-- CON-002  19/20 시간대 동시예약        : 00_Test_Harness 가 2026-11-16 AM 에 F001~F019 19건을 심는다.
-- CON-003  동일 Patient 다른 시간대     : CONC1 에게 유효업무가 없어야 한다. 위 DELETE 가 보장한다.
...
BEGIN
    ...
    --   접수마감 전이어야 한다(304). PM 시간대 은 마감이 16:00 이라 11:00~15:50 창을 쓸 수 있다 (스펙 §38.7).
    INSERT INTO [dbo].[예약접수] ([수검자ID], [예약일], [시간대코드], [상태코드], [국가검사항목], [추가검사항목])
    VALUES (@P1, @오늘날짜, 'PM', 'RSV', @Basic, NULL);
...
BEGIN
    ...
    --       행버전 이 그대로 남고 B 도 성공해 버려 601 이 영영 관측되지 않는다.
    INSERT INTO [dbo].[예약접수] ([수검자ID], [예약일], [시간대코드], [상태코드], [국가검사항목], [추가검사항목])
...
BEGIN
    ...
    --   오늘 PM 시간대 을 RSV 20건으로 채운다. B 가 그중 하나를 접수완료(RSV -> RCP)하는 사이
    ...
    UPDATE w SET w.[예약일] = @오늘날짜, w.[시간대코드] = 'PM'
      FROM [dbo].[예약접수] w
      JOIN [dbo].[수검자] p ON p.[수검자ID] = w.[수검자ID]
    ...
    INSERT INTO [dbo].[예약접수] ([수검자ID], [예약일], [시간대코드], [상태코드], [국가검사항목], [추가검사항목])
    VALUES (@P2, @오늘날짜, 'PM', 'RSV', @Basic, NULL);

    DECLARE @N INT = (SELECT COUNT(*) FROM [dbo].[예약접수]
                       WHERE [예약일] = @오늘날짜 AND [시간대코드] = 'PM'
                         AND [상태코드] IN ('RSV', 'RCP'));
