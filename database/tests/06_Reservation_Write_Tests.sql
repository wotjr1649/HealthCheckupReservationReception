...
--
-- 업무일 가드 (스펙 §33.2a). Write SP 는 308/309 를 업무 Rule 보다 먼저 판정한다.
...
BEGIN
    ...
    IF @Off0 = @Off1 AND @@TRANCOUNT = 0
        PRINT 'PASS RWR-OFF 업무일 밖 Write SP 호출이 DB 를 바꾸지 않았다  ' + @Off1;
    ELSE BEGIN PRINT 'FAIL RWR-OFF 창 밖 호출이 데이터를 바꿨다  ' + @Off0 + ' -> ' + @Off1;
               SET @Fail += 1; END

    PRINT 'NOT RUN RWR-001~052 업무일(월~토, 비휴무일) 밖 - 요일·휴무일은 00 이 날짜로 정한 것이라 넓히지 않는다';
...
DECLARE @W BIGINT, @Rv BINARY(8), @Cfg NVARCHAR(200), @국가검사 NVARCHAR(100), @추가검사 NVARCHAR(50);
...
EXEC [dbo].[USP_HC_예약_등록] @Pt,  'NORMAL', '2020-01-06', 'AM', 0,0,0,0,0,0,0, N'TEST';  -- 300 과거일
EXEC [dbo].[USP_HC_예약_등록] @Pt,  'NORMAL', '2026-11-22', 'AM', 0,0,0,0,0,0,0, N'TEST';  -- 301 일요일
EXEC [dbo].[USP_HC_예약_등록] @Pt,  'NORMAL', '2026-12-25', 'AM', 0,0,0,0,0,0,0, N'TEST';  -- 302 휴무일
EXEC [dbo].[USP_HC_예약_등록] @Pt,  'NORMAL', '2026-11-21', 'PM', 0,0,0,0,0,0,0, N'TEST';  -- 303 토요일 PM
EXEC [dbo].[USP_HC_예약_등록] @Pt,  'WALKIN', '2026-11-16', 'AM', 0,0,0,0,0,0,0, N'TEST';  -- 102 WalkIn 날짜
EXEC [dbo].[USP_HC_예약_등록] @P18, 'NORMAL', '2026-11-17', 'AM', 0,0,0,0,0,0,0, N'TEST';  -- 400 만 18세
EXEC [dbo].[USP_HC_예약_등록] @Pt,  'NORMAL', '2026-11-17', 'AM', 0,0,1,0,0,0,0, N'TEST';  -- 411 남성 OPT03
...
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-17', 'AM', 1,0,0,0,0,0,0, N'TEST';
...
-- RWR-009  검사구성 = NEX 8~11 + 유효선택여부 AEX. 빈 문자열은 0개다 (스펙 §21.2a).
SELECT @국가검사 = [국가검사항목], @추가검사 = [추가검사항목] FROM [dbo].[예약접수] WHERE [업무ID] = @W;
IF ((CASE WHEN LEN(ISNULL(@국가검사, N'')) = 0 THEN 0
          ELSE LEN(@국가검사) - LEN(REPLACE(@국가검사, N',', N'')) + 1 END) BETWEEN 8 AND 11
    AND @추가검사 = N'EX014')
...
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-18', 'AM', 0,0,0,0,0,0,0, N'TEST';
EXEC [dbo].[USP_HC_예약_등록] @Pt, 'NORMAL', '2026-11-19', 'AM', 0,0,0,0,0,0,0, N'TEST';
...
EXEC [dbo].[USP_HC_예약_등록] @P2, 'NORMAL', '2026-11-19', 'AM', 0,0,0,0,0,0,0, N'TEST';
EXEC [dbo].[USP_HC_예약_등록] @P2, 'NORMAL', '2026-11-20', 'AM', 0,0,0,0,0,0,0, N'TEST';
...
EXEC [dbo].[USP_HC_예약_변경] @Wcnr, @RvCnr, '2026-11-18', 'PM', 0,0,0,0,0,0,0, N'TEST';
...
DECLARE @시간대인원 INT = (SELECT COUNT(*) FROM [dbo].[예약접수]
                      WHERE [예약일] = '2026-11-16' AND [시간대코드] = 'AM'
                        AND [상태코드] IN ('RSV', 'RCP'));
IF @시간대인원 = 20 PRINT 'PASS RWR-012 사전조건 20/20 성립';
ELSE BEGIN PRINT 'FAIL RWR-012 사전조건 Slot=' + CONVERT(VARCHAR(5), @시간대인원); SET @Fail += 1; END
...
-- RWR-030  20/20 시간대 에 있는 Work 의 AEX 변경은 305 로 막히지 않는다 (스펙 §30.1).
...
EXEC [dbo].[USP_HC_예약_변경] @W30, @Rv30, '2026-11-16', 'AM', 1,0,0,0,0,0,0, N'TEST';
...
-- RWR-020  No-op 은 행버전 을 바꾸지 않는다 (스펙 §28)
EXEC [dbo].[USP_HC_예약_변경] @W, @Rv, '2026-11-17', 'AM', 1,0,0,0,0,0,0, N'TEST';
...
-- RWR-025 / RWR-027  stale 행버전 과 미존재 업무ID 는 아무것도 바꾸지 않는다
EXEC [dbo].[USP_HC_예약_변경] @W, 0x0000000000000001, '2026-11-18', 'PM', 1,0,0,0,0,0,0, N'TEST';
EXEC [dbo].[USP_HC_예약_변경] -1, @Rv, '2026-11-18', 'PM', 1,0,0,0,0,0,0, N'TEST';
IF ((SELECT [행버전] FROM [dbo].[예약접수] WHERE [업무ID] = @W) = @Rv
    AND (SELECT [예약일] FROM [dbo].[예약접수] WHERE [업무ID] = @W) = '2026-11-17')
...
EXEC [dbo].[USP_HC_예약_변경] @W, @Rv, '2026-11-17', 'PM', 1,0,0,0,0,0,0, N'TEST';
...
EXEC [dbo].[USP_HC_예약_변경] @W, @Rv, '2026-11-19', 'AM', 1,0,0,0,0,0,0, N'TEST';
...
EXEC [dbo].[USP_HC_예약_변경] @W29, @Rv29, '2026-11-25', 'AM', 0,0,0,0,0,0,0, N'TEST';
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM [dbo].[UFN_HC_국가검사구성](@P31, @D31) WHERE [검사항목코드] = 'EX012')
    BEGIN PRINT N'FAIL RWR-031 사전조건 변경 전에 이미 EX012 가 있다'; SET @Fail += 1; END
    ELSE
    BEGIN
        EXEC [dbo].[USP_HC_예약_변경] @W31, @Rv31, '2026-11-20', 'AM', 0,0,0,1,0,0,0, N'TEST';
...
BEGIN
    ...
    EXEC [dbo].[USP_HC_예약_변경] @W34, @Rv34, '2026-11-19', 'AM', 0,0,0,0,0,0,0, N'TEST';
...
-- RWR-044  취소는 그 시간대 의 정원을 1 줄인다 (00 RP-03 의 현재인원 정의)
IF ((SELECT COUNT(*) FROM [dbo].[예약접수]
      WHERE [예약일] = '2026-11-16' AND [시간대코드] = 'AM'
