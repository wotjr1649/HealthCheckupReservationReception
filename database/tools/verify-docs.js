...
// "같은 줄에 R2 가 있으면 통과" 같은 휴리스틱은 그 자체가 우회 통로다.
{
  const R2NUM = [
    ...
    // ['CHECK 23'] 은 뺀다 — CK_검사코드_CODE_FORMAT 신설로 23 이 R3 최종값이 되었다.
    // 22 -> 23 은 R2 잔재가 아니라 정상 수치다. CHECK 개수는 V08 이 04 §8 실측과 직접 대조한다.
    // `5개 업무/조회 Nonclustered Index` 처럼 수량사가 **앞**에 오는 형태를 초판이 놓쳤다(04 §0.4.2).
    ['NCI 5',         /\bNCI\s*5(?![0-9])|Nonclustered Index\s*5(?![0-9])|5\s*개[^0-9\n]{0,16}?(?:Nonclustered Index|NCI)\b/],
