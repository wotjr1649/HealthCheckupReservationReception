...
이 파일은 **모든 세션이 읽는다.** `database/`·`winforms/` 어디서 시작하든 계층으로 로드된다.
디렉터리별 작업 경계는 그 디렉터리의 `CLAUDE.md` 가 따로 적는다 — 여기서는 저장소 전체를 다스리는 것만 적는다.
...
database/           Phase 4 DB 의 SQL·스크립트·테스트·증거   → database/CLAUDE.md
