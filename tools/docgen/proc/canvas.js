...
class Canvas {
  ...
  toSvg(px = 1600) {
    ...
    // 화살촉 marker — PPTX의 endArrowType:'triangle' 과 눈으로 대조하려면 SVG에도 있어야 한다.
    const arrowColors = [...new Set(ITEMS.filter(i => i.t === 'line' && i.arrow).map(i => i.stroke))];
    const defs = arrowColors.map(col =>
      `<marker id="ar-${col}" viewBox="0 0 10 10" refX="9.5" refY="5" markerWidth="5" markerHeight="5" orient="auto" markerUnits="strokeWidth"><path d="M 0 0 L 10 5 L 0 10 z" fill="#${col}"/></marker>`).join('');
    ...
    for (const i of ITEMS) {
      if (i.t === 'rect') {
        ...
        o.push(`<line x1="${(i.x1 * k).toFixed(2)}" y1="${(i.y1 * k).toFixed(2)}" x2="${(i.x2 * k).toFixed(2)}" y2="${(i.y2 * k).toFixed(2)}" stroke="#${i.stroke}" stroke-width="${(i.sw / 72 * k).toFixed(2)}"${dash(i.dash)}${i.arrow ? ` marker-end="url(#ar-${i.stroke})"` : ''}/>`);
      } else if (i.t === 'text') {
