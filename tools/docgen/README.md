...
여섯 종을 만들고 **반드시** `verify_output.js` 를 지난다. 생성기를 하나씩 손으로 돌리면
마지막 검사를 빼먹는다 — 실제로 `build_00.js` 의 금지 문자열 1건이 오래 떠 있었다.

## 공개 적합성 — `verify_output.js`

산출물은 사내 공개용이다. 저장소 안에서만 뜻이 있는 통제 어휘와 따라갈 수 없는 절 참조가
한 글자도 새어 나가면 안 된다. 여섯 종의 **실제 파일**에서 텍스트를 뽑아 본다
(`xlsx` 는 exceljs, `pptx` 는 슬라이드 XML 의 `<a:t>`). 한 건이라도 남으면 `exit 1` 이다.

```text
막는다   FINAL · READ-ONLY · 기준선 · HC-RSV-RCP · 변경 통제 · 최종 검수 · 최종 판정
         시험 판정 네 낱말 · 회귀 · 커밋 · git · Phase · 후속 문서 · 적대적
         실측 · v1.0 · CANDIDATE · R2/R3/R4 · 절 참조 · 과제
허용한다  검사결과 입력·판독·최종 판정   00 §8.2 제외 범위. 업무 내용이다
         일정 검증 게이트 · 게이트 1~7   01 의 업무 용어다
         ReadOnly                    화면 용어. READ-ONLY 와 대소문자가 다르다
```

```text
반드시 넣는다  03 화면설계서 — 「설계 시점의 화면 정의」 · 「동작의 기준은 실행 프로그램」
               이 두 조각이 없으면 exit 1. 문구는 wireframe/screens/a0_list.js 가 갖는다
```

`[X]` **막는 규칙만 있으면 「무엇이 빠졌는가」는 아무도 안 본다.** `03` 은 설계 시점의
기록이고 실행 프로그램과 다른 부분이 있는데, 그 사실이 공개본 안에 없으면 받아 보는 쪽은
그림과 다른 프로그램을 결함으로 읽는다. 저장소 안 문서에 적는 것으로는 닿지 않는다.

`[!]` **`selftest` 가 있다.** `node tools/docgen/verify_output.js selftest` — 가짜 입력으로
규칙이 실제로 잡는지 본다(산출물이 아예 없을 때 조용히 통과하지 않는 것까지). `build_all.js`
가 판정 직전에 이것을 먼저 돌린다.

`[X]` **오탐만 내는 규칙은 넣지 않는다.** `폐기`(입력을 폐기한다)와 초기데이터 관련 낱말을
넣었다가 뺐다 — 참 적중이 0 이고 예외 목록만 키웠다. 예외 목록이 곧 우회 통로다.

`[!]` 절 참조는 생성기 **주석**에는 남는다. 코드를 읽는 사람에게는 근거이기 때문이다.
산출물로 나가는 문자열에서만 없앤다 — `xlsx/md.js` 의 `pub()` 이 셀에 쓰는 모든 값을 거른다.
괄호로 묶인 참조는 지우고, 문장 한가운데 박힌 참조는 **지우지 않고 실패시킨다**
(`§8.2.3의 정렬 누수` 를 지우면 `의 정렬 누수` 가 되어 말이 깨진다).
`wireframe/screens` · `proc/slides` 의 일괄 정리는 `strip_refs.js` 가 한다.
