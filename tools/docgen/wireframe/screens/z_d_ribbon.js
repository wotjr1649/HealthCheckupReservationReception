'use strict';
// 부록 D — Ribbon Action Matrix + 당일 Context 마감 (원문 03 §9.6 / §9.7 / §8.6 / §5.2 / §8.10 / §23.4)
...
const pages = tablePages('부록 D.  Ribbon Action · 마감 Matrix', [
  ...
  },
  {
    ...
    lines: ['[접수]는 상태변경 버튼이 아니라 접수 Context / P03으로 연결하는 Shortcut이다.',
      '[변경이력]은 상태와 무관하게 행이 선택되면 열린다 — 취소된 업무의 변경 내역도 남아 있고 그것을 보는 것이 열람의 목적이다.',
      '공통 업무불가는 이 표에 행이 없다 — Action을 닫지 않고 상태영역에만 나타난다. 예약 Write는 저장 시점에 DB가 308/309로 막고 화면은 그 결과를 Blocking으로 보인다.'],
  },
  {
    ...
    cols: [
      ...
      { t: '추가검사변경', flex: 1 }, { t: '접수취소', flex: 1 }, { t: '변경이력', flex: 1 },
    ],
    rows: [
      ['미선택', 'O', 'X', 'X', 'X', 'X', 'X'],
      ['예약 (RSV)', 'O', 'O', '조건부 O', 'X', 'X', 'O'],
      ['접수완료 (RCP)', 'O', 'X', 'X', 'O', 'O', 'O'],
      ['예약취소 (CNR)', 'O', 'X', 'X', 'X', 'X', 'O'],
      ['접수취소 (CNC)', 'O', 'X', 'X', 'X', 'X', 'O'],
    ],
  },
  {
    kind: 'note',
    lines: ['[현장 당일예약]은 선택행과 무관한 독립 Action이다. 이 버튼 하나가 「수검자 확정 → 당일 업무 확인 → 신규예약 WalkIn(예약일 = 오늘 ReadOnly) → Workbench 자동선택 → 접수 처리」 흐름의 유일한 진입점이다.',
      '[변경이력]은 예약 Context Ribbon과 같다 — 상태와 무관하게 행이 선택되면 열린다. 공통 업무불가가 이 표에 행을 갖지 않는 것도 같다.'],
  },
  {
    ...
    cols: [
      ...
      { t: '신규예약', flex: 1 }, { t: '변경이력', flex: 1 }, { t: '컬럼설정', flex: 1 },
    ],
    rows: [
      ['행 미선택', 'O', 'O', 'X', 'X', 'X', 'O'],
      ['행 선택', 'O', 'O', 'O', 'O', 'O', 'O'],
  ...
  {
    kind: 'note',
    lines: ['행 미선택에서는 대상 행이 없으므로 [정보수정]·[신규예약]·[변경이력]이 닫힌다.',
      '공통 업무불가는 이 표에 행이 없다 — 수검자 등록·수정은 기준정보 정비라 애초에 308/309의 대상이 아니고, 화면도 그 상태에서 Action을 닫지 않는다.'],
  },
  {
