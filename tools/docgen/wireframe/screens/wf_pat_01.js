...
const desc = [
  ...
  { n: '1', text: '행 미선택이면 [정보수정]·[신규예약]·[변경이력] 비활성' },
  { n: '', text: '공통 업무불가는 Action을 닫지 않는다 — 상태영역 안내이고 판정은 저장 시점에 DB가 한다' },
  { n: '2', text: '조건 없으면 전체 조회. 차트번호·이름은 포함검색. 조건은 AND 결합' },
  { n: '3', text: '차트번호·주민번호·생년월일·휴대전화는 정확검색, 이름은 앞부분 일치' },
  { n: '4', text: '단일 행 선택. 재조회 시 선택행과 상세를 초기화한다' },
  { n: '5', text: '주민등록번호는 마스킹 없이 전체값을 표시한다 (SocialNumber)' },
  { n: '6', text: '삭제는 제공하지 않는다. 정보 오류는 [정보수정]으로 처리 (EP-10)' },
  { n: '7', text: 'PatientId·정규화 컬럼·생성/수정시각은 Grid에 노출하지 않는다' },
];
