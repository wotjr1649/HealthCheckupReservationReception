...
const LEGEND = [
  ['프로그램', '성인 일반건강검진의 예약과 접수를 처리하는 원내 프로그램. 상단 진입점 4개(수검자 관리 / 신규 예약 / 예약 관리 / 접수 관리), 업무 Tab 3개, Modal 7개, 확인창 2개로 구성한다.'],
  ['동반 문서', '「00_검진_예약접수_업무정책」 CP · EP · RP · RCP 정책과 TGT · NEX · AEX · HOL Rule의 개별 내용   /   「01_검진_예약접수_업무프로세스」 P01~P03 단계 · 분기 · 상태전이   /   「02_검진_예약접수_기능정의」 Function ID 16개의 기능 계약   /   「03_검진_예약접수_화면설계서」 본 문서. 네 문서는 하나의 세트이며 정책 ID · 프로세스 ID · 기능 ID로 서로를 참조한다.'],
  ['화면 용어', 'PatientId 수검자 Master 내부 식별자   ·   WorkId 예약/접수 업무 1건의 식별자   ·   Normal 일반 신규예약 Context   ·   WalkIn 현장 당일예약 Context(예약일 = 오늘 고정 ReadOnly)   ·   RowVersion / LastEditDate 저장 시 함께 전달하는 동시성값 (§8.1, §9.1, §16)'],
  ['업무 상태코드', '예약(RSV) · 접수완료(RCP) · 예약취소(CNR) · 접수취소(CNC). 취소는 물리 삭제하지 않고 같은 행의 상태만 바꾸며 복원하지 않는다. (§14.1, CP-05)'],
  ['검증 표현 원칙', '미리 판단 가능한 선택불가 → Disabled + 사유  /  입력필드 오류 → Inline 오류  /  저장·상태전이 최종차단 → Blocking Message + 중단  /  동시성 충돌 → Blocking Message + 최신값 Refresh (§15.1)'],
  ['규칙 ID 출처', 'CP / EP / RP / RCP / TGT / NEX / AEX / HOL 은 「업무정책」 문서에 정의된 규칙 ID다. 각 화면 설명의 괄호 안 표기가 그 규칙을 가리킨다.'],
  ['대상 환경', 'C# WinForms · .NET Framework 4.6.1 · DevExpress Components 20.2 · MSSQL. 최소 검증 해상도 1366×768, 권장 1920×1080. Windows 배율 100% / 125%. (§1.4)'],
  ['공통 업무조건', '휴무일이거나 운영시간(09:00~18:00) 밖이면 모든 화면의 업무 Action을 비활성한다. 화면 표시는 안내값이며 저장 성공을 보장하지 않는다. (§1.3, CP-01~04)'],
  ['구현 제외', '로그인·권한, 통계 Dashboard, 실제 검사 수행·결과 입력·판독, 공단 대상자 API, 휴무일·검사 Master·AEX 관리자 CRUD, 가격·할인·수납·결제, 수검자 삭제·복원, 출력·인쇄·엑셀 내보내기, 사용자별 Grid Layout 저장. (§20)'],
];
