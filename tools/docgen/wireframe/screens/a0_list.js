...
const LEGEND = [
  // 이 한 줄이 공개본이 스스로를 소개하는 자리다. 받아 보는 쪽은 저장소를 열지 않으므로,
  // 여기 없으면 그림과 다른 프로그램을 결함으로 읽는다. verify_output.js 가 지킨다.
  ['문서의 지위', '이 문서는 설계 시점의 화면 정의다. 개발 과정에서 사용성 판단에 따라 실제 화면과 달라진 부분이 있으며, 동작의 기준은 실행 프로그램이다. 업무 규칙과 항목의 의미는 이 문서가 기준이다.'],
