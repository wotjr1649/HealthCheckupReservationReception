...
function draw(c) {
  const v = K.shell(c, {
    nav: ['수검자 관리', '신규 예약', '예약 관리', '접수 관리'], navActive: 3,
    ribbon: [
...
const desc = [
  ...
  { n: '', text: '예약 Context는 [현장 당일예약]·[추가검사변경]·[접수취소] 없이 [예약취소]가 대신 있다 (§9.6). 두 Context의 상태별 활성/비활성 40칸은 부록 D 참조. 접수 Context 미선택 상태에서 활성인 것은 [조회]·[컬럼설정]·[현장 당일예약]뿐이다.' },
