...
const pages = tablePages('부록 B.  조회 · 검색 계약', [
  {
    ...
    rows: [
      ...
      ['최소 조건', '없다. 조건이 하나도 없으면 전체 목록을 조회한다.'],
      ['결합', '입력된 조건은 AND로 결합한다.'],
      ['포함검색', '차트번호와 이름은 친 값이 어디에 들어 있든 찾는다.'],
      ['정확검색', '주민번호 · 생년월일 · 휴대전화는 정규화 후 정확검색한다.'],
      ['메타문자', '사용자가 친 % _ [ 는 글자 그대로 찾는다.'],
  ...
  },
  {
    ...
    rows: [
      ['수검자 관리\n(WF-PAT-01)', '차트번호 → 이름 → 생년월일 → 성별 → 휴대전화번호',
        ...
        '내부키(PatientId · WorkId), 정규화 컬럼, 동시성값(RowVersion), 생성·수정시각, 업무 미사용 컬럼'],
